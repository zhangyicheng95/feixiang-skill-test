...
3. **状态变化** → 等"状态确实变了"再断言，按以下优先级选等待方式
   ...
   2. `expect(...).to_have_class(/done/, timeout=5000)` —— 次之
   3. `wait_for_timeout(≤2000ms)` —— 最后兜底，**单条 ≤ 2000ms 且累计 ≤ 20s**
      （与前置条件中的 20s 总等待预算闭环对齐）
...
```python
# must-cover from user request:
# [1] 横向/竖向火柴可拖拽          [2] 摆出正确数字 → √ 出现
# [3] 表扬语言出现                 [4] 摆错 → "想一想" 提示
# [5] 返回上一步功能               [6] 重新开始清屏
...
> 提示：(a)(b)(c) 的兜底等待全部累计计入 [前置条件](#前置条件) 中"总等待 ≤ 20 秒"的预算。
> 若多次 5.3 等待累计已逼近 20s，请拆分到下一次 `test_html` 调用，不要硬塞进一次。
...
**唯一推荐的诊断模式，[失败处理](#测试结果处理) 里所有指向"诊断"的步骤都来这里**。

适用场景：
- 第一次测试某个新生成的 HTML，对实际 DOM 结构、文本内容、内部 state 不确定
- 测试反复失败、想"调宽断言"前——必须先来这里看实际值，不能拍脑袋改
- [失败处理](#严禁用同一段测试代码反复测试不同-html-版本) 中第 2 次同代码失败，强制切到这里

```python
# 1. Python print 输出（注意：当前云函数只回传 page console，Python stdout 暂不回传，
#    建议主用 page.evaluate(...) + console.log 把可疑值送到 page console）
texts = page.locator(".item").all_inner_texts()
print("DEBUG items =", texts)

# 2. 推荐：通过 page.evaluate + console.log 把值灌进 page console，结果会随 consoleErrors 回传
page.evaluate("""() => {
...
# 3. 用最弱断言让本次测试先通过（拿到诊断信息），下一次再精确断言或修 HTML
assert page.locator("#sun-latitude").count() > 0, "目标元素根本不存在 → HTML 没生成对应节点"
```

> 重要：当前云函数响应里 `console` 字段只承载 **page 浏览器 console**。Python `print` 的 stdout
> 暂不回传，所以诊断时务必用 `page.evaluate("console.log(...)")` 而不是只 `print`，
> 否则下一轮决策时拿不到诊断结果。
...
1. **回看顶部 must-cover 清单**：逐条检查测试代码里是否有对应断言（建议用清单标号 `[1] [2] ...`
   去 grep 自己的 `playwrightCode`），任何一条 `[N]` 在测试代码里没出现 → **未真正通过**
2. **若清单里有未覆盖项**：本次"通过"是**部分通过**，不能视为交付完成。补一次 `test_html` 调用
   测掉漏项，或把漏项整合到下一次回归测试里，再继续后续流程
3. **若全部 `[N]` 都有断言且 pass**：才视为真正通过，可以推进后续步骤

> 这一步是为了堵住 chatInput=261 反例（4 个 locator + 4 个 enabled 就交付）的口子。
> 没有这一步，"指标好看但实质浅"会持续发生。
...
3. **判定流程（必经路径）**：
   ...
   失败
     ...
     │           → 测试自身太慢：拆调用 / 削减视口循环 / 缩短 wait_for_timeout，HTML 不动
     │
     └─ 把诊断结果作为修复决策依据
   ```

4. **执行修复**：按 3 决策的方向，用 `edit_file` 修改对应文件，拿到新 resourceId 后重新测试。
...
- 如果**第 2 次测试用了和第 1 次相同的代码**（仅改 HTML 没改测试），且**结果仍是失败**，
  立刻停止"改 HTML+重测"循环，**切到 [5.5 诊断打印](#55-诊断打印先看再断言)**：跑一次诊断版
  拿到实际值（弱断言 + `print` + `evaluate(...)`），再回头精确断言或修 HTML。
