---
name: test-html
description: 指导如何用 test_html 工具对 HTML 文件做 Playwright 单测，含测试模板（基础渲染、响应式布局、LaTeX 公式、交互功能、拖拽、资源加载）、playwrightCode 编写规范和失败处理流程。无论新建 HTML 还是修改已有 HTML 都适用。以下场景必须先加载本 skill 再操作：1) 用 create_file 生成 HTML 后需验证页面功能/布局；2) 用 edit_file 修改已有 HTML 后需回归验证；3) 用户反馈某 HTML 文件功能/布局/渲染异常时——先加载本 skill 编写复现测试确认问题，再动手修改（先测后改，不要直接改）；4) 准备调用 test_html 工具时。
---
