...
每次 `test_html` 调用拿到结果后，在 assistant 文本回复里**必须**输出下面这块结论卡片，给用户和
后续决策者一个明确判定。**不要用"测试通过了已修复"这种含糊句子糊弄过去**：

```
## test_html 结论
- 状态: ✓ 完全通过 / ⚠ 部分通过 / ✗ 失败
- must-cover 覆盖度: N/M 命中
  - [1] <清单条目>: ✓ / ✗ <断言简述或缺失原因>
  - [2] ...
- 根因判定（仅在 ⚠/✗ 时填）: HTML 缺陷 / 测试代码缺陷 / 执行超时（细分见下）
- 下一步动作: edit_file 修 HTML / 改测试代码再测 / 拆调用重测 / 交付
- 还需要的回归项: <列出未覆盖的清单项；全覆盖时填"无">
```

只有当**状态 = ✓ 完全通过 且 N/M 全命中**时，才视为可交付；否则按"下一步动作"继续推进。
