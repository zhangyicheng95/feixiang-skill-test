...
**声明式约束硬规则（否定式 / 全称式，最易被漏）**：用户消息里每条带
**禁止 / 只准 / 全部 / 每个 / 不得 / 固定为 / 至少 N 个** 的约束，都必须进清单，并对应一条
**否定式或全称断言**——"动词+名词"只覆盖正向存在性（有按钮、可点击），**覆盖不了"禁止 X / 每一页都要 Y /
调色板只准这几色"这类约束，它们是漏测重灾区**。口径升级：把"<u>这个</u>背景色对"写成"<u>全页</u>无禁用色"，
把"<u>有</u>水印"写成"<u>每页都有</u>水印"（见模板 6）。

**核心闭环硬规则（可玩 / 有核心功能类）**：若产物是**游戏 / 答题 / 模拟器 / 互动演示**，
must-cover **第一条必须是"核心玩法闭环"端到端**（如 24 点：选数字→选运算符 +−×÷→判定=24→对错反馈→重置），
用一条贯穿断言验证整条链路**真的能走通并产生反馈**，而非只验"按钮存在"。**核心闭环未通过 → 禁止判定为交付完成、禁止 terminate**。

**spec 优先播种**：若产物 HTML 首行有 `<!-- spec: ... -->`（或 task_write 快照里有任务级 spec / `spec_ref`），
**优先据它播种清单**——spec 里每个 `forbid / require / core-loop / count / palette` 都转成一条断言，不要另起炉灶；
spec 偏大被外置为资源（`spec_ref=resourceId`）时，先 `read_file` 该资源再逐条转断言。
...
### 6. 约束符合性 + 核心闭环（声明式约束 / 可玩类必测）

针对"禁止 / 只准 / 全部 / 每个 / 固定为"这类声明式约束，以及游戏/答题类的核心玩法闭环。
**这类断言廉价（一次 `getComputedStyle` 遍历、无需等待），永远放进首次调用，不要为省 20s 预算砍掉。**

#### 6.1 否定式：禁止项全局扫（如"禁阴影 / 禁蓝绿 / 调色板只准白名单"）

```python
viol = page.evaluate("""() => {
    const bad = {shadow:0, color:new Set()};
    const blueGreen = c => {const m=c.match(/\\d+/g); if(!m) return false;
        const [r,g,b]=m.map(Number); return (b>r+30&&b>g+10)||(g>r+30&&g>b+10);};
    document.querySelectorAll('body *').forEach(el => {
        const s = getComputedStyle(el);
        if (s.boxShadow !== 'none' || s.textShadow !== 'none') bad.shadow++;
        [s.color, s.backgroundColor].forEach(c => { if (blueGreen(c)) bad.color.add(c); });
    });
    return {shadow: bad.shadow, color: [...bad.color]};
}""")
assert viol['shadow'] == 0, f"违反禁阴影：{viol['shadow']} 处用了 box/text-shadow"
assert len(viol['color']) == 0, f"违反禁蓝绿：{viol['color']}"
```

#### 6.2 全称式：每一个都要（如"每页都有水印/页码"、"禁英文"）

```python
pages = page.locator(".page").count()
wm = page.locator(".watermark").count()
assert wm == pages, f"水印未覆盖每页：{wm}/{pages}"
# 全局文本约束：禁英文 / 数字须中文大写（按 spec 取舍）
bad_en = page.evaluate("() => (document.body.innerText.match(/[A-Za-z]{2,}/g)||[]).length")
assert bad_en == 0, f"违反禁英文：正文出现 {bad_en} 个英文词"
```

#### 6.3 核心闭环：端到端走一遍（可玩类 must-cover 第一条）

```python
# 例：24 点游戏——选 4 数字 → 选运算符 → 判定 → 对错反馈 → 重置
# 优先直接调暴露在 window 上的核心函数，验证"真的能算出 24"，而非只验按钮存在（见 4.4）
assert page.evaluate("typeof applyOperator === 'function'"), "核心闭环缺失：无运算符处理逻辑（连符号都点不了）"
res = page.evaluate("typeof checkResult === 'function' ? checkResult() : undefined")
assert res is not None, "核心闭环缺失：无判定逻辑"
# UI 链路：点数字→点运算符→点数字→判定，断言出现「对/错」反馈（选择器按实际 DOM）
# 关键是"链路能走通并产生反馈"，而非"按钮可见"
```

> 反例直指 #14134：模型交付了"连运算符都没有"的 24 点游戏外壳，却因为测试只验"按钮存在/能渲染"而判通过。
> 核心闭环必须由 6.3 端到端兜住；产物级 spec（HTML 首行 `<!-- spec: core-loop=... -->`）应直接驱动本条断言。
...
| 用户给了"禁止/只准/全部/每个/固定为"约束 | + 约束符合性（模板 6.1 否定式 / 6.2 全称式） |
| 产物是游戏/答题/模拟器/互动演示 | + 核心闭环端到端（模板 6.3），且为 must-cover **第一条**；未过禁交付 |
| HTML 首行有 `<!-- spec: -->` 或 task_write 有 spec/spec_ref | 据 spec 的 forbid/require/core-loop/count 逐条播种断言 |
