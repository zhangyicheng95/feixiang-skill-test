...
13. ✅ **拖拽元素及其所有祖先**不得带 `pointer-events: none`（除必检 #9 拖拽中临时使用外）。CSS spec 规定 `pointer-events: none` 让元素**及其后代**都不接收事件，**除非后代显式声明成 `auto` 等非 none 值**（默认值 `auto` 不算"显式"）。常见踩雷：装饰性容器（`.bank` / `.overlay` / `.guide-mask`）为了"不阻挡下层点击"加了 `pointer-events: none`，把里面装的拖拽元素一起失活 → 完全拖不动 `[全部]`
...
| 错误分支一行 `el.style.position = 'static'; el.style.transform = 'none'` 归位 | 不还 originalParent（如果 fixed 时挪到 body 了）/ 不清宽高 inline → DOM 节点错位、布局塌缩；无动画体验生硬 | 错误分支必须二选一：① 原样复制模板 C 的 FLIP 9 行；② 显式降级为 `originalParent.insertBefore(el, originalNext) + resetFloatStyles(el)` 两行（无动画也合规，但要写注释声明） |
...
- [ ] grep `pointer-events: none`，对每个命中位置确认其作用范围**不包含**任何 `[data-drag]` 元素的祖先链（必检 #13）
- [ ] 「输出约定」6 条全部满足
...
| #13 pointer-events 祖先链 | ✅ | grep 全文 0 命中 |
```
