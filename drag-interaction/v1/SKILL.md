...
description: 在 HTML/课件中实现拖拽类交互（自由拖动、列表排序、拼图、连线、配对）时加载。提供 STOP 反问触发器、模板路由、必检清单、反模式表与可复制模板，覆盖 PC 鼠标 + 移动端触摸双端。命中关键词："拖拽 / 拖动 / 排序 / 拼图 / 连线 / 配对 / drag / sortable / 移动元素 / 物体拖动 / 把 X 拖到 Y"。不适用于纯展示页面。
---
...
| 用户要"改/调整/优化/修复"现有 HTML | 先 `read_file` 拿当前内容，禁止凭想象重写整页 |
...
8. ✅ drop 命中用 `document.elementFromPoint(x, y).closest('.drop-zone')`，禁用手算 hitbox `[C]`
9. ✅ `elementFromPoint` 前临时给拖拽元素加 `pointer-events: none`，否则永远捞到自己 `[C]`
...
| `if (x > zone.x && x < zone.x + zone.w)` | drop-zone 嵌套 / 父级 `transform` / 页面缩放时全错 | `elementFromPoint(x, y).closest('.drop-zone')` |
| 100 个元素各自 `addEventListener` | 性能差、初始化慢 | 事件委托到容器，`e.target.closest('[data-drag]')` |
