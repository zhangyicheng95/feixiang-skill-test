...
4. ✅ `pointerdown` 后调用 `el.setPointerCapture(e.pointerId)`——但 **C 模板严格顺序：先 `document.body.appendChild(el)` → 再 `setPointerCapture`**（顺序反了会触发 implicit release）。`setPointerCapture` 是「锦上添花」，**必检 #15 的 document 委托才是底线兜底**——不要让 capture 成为事件路由的唯一依赖 `[A, C]`
...
15. ✅ **所有 `pointer*` 委托到 `document` 而非绑在元素上** `[A, C]`：`document.addEventListener('pointerdown' / 'pointermove' / 'pointerup' / 'pointercancel', ...)`。原因：① 拖拽元素被 `appendChild` 到 body 后，绑在原容器上的监听就丢了（模板 A 委托到 stage，模板 C 必须委托到 document）；② capture 偶发失效时，document 上的冒泡监听是唯一兜底；③ 新增/重生成可拖元素时不用重新绑监听
16. ✅ **`onPointerDown` 必须有"残留状态强制清理"守卫** `[A, C]`：开头判断 `if (draggedEl) forceResetActive()`——上一次拖拽如果异常未结束（pointerup/cancel 都没派发，工程上罕见但实测会触发），自动把残留浮起元素归位再开始新一次拖拽。不加守卫的代价：一次异常 = 用户必须刷新页面
17. ✅ **页面 ≥ 5 个待拖元素时，错误分支必须走「降级方案」** `[C]`：禁止用 `anime.js` / FLIP 等 ≥ 300ms 异步弹回。多元素并存 + 异步弹回 = 一次回调抛错全局卡死（详见 `templates/drop-match.md`「错误分支降级方案」节）。降级判断式同时满足以下任一即强制降级：① 待拖元素 ≥ 5；② 用户操作可能在弹回中连续拖另一张（如分类游戏、连线题）；③ 没有完整的 `try/catch` 兜底

> 模板 B 的 1, 4, 5, 6, 7, 8, 9, 10, 15, 16, 17 由库代劳。
...
- [ ] grep 生成的 HTML，关键字段都在（按模板适用）：
  ...
  - `document.addEventListener.*pointer`（A/C 必有 ≥ 3 处委托）—— 必检 #15
- [ ] grep `pointer-events: none`，对每个命中位置确认：① 不包含任何 `[data-drag]` 元素的祖先链；② 也不在任何 drop zone（`.drop-zone` / `.slot` / `.plate` 等）自身上；③ **不在浮起 class（`.floating` / `.dragging` 等）上**（必检 #13 + 反模式「浮起 class 永久关 PE」）
...
- [ ] grep `setPointerCapture`，确认所在函数中**前一行**是 `document.body.appendChild` 或同等浮起到 body 的操作（必检 #4 顺序）
- [ ] grep `pointerdown` 处理函数开头是否有 `if (draggedEl)` / `if (activeEl)` 形式的状态守卫（必检 #16）
- [ ] 待拖元素 ≥ 5 个时，确认错误分支**不含** `anime(` / `requestAnimationFrame.*\.left` 等异步弹回，已走「直接归位」降级（必检 #17）
- [ ] 「输出约定」6 条全部满足
