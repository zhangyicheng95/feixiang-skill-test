...
5. **三级可信度分层**（v10.9.1）：
   - **`confidence: 'verified'`**（~446 字）：OVERRIDE 硬覆盖或教研逐字 reviewed，**100% 可信**
   - **`confidence: 'auto'`**（~2396 字）：小学教材字，走系统性规则或默认映射（**≥98% 可靠**）
   - **`confidence: 'extended'`**（4976 字）：非小学教材字，未审核，**≥98% 但建议标注"待验证"**

   `tier` 字段保留向后兼容：
   - `tier: 'textbook'` = confidence in {verified, auto}
   - `tier: 'extended'` = confidence = extended
...
<script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v43/templates/stroke-card.js"></script>
```

## ✨ 推荐写法：使用 `<stroke-card>` 组件（一行搞定，**根治双数据源问题**）
...
**LLM 只需写 `<stroke-card char="X">`，组件内部统一调用 `getStrokeData` 渲染字头 / 画数 / 逐笔 / tier 徽章 / 提示文本**。所有需要显示笔画名的地方都由组件控制，LLM **不必也不应**在页面任何位置手写笔画名，**彻底根除双数据源冲突**。
...
| show-tier | true/false | true | 是否显示教材/待验证徽章 |
...
| **10** | **在"提示区 / 思考过程 / 描述文本 / 卡片 tooltip"里手写笔画名称** | **双数据源冲突**（如截图 bug："尔"字右侧分解 ≠ 左侧提示）|
