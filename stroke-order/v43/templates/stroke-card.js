...
(function (global) {
  ...
  const STYLE = `
    ...
    /* v10.9.1 三级 confidence 徽章 */
    .tier { font-size: 11px; padding: 2px 8px; border-radius: 10px; cursor: help; }
    .tier.verified { background: #d1fae5; color: #047857; }      /* 教研审核 */
    .tier.auto     { background: #dbeafe; color: #1e40af; }      /* 教材字·自动 */
    .tier.extended { background: #fef3c7; color: #92400e; }      /* 字表外·待验证 */
  ...
  class StrokeCard extends HTMLElement {
    ...
    _render() {
      ...
      // v10.9.1 三级 confidence 徽章 + hover 提示数据来源
      let tierHtml = '';
      if (showTier) {
        const conf = d.confidence || (d.tier === 'textbook' ? 'auto' : 'extended');
        const badgeMap = {
          verified: { text: '✓ 教研审核', title: '该字已通过 OVERRIDE 或教研逐字核对（最高可信度）' },
          auto:     { text: '◆ 教材自动', title: '小学教材字·系统性规则或默认映射（≥98% 可靠，建议教研复核）' },
          extended: { text: '⚠ 字表外', title: '不在小学语文教材字表·仅供参考（≥98% 可靠但未审核）' },
        };
        const b = badgeMap[conf] || badgeMap.extended;
        tierHtml = `<span class="tier ${conf}" title="${b.title}">${b.text}</span>`;
      }
  ...
  global._strokeCard = { version: '1.0.0' };
