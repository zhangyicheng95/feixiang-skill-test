 ...
 *
 * 引入方式：
 *   <script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v43/templates/stroke-loader.js"></script>
 *
 * 使用方式：
...
(function (global) {
  ...
  var DATA_URL = 'https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v43/assets/stroke-data.json';
  ...
  var textbookSet = null;     // Set: 教材字 2842 字
  var verifiedSet = null;     // Set: 教研审核/OVERRIDE 字 ~446 字（最高可信度）
  ...
  function loadData() {
    ...
    xhr.onload = function () {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        // 构建 verified 字索引（v10.9.1+，教研审核/OVERRIDE）
        if (resp.verified_chars && typeof resp.verified_chars === 'string') {
          verifiedSet = new Set();
          for (var m = 0; m < resp.verified_chars.length; m++) {
            verifiedSet.add(resp.verified_chars[m]);
          }
        }
        try {
          if (global.console && global.console.debug) {
            ...
            var verifiedN = verifiedSet ? verifiedSet.size : 0;
            global.console.debug('[stroke-loader] 已加载 ' + totalN + ' 字 ('
              + '★★★verified=' + verifiedN + ' / ★★textbook=' + (textbookN - verifiedN)
              + ' / ★extended=' + (totalN - textbookN) + ')');
          }
        } catch (e) {}
        setStatus('ready');
      } else {
  ...
  function getStrokeData(char) {
    ...
    // v10.9.1: 三级 confidence
    //   - 'verified': OVERRIDE 或教研 reviewed，100% 可靠
    //   - 'auto':     教材字走系统性规则或默认映射，≥98% 可靠
    //   - 'extended': 字典扩展字，≥98% 但未完全审核
    var confidence;
    if (verifiedSet && verifiedSet.has(char)) {
      confidence = 'verified';
    } else if (tier === 'textbook') {
      confidence = 'auto';
    } else {
      confidence = 'extended';
    }
    ...
    return {
      ...
      source: 'db',
      tier: tier,                         // textbook / extended
      confidence: confidence              // verified / auto / extended (v10.9.1)
    };
  }

  /**
   * 判断一个字是否是教研审核过的字（最高可信度）
   */
  function isVerifiedChar(char) {
    if (loadStatus !== 'ready' || !verifiedSet) return null;
    return verifiedSet.has(char);
  }
  ...
  global.isVerifiedChar = isVerifiedChar;
  global._strokeLoader = {
    ...
    getVerifiedTotal: function () { return verifiedSet ? verifiedSet.size : 0; },
