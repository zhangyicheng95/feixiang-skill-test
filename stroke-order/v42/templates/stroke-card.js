...
(function (global) {
  ...
  const STYLE = `
    ...
    .tier.textbook { background: #d1fae5; color: #047857; }
    .tier.extended { background: #f3f4f6; color: #6b7280; }
  ...
  class StrokeCard extends HTMLElement {
    ...
    _render() {
      ...
      const tierHtml = showTier
        ? `<span class="tier ${d.tier}">${d.tier === 'textbook' ? '✓ 教材权威' : '⚠ 待验证'}</span>`
        : '';
