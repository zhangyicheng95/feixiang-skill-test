...
<!-- 第二步：动态加载 cnchar-order（多源级联，解决单一 CDN 失败问题） -->
<script>
(function() {
  var urls = [
  ...
  function tryLoad(i) {
    if (i >= urls.length) {
      window._cncharOrderFailed = true;
      return;
    }
    var s = document.createElement('script');
    s.src = urls[i];
    s.onload = function() {
      window._cncharOrderLoaded = true;
      if (typeof window._onCncharReady === 'function') window._onCncharReady();
    };
...
// 【加载校验】确认 cnchar-order 可用
function verifyCnchar() {
  if (window._cncharOrderFailed) return false;
  if (!window._cncharOrderLoaded) return false;
  try {
...
(function applyOverrides() {
  ...
  var fixes = {
    '吃': 'fcrsjou',     // 6画(cnchar误为7画): 竖横折横撇横横折弯钩
    '亿': 'sfo',         // 3画(cnchar误为4画): 撇竖横折弯钩
    '到': 'jfisekfg',    // 8画(cnchar误为7画): 横竖提撇横撇点竖竖钩
    '话': 'kpsjjfcj',    // 8画(cnchar误为9画): 点横折提撇横横竖横折横
    '山': 'fbf',         // 竖折(cnchar误用竖弯)
...
```javascript
// ✅ 正确用法：等待 cnchar-order 加载完成后再查询
function initApp() {
  var chars = ['竹', '吃', '话', '说', '亿', '到'];
  chars.forEach(function(char) {
    var data = getStrokeData(char);
    if (!data || data.source === 'error') {
      // 数据不可用：显示错误提示，绝不用 AI 记忆填充
      renderError(char, data ? data.error : '该字暂无数据');
      return;
    }
    // data.count = 笔画数
...
// 等待 cnchar-order 加载完成
if (window._cncharOrderLoaded) {
  initApp();
} else {
  window._onCncharReady = initApp;
  // 超时兜底：3秒后如果还没加载完，用 VERIFIED 数据渲染能渲染的
  setTimeout(function() {
    if (!window._cncharOrderLoaded) initApp();
  }, 3000);
}
```
