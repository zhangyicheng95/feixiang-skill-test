 ...
 *
 * 引入方式：
 *   <script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v39/templates/stroke-loader.js"></script>
 *
 * 使用方式：
...
(function (global) {
  ...
  var DATA_URL = 'https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v39/assets/stroke-data.json';
  ...
  var textbookSet = null;     // Set: 教材字 2842 字（高置信度）
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        try {
          if (global.console && global.console.debug) {
            ...
            global.console.debug('[stroke-loader] 已加载 ' + totalN + ' 字 (教材 ' + textbookN + ' + 扩展 ' + (totalN - textbookN) + ')');
          }
        } catch (e) {}
        setStatus('ready');
      } else {
  ...
  function getStrokeData(char) {
    ...
    // v10.7: 同时返回字符串数组 strokes 和对象数组 strokes_detail，兼容 LLM 两种自然写法
    //   - strokes: ['撇','横','横','弯钩']  （简洁，推荐）
    //   - strokes_detail: [{index:1, name:'撇', path:''}, ...]  （带元信息，兼容 s.name 写法）
    var detail = [];
    for (var k = 0; k < strokes.length; k++) {
      detail.push({ index: k + 1, name: strokes[k], path: '' });
    }
    return {
      char: char,
      count: strokes.length,
      strokes: strokes.slice(),
      strokes_detail: detail,
      source: 'db',
      tier: tier
    };
  }
