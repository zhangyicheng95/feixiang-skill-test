...
以下规则供**用户/教师审核 cnchar 输出时参考**，用于判断笔画顺序是否合理。AI 不得将这些规则作为自行推断笔顺的依据（AI 的推断同样不可靠）。
...
function applyFixes() {
  ...
  // ── 笔画数/顺序修正 ──
  // 字母: j横 f竖 s撇 l捺 k点 i提 c横折 n撇折 m撇点
  //       g竖钩 t弯钩 y斜钩|卧钩 u竖弯钩 e横撇|横钩
  //       r横折钩 o横斜钩|横折弯钩 h竖提 p横折提
  //       b竖弯 v横折折|横折弯 a横折折撇 x竖折折|竖折撇
  //       w横折折折钩|横撇弯钩 z竖折折钩 q横折折折
  // ⚠ 重要：以下修正编码必须来自教材/用户验证，严禁 AI 自行推断！
  // AI 的笔顺判断同样不可靠，曾把正确的改错（可），也曾把错的判为对（到）。
  var fixes = {
    // 笔画数修正（cnchar 拆分/合并错误，经用户/教材验证）
    '吃': 'fcjsjo',      // 6画(cnchar误7): 竖横折横撇横横折弯钩
    '亿': 'sfo',          // 3画(cnchar误4): 撇竖横折弯钩
    '说': 'kpksfcjsu',    // 9画(cnchar误7): 点横折提点撇竖横折横撇竖弯钩
    '到': 'jnkjfifg',     // 8画(cnchar误7): 横撇折点横竖提竖竖钩
    '话': 'kpsjjfcj',     // 8画(cnchar误9): 点横折提撇横横竖横折横
    // 竖折修正（cnchar 无竖折编码）
    '山': 'fbf',          // 竖→竖折→竖
    '牙': 'jbgs',         // 横→竖折→竖钩→撇
    '出': 'bffbf',        // 竖折→竖→竖→竖折→竖
...
## 八、发现新 cnchar 错误的修正流程

当测试发现某字笔顺不正确时，按以下步骤添加修正：

### 步骤 1：确认正确笔顺（必须人工完成）

对照部编版教材确定该字的正确笔画数和逐笔名称。**AI 不得代替此步骤** —— AI 曾把可的正确顺序改错，把到的错误顺序判为正确。笔顺确认必须以教材为准。

### 步骤 2：转换为 cnchar 字母编码

```
j=横  f=竖  s=撇  l=捺  k=点  i=提
c=横折  n=撇折  m=撇点  e=横撇|横钩  g=竖钩  t=弯钩
y=斜钩|卧钩  u=竖弯钩  r=横折钩  o=横斜钩|横折弯钩
h=竖提  p=横折提  b=竖弯(也用于竖折)  v=横折折|横折弯
a=横折折撇  x=竖折折|竖折撇  w=横折折折钩|横撇弯钩
z=竖折折钩  q=横折折折
```

### 步骤 3：添加到 fixes 对象

在 `applyFixes()` 的 `fixes` 对象中添加一行：
...
| 字 | cnchar 原问题 | 修正编码 | 正确笔顺（经教材/用户验证） |
|----|-----------|--------|--------|
...
| 山 | 竖弯→应为竖折 | fbf | 竖→竖折→竖(3画) |
| 牙 | 撇折→应为竖折 | jbgs | 横→竖折→竖钩→撇(4画) |
| 出 | 竖弯→应为竖折 | bffbf | 竖折→竖→竖→竖折→竖(5画) |
| 九 | 横斜钩→应为横折弯钩 | so | 撇→横折弯钩(2画) |
| 几 | 同上 | so | 撇→横折弯钩(2画) |
| 凡 | 同上 | sok | 撇→横折弯钩→点(3画) |

> **教训**：可 的 cnchar 输出（横→竖→横折→横→竖钩）本来正确，AI 错误地将第4-5笔调换。
> 到 的 cnchar 输出本来错误，AI 的修正编码也是错误的（第2笔应为撇折而非竖）。
> **结论**：setOrder 编码必须来自教材/用户验证，AI 不具备可靠判断笔顺的能力。

### 笔画名称与笔顺规则的作用定位

| 机制 | 作用对象 | 能力 | 局限 |
|------|---------|------|------|
| **32 标准笔画名称 (STD)** | 代码运行时校验 | ✅ 拦截非法名称（"口""儿"等） | 不能判断顺序对错 |
| **笔顺规则** | 人工审核参考 | ✅ 帮助用户/教师判断顺序合理性 | 不能自动化执行 |
| **setOrder 修正** | 覆盖 cnchar 错误 | ✅ 修正已知错误字 | 编码必须来自人工验证 |
| **VERIFIED 数据** | 最高优先级覆盖 | ✅ 完全可靠（教研校验） | 覆盖面有限（~150字） |
| **fixName 修正** | 名称偏差自动修正 | ✅ 修正5类已知命名偏差 | 字表需持续扩充 |
