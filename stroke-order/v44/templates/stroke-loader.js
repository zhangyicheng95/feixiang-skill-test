 ...
 *
 * 引入方式：
 *   <script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v44/templates/stroke-loader.js"></script>
 *
 * 使用方式：
...
(function (global) {
  ...
  var DATA_URL = 'https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v44/assets/stroke-data.json';
  ...
  function getStrokeData(char) {
    ...
    // v10.9.2: 三级 confidence（严格按 tier 优先）
    //   - 'extended': 字典扩展字 → 永远 extended（即使命中白名单也不升级）
    //   - 'verified': 教材字 + (OVERRIDE / 教研 reviewed / 精确白名单 / 用户确认)
    //   - 'auto':     教材字 + 系统性规则或默认映射
    var confidence;
    if (tier === 'extended') {
      confidence = 'extended';
    } else if (verifiedSet && verifiedSet.has(char)) {
      confidence = 'verified';
    } else {
      confidence = 'auto';
    }
