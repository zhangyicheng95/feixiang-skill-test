...
> 本文件是 courseware-generator 技能的 Phase 5 子流程。进入本流程时，课件大纲已由 Phase 1（outline-guidance.md）生成并经 Phase 2 用户确认，**模板已在 Phase 3 由用户强制选定**，所需素材、壳兼容设计契约与空课件框架已由 Phase 4 准备完毕。

```
步骤 -2 确认设计契约：读取 Phase 4 的 designContractSummary / shellCompatibilityChecklist / pageTypeMap / layoutMap
  ↓
步骤 -1 必要时回读模板局部：仅在契约缺字段时，回读 template.md / shared.css / layouts 的局部片段
  ↓
步骤 0  复杂度评估：根据 Phase 4 已搭好的空课件框架和填充清单，评估各页填充与微调复杂度
  ↓
步骤 1  生成：基于 Phase 4 已搭好的空框架，只替换占位符 + 做少量版面微调
  ↓
步骤 2  验收与交付：逐页核对大纲 + 按模板规范自检 → 交付
```

> **⚠️ 前置条件**：
> 1. 对话历史中已包含经用户确认的完整课件大纲（含逐页设计表格）。大纲中的课标解读、教材分析、学情分析、学习目标等内容均已由 Phase 1 完成，本流程严格按大纲生成 HTML，不得偏离大纲内容。
> 2. **Phase 3 已完成模板选择**，用户的选择（模板 ID 与模板路径）已记录在上下文中。
> 3. Phase 4 已完成**壳兼容设计契约编译**，并输出 `designContractSummary / shellCompatibilityChecklist / pageTypeMap / layoutMap`。
> 4. Phase 4 已完成素材准备与空课件框架搭建，并输出占位符填充清单。生成 HTML 时须使用这些素材 URL，**严禁使用 base64 编码替代**。
...
4. **由于 Phase 3 已强制选定模板**：必须严格遵守"§ 关于模板的硬性规则"（见下方第 0 节）。
5. **优先依赖设计契约，而不是反复读取模板原文件**：除非契约缺字段，否则不要把整份 `template.md` / `shared.css` / `layouts` 重新喂给每一页。

---

## 〇、关于模板的硬性规则（Phase 3 选定模板后强制生效）

**触发条件**：Phase 3 中用户已完成模板单选。本节规则在该情况下**最高优先级**，与本文件其他章节冲突时以本节为准。

### 0.1 必读资源

进入步骤 -2 后，必须优先读取 **Phase 4 已编译好的契约产物**：

| 资源 | 用途 | 是否必读 |
|------|------|----------|
| `designContractSummary` | 模板精简执行摘要，Phase 5 的主依据 | ✅ 必读 |
| `shellCompatibilityChecklist` | 壳兼容检查项 | ✅ 必读 |
| `pageTypeMap` | 页面分型：静态页 / 轻互动页 / 强互动页 | ✅ 必读 |
| `layoutMap` | 第 N 页使用哪个 layout 或 `native-interaction` | ✅ 必读 |
| `contentPreservationPlan` | 第 N 页不可删减的教学内容单元与回退策略 | ✅ 必读 |

只有当上面任一项缺失或不完整时，才回读**选定模板的入口文件**：

| 文件 | 用途 | 是否必读 |
|------|------|----------|
| `templates/design-contract-workflow.md` | 契约编译逻辑与壳兼容边界 | 条件性回读 |
| `templates/<模板路径>/template.md` | 风格说明书 | 条件性回读 |
| `templates/<模板路径>/tokens.json` | 颜色/字体/间距 token | 条件性回读 |
| `templates/<模板路径>/shared.css` | 待装入 `<template class="page-shared">` 的样式 | 条件性回读 |
| `templates/<模板路径>/layouts/*.html` | 静态页版式骨架库 | 条件性回读 |
| `templates/<模板路径>/references/*.html` | 仅供视觉参考的非生产稿 | 条件性回读 |

### 0.1.1 Phase 4 空框架前置条件

进入本文件时，Phase 5 的输入不再是「只有大纲和素材」，而是 Phase 4 已经搭好的**空课件框架**：

- 外层 HTML 已包含正确壳脚本和完整 `page-shared`
- 每一页已经有 `<template class="page-data" data-id="N" data-name="页面名">`
- 静态展示页已经套入对应 template layout 结构
- 交互页已经套入「模板背景壳 + v69 原生互动组件」结构
- layout 或原生组件内的 `class`、`data-*`、`<script>` 已固定
- 需要填的内容全部是带页码前缀的占位符（如 `{{P3_TITLE}}`、`{{P6_QUESTION_TEXT}}`）

若 Phase 5 开始时发现没有空课件框架 resourceId，或 page-data 尚未按大纲页数搭好，**必须回到 Phase 4 补齐框架**，禁止在 Phase 5 临时重建。

### 0.1.2 壳兼容前置条件

Phase 5 在写任何一页前，必须再次确认契约中的壳兼容约束仍然生效：

- 不自定义翻页、全屏、缩略图、预览容器
- 不在 `html` / `body` / 根容器使用 `100vh`、`position:fixed`、整体 `transform/zoom`
- 不让模板装饰层挡住操作层
- 不让模板改写 v69 原生互动协议
- 不把模板“整页浏览器网页”的假设带进飞象壳

### 0.2 CSS 装载规则

1. **必须把 shared.css 的全部内容（含 layouts 版式样式，约 700+ 行）完整写入** `<template class="page-shared">` 的 `<style>` 中——**禁止只复制前半段 token/装饰部分**
2. 如果 `<template class="page-shared">` 已存在 interaction-design 的 CSS 变量，**两者合并**写入（顺序：模板 CSS 在前，interaction-design 变量在后）
3. 不要用 `<link href="templates/...">` 引用，**必须 inline**（因为部署后路径可能变化）
4. Phase 5 只使用 Phase 4 已写入的样式结果，**不要**在填页时重新大段重写 `page-shared`

### 0.3 页面双轨 + 内容保量规则（本次改造的核心）

Phase 4 / Phase 5 必须先把页面分成两类，再决定模板介入深度：
...
处理方式：
- 可以直接使用模板 `layouts/*.html`
- 模板负责版式、卡片、分栏、装饰、标题区
- Phase 5 只替换占位符与少量微调占位符
- 但前提是 `contentPreservationPlan` 已确认该 layout 装得下本页必须保留的题干、讲解步骤、例子、提示语
- 若装不下，优先减装饰、减空白、改卡片密度；仍装不下就回退为“模板视觉变量 + v69 标准静态结构”，禁止删内容
...
- 排序题
- 配对题
- 任意含点击状态、拖拽状态、翻页恢复、答案反馈脚本的页面

处理方式：
- **一律使用 v69 原生互动结构**
- 模板**只负责背景层、标题区、主题 token、少量不挡操作的装饰**
- 模板**不能**重新包裹、拆分、替换原生互动组件的 DOM / `data-*` / `<script>`
- 即使模板目录中存在互动参考页，也**只作视觉参考，不作为生产互动页骨架**

### 0.4 版式选用规则

layout 选择应在 Phase 4 完成。Phase 5 不再重新决定 layout，只检查 Phase 4 的 `layoutMap` 是否合理。优先依据 `designContractSummary + layoutMap`，只有契约缺字段时才回读选定模板的 `template.md`。

**Phase 4 版式使用方法**：

1. A 类静态展示页：复制选定 layout 文件里的 `<template class="page-data">` 块作为该页起点
2. B 类交互页：以 v69 原生互动结构作为该页起点，再套模板提供的背景壳 helper class
3. 写死 `data-id` 与 `data-name`
4. 将通用占位符改为带页码前缀的唯一占位符（如 `{{title}}` → `{{P3_TITLE}}`）
5. 保留完整结构、class、script
6. 必要时添加少量微调占位符（如 `{{P2_IMAGE_STYLE}}`）
7. 若某页 layout 与 `contentPreservationPlan` 冲突，必须在 Phase 4 改用更宽松骨架，不能在 Phase 5 通过删减内容来“塞进”版式

**Phase 5 内容填充方法**：

1. 读取 Phase 4 的占位符填充清单
2. 用 `edit_file` / `multi_edit` 只替换 `{{P*_...}}` 占位符
3. 如图片/互动导致排版问题，只替换预留的微调占位符
4. **不要破坏**：配色变量、字体变量、关键 class 名、`data-id`、`<script>`
5. B 类交互页只允许调整背景壳标题、背景强度、弱装饰，不允许改写互动组件协议
6. 若某页出现模板与壳冲突，优先服从 `shellCompatibilityChecklist`，必要时回退为 `native-interaction + 契约 token`

### 0.5 颜色与字体（硬性约束）

- **严禁硬编码颜色值**（如 `#1976D2`、`rgb(...)`）
- **必须使用 shared.css 里定义的 CSS 变量**（如 `var(--c-primary)`、`var(--c-accent)`）
- **字体必须使用 CSS 变量** `var(--ff-title)` / `var(--ff-body)`
- **禁止硬编码字体名**（如 `font-family: SimSun`）

#### 🔒 0.5.0 严禁使用 LaTeX 公式语法（致命错误）

**飞象老师模板系统不引入 KaTeX/MathJax 等公式渲染器**（外部库会被安全机制过滤）。AI 写数学公式时**禁止使用 LaTeX 语法**，否则字符串原文会直接显示为乱码。

| ❌ 错（LaTeX，会乱码）| ✅ 对（HTML+Unicode）|
|----|----|
| `\(8 \times 4 = 32\)` | `<span class="formula-inline">8 × 4 = 32</span>` |
| `\(10 \div 2 = 5\)` | `<span class="formula-inline">10 ÷ 2 = 5</span>` |
| `$$ax^2 + bx + c = 0$$` | `<div class="formula-block">ax<sup>2</sup> + bx + c = 0</div>` |
| `\frac{a}{b}` | `<span class="frac"><b>a</b><i>b</i></span>` |
| `\sqrt{9}` | `<span class="sqrt">9</span>` |

**详细规范**见模板的 `template.md` § 七「数学公式写法」章节，包含完整 Unicode 数学符号表 + HTML 公式实现示例。

#### 🔒 0.5.1 严禁外部字体引入（致命错误）

**飞象老师安全机制会自动过滤 `<template class="page-shared">` 内的外部 URL**，把它们替换为 `[unsafe-url-removed]`，导致 CSS 语法错误，**整段 page-shared 的样式全部失效**，所有装饰和配色都不工作。

| ❌ 禁止 | 后果 |
|--------|------|
| `@import url('https://fonts.googleapis.com/...')` | 被替换为 `@import url('[unsafe-url-removed]`（未闭合），整段 CSS 失效 |
| `<link href="https://fonts.googleapis.com/...">` 在 page-shared 内 | 同样被过滤 |
| `<link href="https://cdn.jsdelivr.net/...">` 在 page-shared 内 | 同样被过滤 |
| 任何外部 URL 在 page-shared 内（除了 SVG 内部的 xmlns） | 同样被过滤 |

**正确做法**：模板的 shared.css **必须只用系统字体 fallback**，例如：

```css
--ff-title: -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Microsoft YaHei', sans-serif;
--ff-body:  -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Microsoft YaHei', sans-serif;
```

**例外**：壳框架引用（`<script src="https://musk-test.fbcontent.cn/...">`）放在 `<template class="page-shared">` **外面**、`</body>` 之前，不受此限制。

### 0.6 交互页硬性要求

所有交互页都按下面规则处理：

1. **必须保留 v69 原生互动组件的完整 `<script>` / 状态恢复协议 / `data-*` 结构**
2. **禁止**为了贴模板，把原生互动组件拆进模板卡片阵列、多列网格、旋转装饰容器中
3. 交互页外层只允许增加模板背景壳，例如 `rz-native-shell` / `bo-native-shell`
4. 背景壳里的装饰必须 `pointer-events:none` 或天然不挡操作
5. 若模板提供互动参考页，它只作视觉参考，不作为生产课件默认骨架
6. **禁止**在交互页里新增整页级 `wheel` / `keydown` / `Arrow*` / `Space` 监听来实现交互，以免和壳翻页冲突
7. **禁止**给交互页额外包一层模板题卡、模板选项网格、模板反馈盒，导致原生点击/拖拽/状态恢复失效

推荐结构：

```html
<template class="page-data" data-id="6" data-name="课堂练习">
  <section class="bo-native-shell">
    <div class="bo-native-shell__title">{{P6_TITLE}}</div>
    <div class="bo-native-shell__hud">{{P6_KICKER}}</div>
    <div class="bo-native-shell__stage">
      <!-- 这里放 v69 原生互动组件 -->
    </div>
  </section>
</template>
...
如果当前模板没有提供 helper class，也仍然要遵守同样原则：模板只包背景，不包组件。

### 0.7 generate_image 的使用（核心边界）

**生图 prompt 完全自由**：`picture_gen` 的 prompt 由 AI 根据题目实际内容自由编写，**不需要追加任何模板风格后缀**。

- ✅ 鸡兔同笼页：`picture_gen("一只鸡和一只兔子在笼子里")`
- ✅ 光合作用页：`picture_gen("一片绿叶在阳光下进行光合作用")`
- ❌ 禁止：`picture_gen("..., flat vector illustration, hand-drawn style ...")`

**图片的"用图"规则**：

| 用法 | 是否允许 |
|------|---------|
| 把图片放在 layout 的图片占位区（如 `.image-slot` / `.it-image-slot` / `.fm-side-img`）| ✅ 允许 |
| 在文字区**新增**一个小 `<img>` 元素 | ✅ 允许（注意尺寸） |
| 用 generate_image 生成的图作为整页 `background-image` | ❌ **禁止**（这是 Colleague AI 路线，与互动课件不兼容）|

### 0.8 排版避碰撞规则（AI 必须自检）

**模板的版面是"基础骨架"，AI 在填入内容后必须自检排版**：

1. **元素无重叠**：图片不能盖文字、装饰不能盖正文
2. **内容不溢出 960×540 画布**
3. **图片自适应**：
   - 用 `.image-slot` 容器（`object-fit:contain`）
   - 如果生成的图很大，调整 `.image-slot` 的 width/height 缩小
   - 如果图片位置遮挡文字，把图移到空余位置（右下/右上等）
4. **文字自适应**：
   - 内容多时缩小字号或减少描述
   - 内容少时可以放大字号或增加卡片间距
5. **元素数量调整**：根据内容增减元素（如 concept 的 3 卡改 2/4 卡，CSS 同步调 grid-template-columns）
6. **交互页优先保证可操作面积**：模板背景壳不要压缩按钮、拖拽区、连线区、反馈区

### 0.9 自检 checklist（步骤 2 验收时执行）

每页生成完后逐项核对：

- [ ] CSS 变量未被覆盖、未被硬编码颜色替代
- [ ] 字体全部走 `var(--ff-*)`，没有硬编码字体名
- [ ] 该页使用的 layout 与页面用途匹配
- [ ] **元素无重叠**（图片不盖文字、装饰不盖内容）
- [ ] **内容不溢出** 960×540 画布
- [ ] **图片占位**用了 `object-fit:contain` 或限制了尺寸
- [ ] 若是交互页：v69 原生互动组件的 `<script>` / 状态恢复协议仍完整
- [ ] 若是交互页：模板只作用于背景壳，没有改写组件 DOM
- [ ] 骨架末尾壳框架 JS 为 html-guide §3.1 固定 URL（非 head 里的 metis-lectio）

### 0.10 与本文件其他章节的优先级

当本节规则与原文件其他视觉建议冲突时，**以本节为准**（模板是用户选定的全局风格约束）。但通用规则（画布尺寸、不溢出、不改壳框架）**保持一致**。
...
<template class="page-shared">
  ...
  <!-- 🚨 Phase 3 已强制完成模板选择（红线 3）：
       必须把 templates/<模板路径>/shared.css 的全部内容粘贴在 <style> 标签里。
       不做这一步整份课件视觉不会变样！-->
  <style>
    /* TODO: 把已选模板的 shared.css 全部内容写在这里 */
  </style>
</template>
...
<!-- 🚨 Phase 3 已强制完成模板选择（红线 3）：
     每个 <template class="page-data"> 块必须优先按"templates/<模板路径>/layouts/" 下
     当前模板实际存在的版式骨架写：
       - 第 1 页（封面）→ 优先用 layouts/cover.html
       - 情境导入 / 生活实例 → 若模板提供则用 layouts/intro.html
       - 概念讲解 / 要点并列 → 优先用 layouts/concept.html
       - 选择题练习 / 拖拽 / 连线 / 翻牌 / 排序 → 一律使用 v69 原生互动结构
       - 末页 → 优先用 layouts/summary.html
       - 若模板缺少所需静态 layout → 改用 v69 标准结构，但必须继承模板 CSS 变量与标题区
       - 交互页 → 模板只提供背景壳 helper class（如 `rz-native-shell` / `bo-native-shell`）
     禁止回退到"白底黑字微软雅黑"的默认样式！
-->

<template class="page-data" data-id="1" data-name="封面">
  <!-- 第1页内容：复制已选模板的 layouts/cover.html 的 <template> 块作为起点 -->
</template>

<template class="page-data" data-id="2" data-name="页面名称">
  <!-- 第2页内容：根据用途从已选模板的 layouts/ 选对应骨架 -->
</template>
...
- **配色规则**：
  - 配色、字体、装饰**严格由已选模板的 `shared.css` 决定**，必须使用模板的 CSS 变量（`var(--c-primary)` / `var(--ff-title)` 等），**禁止**自行决定颜色和字体，**禁止**套用默认白底样式。
...
- 强互动页的 JS 代码量应 >= 100 行
- 强互动页在生成过程中可按需调用 `generate_image` 和 `generate_voice` 补充生成交互所需的图片和音频素材（如游戏背景图、角色插画等），不受 Phase 3 素材清单限制
...
> 本节的「骨架创建 + page-data 注入」应在 **Phase 4 空课件框架搭建**阶段完成。Phase 5 不再重新创建骨架，也不再重新注入 page-data，只替换 Phase 4 预留的 `{{P*_...}}` 占位符。
...
**模板模式下的额外要求**：
- 此步骤在 Phase 4 执行
- `<template class="page-shared"><style>` 中必须完整写入所选模板的 `shared.css`
- 后续每页注入的是「空 page-data + 页码前缀占位符」，不是最终正文内容
- Phase 5 只替换占位符，不再改骨架、page-shared、壳脚本
...
<template class="page-shared">
  <!-- 🚨 因 Phase 3 已强制选定模板：必须把 templates/<模板路径>/shared.css 的全部内容
       完整粘贴到下面的 <style> 标签里。这是模板生效的关键步骤，跳过这一步整份
       课件的字体、配色、装饰都不会变样。 -->
  <style>
    /* TODO: 粘贴已选模板的 shared.css 全部内容 */
  </style>
</template>
...
6. **强互动页生成时可按需调用素材工具**：如发现需要额外的图片或音频素材，可在生成过程中调用 `generate_image`、`generate_voice` 等工具补充生成，不受 Phase 3 素材清单限制。
...
**模板模式下**：这里注入的是「空模板页」而不是最终内容。每页必须先复制对应 layout，再把通用占位符改为带页码前缀的唯一占位符，例如 `{{title}}` → `{{P3_TITLE}}`。最终内容在 Phase 5 通过替换 `{{P*_...}}` 填入。

**🚨 关于 `<template class="page-data">` 内容的来源（模板已在 Phase 3 强制选定）**：

- **【固定规则】**：**Phase 4 必须**从 `templates/README.md` 中该模板对应的实际目录路径读取 `layouts/<对应版式>.html`，复制 `<template class="page-data">` 块作为该页空框架起点，固定 `data-id` / `data-name`，并把通用占位符改为页码前缀占位符（如 `{{P3_TITLE}}`）。**Phase 5 只替换这些页码前缀占位符**。若当前模板没有对应 layout，则该页改用 v69 标准互动结构，但仍必须继承模板变量、标题区和基础装饰。禁止自行决定 `<style>` 中的配色、字体、布局——这些必须由 **Phase 4 已编译的设计契约** 决定。
...
edit_file:
  ...
  <!-- 复制已选模板的 templates/<模板路径>/layouts/cover.html 的 <template> 块内容到此 -->
</template>
