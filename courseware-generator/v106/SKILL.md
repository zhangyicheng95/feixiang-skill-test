...
2. 读取所选模板的 template.md / tokens.json / shared.css / layouts / references
3. 先生成 shellCompatibilityChecklist
4. 再生成 designContractSummary
5. 再生成 pageTypeMap（静态页 / 轻互动页 / 强互动页）
6. 再生成 contentPreservationPlan（列清每页必须保留的教学信息单元、可压缩项、绝不可删项）
7. 再生成 layoutMap（第 N 页用哪个 layout 或 native-interaction）
```
...
| `contentPreservationPlan` | 第 N 页必须保留的教学内容单元，以及装不下时的回退策略 |
...
- 禁止为了贴合模板而删减题干、步骤、例子、反馈文案等大纲已确认内容
...
1. 先读取 pageTypeMap，再做 layoutMap 落地
   ...
   - layout 选型前必须先读取 `contentPreservationPlan`
   - 若某个静态 layout 无法容纳该页必须保留的教学内容，优先减弱装饰、减少空白、合并视觉卡片；仍装不下则回退到“模板视觉变量 + v69 标准静态结构”
   - **禁止**为了保住模板排版而私自删减该页的讲解步骤、例子、提示语、反馈语
   ...
   - B / C 类交互页不要直接套 template layout；模板目录中的互动参考文件也只作视觉参考
...
4. 输出「占位符填充清单」
   ...
   - 每页还必须附一条 `contentPreservationPlan` 摘要：列明该页不可丢失的题干、知识点、步骤、例子、反馈语
```
...
| 内容保量表 | 第 N 页 → `contentPreservationPlan` |
...
☐ 动作 2：读取 Phase 4 的 designContractSummary + shellCompatibilityChecklist + contentPreservationPlan
...
5. 占位符替换完成后，若出现图片遮挡/内容溢出，只能通过预留微调占位符调整尺寸、行高、卡片数量等；若仍装不下，回到 Phase 4 改成更宽松骨架，禁止直接删内容
...
- 这一页是否已经没有 `{{P*_...}}` 占位符残留？
- 这一页是否没有 LaTeX 公式语法？
- 这一页是否保留了 `contentPreservationPlan` 里要求的全部教学内容单元，而不是为了排版偷偷删减？
