...
- 标题字号、位置、说明文字位置、页脚信息位置和图片槽位大小以 `cover-layout-guide.md` 为准；禁止再套用“封面标题必须居中”的旧规则。
- 封面图必须落在 `coverImageSlot` 指定区域内。若图片比例与槽位不匹配，应使用 `contain` 保护主体或重新按槽位 prompt 生图，禁止用 `object-fit: cover` 裁切主体。
