...
> 本文件只在 Phase 4 首次生成 HTML 前读取，用于让模型自主选择一套贯穿整份互动课件的字体风格。它不新增用户确认字段，不参与模板替换阶段重新选字体。
...
本指南中的字体已经作为测试环境静态资源上传到当前测试链路可访问的 `musk-test.fbcontent.cn` URL。生成课件时：
...
4. 模板替换阶段只允许继承 `--cw-courseware-*` 字体变量；模板自己的字体只能作为 fallback，不能覆盖原版字体决策。
5. 组件框内字体和文字颜色在模板替换阶段保持原课件结果；禁止为了模板或字体风格重写组件内部。
...
6. 禁止模板替换时重新选择字体或用模板字体覆盖 `--cw-courseware-*` 决策。
