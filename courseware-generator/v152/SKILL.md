...
description: 生成多页 PPT 式互动课件（HTML格式）。当用户提出课件生成、教学PPT、多页演示、带缩略图与播放模式的课件等需求时使用。调用本技能的这一轮只能调用 call_skill 一个工具，禁止同轮调用 insert_courseware_design_sop、搜索、专家咨询或大纲生成；读取 SKILL.md 后，除附件读取外，下一轮业务动作只能单独 ask_user 确认 9 项课件信息；ask_user 成功前禁止 SOP、检索、专家咨询和 create_lesson_design；不与 math-design 等单页设计技能混用。涵盖专业大纲生成（课标研读→教材梳理→学情分析→目标拟定→逐页设计）→ 用户确认修改 → HTML分批生成（template标签页面结构、960×540画布、壳框架集成）。不适用于教学游戏、单页动画、精美排版等非互动课件场景。
...
2. **加载技能这一轮只能调用 `call_skill`**：选择本技能时，同一轮 tool_call 只能有 `call_skill(courseware-generator)` 一个工具。禁止在同一轮并发调用 `insert_courseware_design_sop`、搜索、专家咨询、`ask_user` 或 `create_lesson_design`。
