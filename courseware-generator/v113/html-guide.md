...
| 钩子 | 是否必需 | 用途 |
|------|---------|------|
| `data-cw-role="page-root"` | 必需 | 页面最外层可视容器，模板替换时只允许改页面背景、字体、颜色变量与弱装饰 |
| `data-cw-role="title-block"` | 必需 | 标题区容器，模板替换时只允许改已有标题文字的字体、颜色和文字阴影，禁止新增题签文案 |
| `data-cw-role="content-block"` | 必需 | 主内容区容器，模板替换时只允许做文字颜色和页面级色彩适配，禁止重排内容 |
...
6. 互动页也必须完成模板化，但只允许替换 `page-shared`、`page-root`、标题颜色/字体、按钮颜色、反馈颜色和组件颜色变量；**禁止**重排 `interactive-root` 内部 DOM、禁止改组件结构/形状、禁止改壳脚本、禁止改页数和 `data-id`。
