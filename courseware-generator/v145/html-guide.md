...
- 封面页的标题样式必须与普通页面标题装饰隔离。普通页可有标题横杠、左竖线等装饰，但封面标题不得被全局 `[data-cw-role="title-block"] h1` 的 `border-left`、`border-bottom`、`padding-left`、编号装饰等命中；必须使用 `.cover-page ... h1 { border-left:0; border-bottom:0; padding-left:0; }` 或把普通页标题选择器排除 `.cover-page`。
- 若选择 `cover-07-top-title-image-card`，标题区只放主标题；作者、课时、教材、短说明等小字必须放在左下 `.cover-footer` / `.cover-meta`，禁止写成标题下方的 `.cover-copy p`。图片卡片仍按 `cover-07-image-card` 的 `x=.031 y=.198 w=.938 h=.660` 固定落位，标题和图片之间至少保留 `24px` 视觉间距。
...
- 封面版式中的标题区、说明区、页脚区也是真实位置约束，不只是参考建议。生成封面 HTML 时必须按 `cover-layout-guide.md` 的比例坐标落位，禁止只固定图片槽位却自由摆放文字。
- 若选择 `cover-07-top-title-image-card`：`.cover-copy` 只能承载主标题，标题字号建议 `30-36px`；`.cover-footer` / `.cover-meta` 必须位于左下区域；禁止让小字出现在左上标题区，禁止使用 `72px` 级别封面英雄大字挤压图片卡片。
- 封面页必须显式重置普通页标题装饰：`.cover-page [data-cw-role="title-block"] h1` 不得有左竖杠、下划线、额外 `padding-left` 或普通页标题横杠。
...
- [ ] 封面标题未继承普通页标题装饰：没有左竖杠、标题横杠、编号装饰或额外 `padding-left`
- [ ] 若第 1 页使用 `cover-07-top-title-image-card`，小字/作者/教材/课时信息在左下 `.cover-footer` / `.cover-meta`，不是左上 `.cover-copy p`；标题区与图片卡片至少有 `24px` 间距
