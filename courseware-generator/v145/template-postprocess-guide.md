...
6. 若原版 HTML 中存在 `CW_TYPOGRAPHY_DECISION` 或 `--cw-courseware-*` 字体变量，批次 0 注入模板资产后这些标记和变量必须仍存在。模板 `tokens.css` 中的标题/正文/标签字体变量应优先引用 `--cw-courseware-*`，只有原版变量缺失时才使用模板自带字体 fallback。
7. 若最终文件中某页有 `data-cw-template` / `data-cw-variant`，但背景、标题颜色或正文颜色仍被原页 CSS 明显覆盖，优先修正模板共享 CSS 的安全选择器；若是 `page-root` 内联背景造成冲突，只允许删除 `page-root` 的 `background` / `background-image` / `background-color` / `background-size` / `background-position`。但第 1 页若 `data-cover-visual="full-bleed-background"`，必须优先保护独立 `cover-visual` 媒体层，禁止删除、替换或遮盖封面主视觉图。禁止通过改组件 DOM、改组件 class、删原页组件样式、重选字体或重写组件外观来“修好视觉”。
8. 模板资产标记之后禁止再出现未限定模板的页面背景规则，例如 `[data-cw-role="page-root"] { background... }`、`background-color`、`background-image`、`[data-cw-role="page-root"]::before { background... }`。若原课件样式中存在这类规则，必须保证模板 `tokens.css` / `page-shared.css` 位于它们之后；否则会出现原课件米灰、浅灰、水墨底图覆盖像素风/复古风背景的假套版。
9. 第 1 页若存在 `data-cover-visual="full-bleed-background"`，说明封面生图是主视觉背景。此时模板背景不能覆盖该封面图；模板只允许作用于文字颜色、文字阴影、独立半透明遮罩和弱装饰，并继承原版字体变量。第 1 页若为 `side-visual` 或 `inline-card`，模板背景可保留，但 `data-cover-role="cover-visual"` 的媒体层必须作为内容图片保护，禁止滤镜、换图、裁切主体或移动位置。
