...
如果 Phase 2.5 用户选了某个视觉模板（如 `math-explorer`），整份 HTML 生成流程改为「**skeleton + 填槽**」模式，覆盖本文件后续章节的部分内容：
...
```
read templates/<选定模板>/skeleton.html   # 拿到完整骨架字符串
create_file 课件.html
...
- 完整的 `<template class="page-shared"><style>`，含模板 shared.css 全套样式
- `<!-- CW_PAGES -->` 注入标记
- 加载占位 `<div id="cw-loading">`

### 0.2 页内容来源（覆盖本文件 § 9.4 的"自由写"部分）
...
3) 在 reasoning 中，把 layout 字符串里所有 {{XXX}} 替换为该页的真实内容
   - 文字占位符 → 大纲字段
