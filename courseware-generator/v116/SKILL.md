...
4. 该流程包含信息确认机制（9 项字段）、SOP 获取、专家推理等步骤，生成专业的逐页课件大纲。
5. 流程结束时调用 `create_lesson_design` 输出大纲。
6. **⚠️ 重要变更**：outline-guidance.md 原始流程中的 `terminate` 调用在本技能中**不执行**。`create_lesson_design` 完成后，直接进入 Phase 2，不终止任务。
