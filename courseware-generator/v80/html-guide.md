...
| 文件 | 用途 | 是否必读 |
|------|------|----------|
| `templates/<选定模板>/template.md` | 风格说明书（基调、配色、字体、装饰手法、禁止事项） | ✅ 必读 |
| `templates/<选定模板>/tokens.json` | 颜色/字体/间距 token | ✅ 必读 |
| `templates/<选定模板>/shared.css` | 待装入 `<template class="page-shared">` 的样式 | ✅ 必读 |
| `templates/<选定模板>/layouts/*.html` | 4-6 个版式骨架（封面/章节/正文/总结...） | ✅ 必读 |
| `templates/<选定模板>/examples/sample.html` | 完整样张（作为 few-shot 视觉参考） | ✅ 必读 |
...
每生成一页时，**先决定该页用哪个 layout**。模板提供 10 个版式覆盖绝大多数页面类型：

| 页面用途 | 优先使用的 layout |
|---------|------------------|
| 第 1 页课件总封面 | `layouts/cover.html` |
| 章节/单元起始页（PART ONE / TWO …） | `layouts/part.html` |
| 导入页 / 情境引入 / 课文配图 / 古文配图 / 实验配图（图 + 说明文字） | `layouts/image-text.html` |
| 核心问题提出 / 重要算理 / 关键公式 / 警示语（单点强调） | `layouts/single-card.html` |
| 列表枚举 / 数据对照 / 真值表 / 实验记录（**表格**为主） | `layouts/table.html` |
| 解题步骤 / 推理过程 / 操作流程 / 总结要点（**带序号**列表） | `layouts/flow-steps.html` |
| 单选题练习 / 课堂检测 / 巩固练习（**含选项**和反馈） | `layouts/quiz-choice.html` |
| 方法 A vs B / 优点 vs 缺点 / 规律对比 / 变式应用（**两两对比**） | `layouts/compare-2.html` |
| 3-4 个并列要点 / 概念分类 / 平行说明（**多卡片网格**） | `layouts/content.html` |
| 末页谢谢观看 / 课堂回顾 | `layouts/summary.html` |
| 其他特殊页面（思维导图、拖拽匹配、复杂互动游戏…） | **没有现成 layout 时，自由生成但必须遵守 0.4-0.5 节的硬性规则**（CSS 变量、page-title、≥ 1 个装饰元素） |

**版式使用方法**：

1. 按上表查到对应 layout 文件名
2. 完整复制该 layout 文件里的 `<template class="page-data">` 块作为该页起点
3. 把占位变量（如 `{{title}}`、`{{step_1_desc}}`、`{{option_A}}`）替换成大纲里的真实内容
4. 根据内容量增删局部元素（如 content.html 的 4 卡 → 2/3 卡，flow-steps 的 6 步 → 3 步）
5. **不要破坏**：配色变量、字体变量、关键 class 名、装饰元素（`.deco-dots` / `.deco-blob` / `.deco-dashes` / `.deco-chip` 等都不许删）

### 0.4 颜色使用规则

- **严禁硬编码颜色值**（如 `#1976D2`、`rgb(...)`）
- **必须使用 shared.css 里定义的 CSS 变量**（如 `var(--c-red)`、`var(--c-navy)`）
- 一页内主色使用数量不得超过模板 template.md 的限定（通常 ≤ 3 种主色）
- 装饰元素只能用模板规定的色板

### 0.5 字体使用规则

- 字体必须使用 CSS 变量 `var(--ff-title)` / `var(--ff-body)` / `var(--ff-en)`
- **禁止硬编码字体名**（如 `font-family: SimSun`、`font-family: 微软雅黑`）
- 不要引入模板未声明的额外字体

### 0.6 生图规则（与 Phase 3 一致）

**【与早期版本不同】生图 prompt 完全自由**：`picture_gen` 的 prompt 由 AI 根据题目实际内容自由编写，**不需要追加任何模板风格后缀**。

- ✅ 鸡兔同笼页：`picture_gen("一只鸡和一只兔子在笼子里")`
- ✅ 光合作用页：`picture_gen("一片绿叶在阳光下进行光合作用")`
- ❌ 禁止：`picture_gen("..., flat vector illustration, hand-drawn style ...")`（这种"强加风格后缀"会让插图失去题目表达力）

模板风格只通过 HTML/CSS 体现（配色、字体、版面、装饰），与生图无关。

### 🔒 0.6.1 生图的"用图"规则（最严格红线）

**picture_gen 生成的图，只能用作"内容补充插图"，绝不能用作模板装饰**：

| 用法 | 是否允许 |
|------|---------|
| 在文字区内**新增**一个小 `<img>` 元素（如 ≤250px 宽） | ✅ 允许 |
| 在 layout 已有的"图片占位"位置（如 `<img src="{{image_url}}">`）填入 picture_gen URL | ✅ 允许 |
| **替换** layout 的装饰 `background-image: var(--deco-*)` 或 `var(--bg-*)` | ❌ **严格禁止** |
| **覆盖** `.cv-deco` / `.tt-deco` / `.mt-deco-img` 等装饰 div 的样式 | ❌ **严格禁止** |
| **删除** 标了 `🔒 LOCKED` 注释的元素，用 picture_gen 的图代替 | ❌ **严格禁止** |
| 用 picture_gen 生成的图作为整页 `background` | ❌ **严格禁止** |

**典型错误示例**（已在实测中出现，必须避免）：

- ❌ 用 `picture_gen("古卷+卷云+谢谢观看")` 生成一张图覆盖 summary.html 的紫色 blob 装饰背景
- ❌ 用 `picture_gen("课堂场景")` 生成一张图替换 cover.html 右侧 `.cv-deco` 的粉色装饰

**正确做法**：装饰保持不动，只换 `{{占位符}}` 文字；如果题目必须配插图，在文字区内**新增**一个小 `<img>`，与装饰共存。

### 0.7 自检 checklist（步骤 2 验收时执行）
...
- [ ] 主色使用数量符合模板限定（通常 ≤ 3 种）
- [ ] 装饰元素只出现在边角，不抢内容焦点
- [ ] 不存在模板 template.md 中明确禁止的设计手法（如渐变背景、写实阴影、宋体等，具体看模板的"禁止事项"章节）
- [ ] 课件第一眼视觉调性与 `examples/sample.html` 接近

### 0.8 与本文件其他章节的优先级

- 本节的"画布尺寸""画布外不溢出""壳框架不可改"等通用规则**与原文件第二节、第四节一致**，不冲突
- 当本节的版式建议与第四节"4.7 各页类型内容预算"冲突时，**以本节为准**（模板提供的版式已经过设计）
- 当本节的视觉规范与原文件其他视觉建议冲突时，**以本节为准**（模板是用户选定的全局风格约束）
