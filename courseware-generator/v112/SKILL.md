...
6. **根据 AI 教案/大纲页面意图匹配视觉变体**：在 `page-root` 上补 `data-cw-variant="<visualVariant>"`，只选择背景、字体、颜色、装饰、组件皮肤和反馈皮肤，不允许模板约束或重排模型已生成的组件位置。
7. **替换深度按页面类型区分**：
   - 普通页：必须替换背景、标题区、组件样式、组件框和反馈皮肤，但不得为了贴合模板截图移动组件、重排卡片或强行改成图表/时间线。
   - 互动页：也必须替换为模板风格，但只允许替换背景、标题壳、按钮组件、按钮皮肤、外层组件框和反馈皮肤；禁止改动互动主体结构。
8. 模板替换时必须优先复用代码资产里的真实样式代码和组件类名，禁止只根据模板说明书的文案自由发挥整套样式。
9. 替换时必须保留原有的壳框架、页面数量、互动逻辑、状态管理、组件位置和素材引用，只允许修改受控的视觉层与皮肤层。
10. **模板不能改内容图片本身**：图片、插图、题图、结束图等内容素材只允许换外框、底板、摆位和阴影，禁止改图片本身颜色、滤镜、混合模式、透明度遮罩，禁止为了贴模板色系而替换原图。
11. **模板不能改交互绑定关系**：按钮换皮时必须保留原有 `onclick`、事件监听目标、函数名、节点 `id`、`class`、`data-*` 标识和脚本查询关系；禁止把原按钮事件名替换成新的函数名，禁止出现“按钮调用 A，脚本只定义 B”的情况。
12. 替换完成后，**必须再次调用 `publish_resource`**，把替换版面后的新版互动课件重新发给用户。
13. 若用户继续要求更换版面或微调视觉，可重复执行 Phase 6 → Phase 7；但每一轮都必须基于**最近一次 edit_file 返回的最新文件版本**进行修改。
