...
- 版面特点：页面背景和标题文字具有复古印刷气质，但不新增模板题签文案
- 替换策略：全部页面都要换成复古印刷风；普通页替换页面背景、标题文字、整体色彩和组件颜色，交互页替换纸张背景、标题颜色、按钮颜色和反馈颜色；不移动、不重排、不改造模型已生成的组件
...
- 版面特点：页面背景和标题文字具有像素霓虹气质，但不新增任务牌、徽章或等级文字
- 替换策略：全部页面都要换成像素霓虹风；普通页替换页面背景、标题文字、整体色彩和组件颜色，交互页替换霓虹背景、标题颜色、按钮颜色和反馈颜色；不移动、不重排、不改造模型已生成的组件
...
- 模板根据 AI 教案/大纲页面意图选择 `data-cw-variant` 视觉变体，但视觉变体只改变页面背景、字体、颜色和组件颜色，**不控制组件位置或组件样式**。
- 强互动页优先保留原始 `v69` 互动结构，但仍必须继承模板的背景、标题颜色、按钮颜色和反馈颜色。
- 模板禁止新增原课件没有的可见文字；模板示例中的题签、徽章、印章、任务牌、英文栏头不能复制到课件里。
- 模板禁止修改内容图片本身的颜色、滤镜、透明度、原始素材和图片组件结构。
- 模板可以替换按钮颜色，但**禁止**改写原有按钮事件绑定、函数名、节点 `id`、关键 `class`、脚本选择器、按钮结构、尺寸、圆角和阴影。

## 模板代码资产包含什么

- `tokens.css`：颜色、字体栈和页面级主题变量。
- `page-shared.css`：基于 `data-cw-template="<slug>"` 和 `data-cw-role` 的页面视觉与颜色级适配代码。
- `component-snippets.html`：页面根节点模板标记、视觉变体和颜色级接入示例；不是可复制的新组件。
