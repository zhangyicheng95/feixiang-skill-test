...
5. **Phase 7 必须像 Phase 4 分批生成课件一样分步执行**，禁止把“注入模板”当成一次性自由改造。固定顺序如下：
   - **Step 7.1 建立页面清单**：先 `read_file` 最新 HTML，逐页列出所有 `<template data-id ...>` / `data-cw-role="page-root"`，形成页码、`data-id`、`data-name`、页面标题、页面类型、拟用 `data-cw-variant` 的清单。
   - **Step 7.2 注入共享模板层**：只把 `tokens.css` 与 `page-shared.css` 合并进唯一的 `page-shared` 样式层，不改单页内容。成功后记录新的 `resourceId`，并立即以该 `resourceId` 继续。
   - **Step 7.3 按连续页码分批套模板**：按页面清单从第 1 页开始顺序处理，每批最多 3-5 页，必须是连续页码，例如 1-4、5-8、9-12。禁止跳页、倒序、跨批重复处理，禁止先处理后面页面再回头补前面页面。
   - **Step 7.4 每批只做本批页面的模板标识和安全颜色适配**：为本批所有 `page-root` 增加 `data-cw-template="<selectedTemplateId>"` 与 `data-cw-variant="<visualVariant>"`，并按边界做背景、标题、正文、按钮、反馈颜色适配；不得移动组件、替换组件结构或新增模板文案。
...
10. **根据 AI 教案/大纲页面意图匹配视觉变体**：在 `page-root` 上补 `data-cw-variant="<visualVariant>"`，只选择背景、字体、颜色和页面氛围，不允许模板约束或重排模型已生成的组件位置。
...
13. 模板替换时必须优先复用代码资产里的真实颜色变量和页面级样式，禁止只根据模板说明书的文案自由发挥整套样式。
14. 替换时必须保留原有的壳框架、页面数量、互动逻辑、状态管理、组件位置、组件结构、组件形状、组件尺寸、组件文案和素材引用，只允许修改受控的页面视觉层与组件颜色层。
