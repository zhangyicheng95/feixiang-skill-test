...
| `data-cw-role="media-block"` | 按需 | 图片、图表、音频播放器等媒体区容器，模板替换时可调整外框和摆位 |
...
4. 互动页也必须完成模板化，但只允许替换 `page-shared`、`page-root`、`title-block`、`component-shell`、`button-skin`、`feedback-layer`；**禁止**重排 `interactive-root` 内部 DOM、禁止改壳脚本、禁止改页数和 `data-id`。
5. 替换完成后，必须再次调用 `publish_resource`，向用户重新发布替换版面后的新版课件。
6. 若用户仍要求换版面，继续基于**最新一次 edit_file 返回的新 resourceId** 迭代，禁止回退到旧版本。
