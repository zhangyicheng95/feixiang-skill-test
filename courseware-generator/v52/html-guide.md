...
每页在独立 iframe 中运行，通过 `new Audio()` 创建的音频对象不属于 DOM 树，切换页面时浏览器音频子系统仍持有引用，导致音频在翻页后继续播放。`page-shared` 中**必须包含**以下音频清理脚本：
...
**原理**：包装 `Audio` 构造函数，自动追踪所有通过 `new Audio()` 创建的音频实例；页面卸载（`pagehide` / `beforeunload`）时自动暂停并释放全部音频。页面代码无需任何修改，`new Audio(url)` 用法保持不变。

**位置**：放在 `page-shared` 的**最前面**（在其他外部脚本之前），确保在任何页面代码创建 Audio 之前完成包装。
...
<template class="page-shared">
  <!-- 音频自动清理：追踪 new Audio() 实例，翻页时自动停止 -->
  <script>
  (function(){var o=window.Audio,a=[];window.Audio=function(s){var e=new o(s);a.push(e);return e};window.Audio.prototype=o.prototype;function c(){a.forEach(function(e){try{e.pause();e.src=''}catch(x){}});a.length=0}window.addEventListener('pagehide',c);window.addEventListener('beforeunload',c)})();
  </script>
