...
7. **封面图先定槽位**：若第 1 页需要封面图，调用 `generate_image` 前必须先读取 `cover-layout-guide.md`，确定 `data-cover-visual`、`data-cover-layout` 和 `coverImageSlot`，再把目标尺寸、比例、横竖方向和构图安全区写入该条图片描述。禁止生成任意比例图片后在 HTML 中用 `object-fit: cover` 裁切主体。
...
2. 读取 **cover-layout-guide.md**，根据第 1 页内容和封面图角色选择设计老师给出的 7 种封面版式之一，确定 `data-cover-visual`、`data-cover-layout` 和 `coverImageSlot`。封面标题位置、字号、图片槽位大小、图片位置必须在原版课件生成时完成，禁止留到模板替换时重排。
3. 读取 **html-guide.md**。
...
6. **使用 Phase 3 中准备的素材 URL**：生成 HTML 时，将图片和音频的 URL 嵌入对应页面，不要自行创建 base64 内容。封面图必须按 `cover-layout-guide.md` 的槽位固定落位，不得用 `flex: 1`、自动填充或随内容撑开的方式代替图片槽位；若图片比例明显不匹配槽位，应使用 `contain` 保护主体或重新按槽位 prompt 生图，禁止裁切主体。**强互动页在生成过程中可按需调用 `generate_image`、`generate_voice` 补充素材；若补调 `generate_image`，同样必须先按 `image-generation-guide.md` 做逐条命中式增强。**
