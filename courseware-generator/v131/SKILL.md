...
| **[image-generation-guide.md](image-generation-guide.md)** | 课件**生图提示词增强**指南：只在模型已经准备调用 `generate_image` 时读取，逐条判断图片描述是否命中语文/历史/文学性等风格类型，命中才增强 prompt，不命中不套风格 | Phase 3 准备图片素材前；Phase 4 强互动页临时补图前 |
...
3. **生图 prompt 只做命中式增强**：准备调用 `generate_image` 前必须读取 `image-generation-guide.md`。该指南不决定是否生图；只能在模型已根据大纲判断确需生图并形成初始图片描述后，逐条判断每个图片描述是否命中语文/历史/文学性等风格类型。命中才追加对应风格 prompt，不命中保持原始图片需求。豆包工作流下优先使用中文提示词，禁止为了套风格把整套课件所有图片统一改成同一种画风。
...
4. **使用 Phase 3 中准备的素材 URL**：生成 HTML 时，将图片和音频的 URL 嵌入对应页面，不要自行创建 base64 内容。**强互动页在生成过程中可按需调用 `generate_image`、`generate_voice` 补充素材**；若补调 `generate_image`，也必须先读取 `image-generation-guide.md` 并逐条做命中式 prompt 增强，不命中的图片需求不得追加该风格库。
...
- **生图 prompt 命中式增强**：`image-generation-guide.md` 只在 `generate_image` 调用前生效；按单张图片需求判断，命中语文/历史/文学性等类型才追加风格 prompt，不命中保持原始素材需求。
