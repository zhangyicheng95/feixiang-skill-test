...
7. 模板替换时必须优先复用代码资产里的真实样式代码和组件类名，禁止只根据模板说明书的文案自由发挥整套样式。
8. 替换时必须保留原有的壳框架、页面数量、互动逻辑、状态管理和素材引用，只允许修改受控的视觉层与版式层。
9. **模板不能改内容图片本身**：图片、插图、题图、结束图等内容素材只允许换外框、底板、摆位和阴影，禁止改图片本身颜色、滤镜、混合模式、透明度遮罩，禁止为了贴模板色系而替换原图。
10. **模板不能改交互绑定关系**：按钮换皮时必须保留原有 `onclick`、事件监听目标、函数名、节点 `id`、`class`、`data-*` 标识和脚本查询关系；禁止把原按钮事件名替换成新的函数名，禁止出现“按钮调用 A，脚本只定义 B”的情况。
11. 替换完成后，**必须再次调用 `publish_resource`**，把替换版面后的新版互动课件重新发给用户。
12. 若用户继续要求更换版面或微调视觉，可重复执行 Phase 6 → Phase 7；但每一轮都必须基于**最近一次 edit_file 返回的最新文件版本**进行修改。
