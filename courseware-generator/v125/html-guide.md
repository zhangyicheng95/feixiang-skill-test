...
步骤 2  验收与首次交付：逐页核对大纲 → 调用 publish_resource 首次发布
  ↓
步骤 3  发布后版面替换（按需）：首次发布后立即 ask_user 确认版面是否满意 → 选择模板 → 对全部页面做安全替换 → 调用 publish_resource 二次发布
```
...
6. **首次发布完成后必须立刻调用 `ask_user`**，询问用户是否满意当前版面；禁止发布后直接结束流程。
7. **若用户选择模板，模板替换必须覆盖全部页面**；普通页做页面视觉与颜色级替换，互动页做轻量颜色级替换，但同样不能漏页，且不能改组件样式。
...
| 钩子 | 是否必需 | 用途 |
|------|---------|------|
| `data-cw-role="page-root"` | 必需 | 页面最外层可视容器，模板替换时只允许改页面背景、字体族、颜色变量与弱装饰 |
| `data-cw-role="title-block"` | 必需 | 标题区容器，模板替换时只允许改已有标题文字的字体族、颜色和文字阴影，禁止新增题签文案，禁止改标题结构/字号/横杠/位置 |
| `data-cw-role="content-block"` | 必需 | 主内容区容器，模板替换时只允许做正文字体族、文字颜色和页面级色彩适配，禁止重排内容 |
| `data-cw-role="media-block"` | 按需 | 图片、图表、音频播放器等媒体区容器，模板替换时只允许做颜色变量适配，**禁止**改变内容图片本身颜色、素材或媒体组件结构 |
| `data-cw-role="component-shell"` | 有卡片/面板/题框时必需 | 普通页和互动页的组件框、面板框、题框、信息卡外壳；模板替换时只作为颜色适配锚点，禁止替换成新模板组件框 |
| `data-cw-role="button-skin"` | 有可点击按钮时必需 | 普通页和互动页的按钮、选项按钮、提交按钮、切换按钮的颜色适配锚点，禁止替换按钮样式或结构 |
| `data-cw-role="interactive-root"` | 交互页必需 | 互动 DOM 的根容器，模板替换时**禁止重排其内部结构**，只允许改页面背景和颜色 token |
| `data-cw-role="feedback-layer"` | 有反馈层时必需 | 正误反馈、解析弹层、提示层容器，模板替换时只允许做颜色适配，禁止改事件逻辑和显示结构 |

#### 使用规则

- 每页只能有 **1 个** `page-root`。
- 每页应有且仅有 **1 个**主 `title-block` 和 **1 个**主 `content-block`；若存在多列或多卡片布局，使用内部 class 继续细分，不要额外复制同名钩子。
- 普通页中的信息卡、题框、讲解框、图片区外框，尽量都包在 `component-shell` 内，便于模板统一做颜色适配；不得因此改变组件形状、尺寸、圆角、阴影、边框粗细、布局和文案。
- 所有真实可点击的按钮、选项块、切换按钮、提交按钮，尽量都挂上 `button-skin`，便于模板统一做按钮颜色适配；不得因此替换按钮结构、尺寸、圆角、阴影或事件绑定。
- 含拖拽、选择、排序、Canvas、闯关等交互的页面，必须提供 `interactive-root`，并让交互脚本以内聚方式绑定其内部节点。
- `media-block` 若包含 `<img>`、`<svg>`、题图、插图等内容素材，模板替换只允许做颜色变量适配；**禁止**对素材节点本身施加 `filter`、`hue-rotate`、`mix-blend-mode`、`backdrop-filter`、着色遮罩或替换原图，也禁止把媒体组件改成新相框/贴纸结构。
- `button-skin` 只负责按钮颜色适配；模板替换时必须保留原按钮节点的事件绑定关系，包括 `onclick`、监听函数名、`id`、脚本查询选择器和 `data-*` 标识，并保留原按钮结构、尺寸、圆角、阴影与文案。
...
- HTML 中 `<template class="page-data">` 数量为 0，或数量少于大纲规划页数。
...
- **发布前硬校验**：调用 `publish_resource` 前必须重新读取最新 HTML 并确认 `<template class="page-data">` 数量等于大纲页数、`data-id` 连续、壳脚本存在、`<!-- CW_PAGES -->` 保留，且不存在自写 `#cw-root` / 父级 iframe/srcdoc / 普通网页容器。任一项不通过时禁止发布，必须回到步骤 1 修复。
- **首次发布必须调用 `publish_resource`**：使用**最近一次 `edit_file` 返回的最新 resourceId** 单独调用 `publish_resource`，把基础互动课件发布给用户。
- **首次发布完成后必须立刻调用 `ask_user`**：询问用户是否满意当前版面，不得在发布后直接结束。
- 首次发布说明「直接浏览器打开即可，无需其他文件或服务器」。

### 10.4 发布后版面替换（按需）

若用户在首次发布后明确表示「内容可以，但想换一套版面」：

1. 不重新走大纲与整课件生成流程，直接基于**当前最新 resourceId 对应的 HTML 文件**进行后置替换。
2. 模板替换必须覆盖**全部页面**，禁止只替换封面、目录或部分普通页；二次发布前必须逐页校验 `data-cw-template` 和 `data-cw-variant`。
3. 普通页必须替换 `page-shared` 和每个 `page-root` 的页面级视觉；`title-block` 只改已有标题文字的字体/颜色；`content-block`、`media-block`、`component-shell`、`button-skin`、`feedback-layer` 只做颜色级适配。
4. 模板禁止新增原课件没有的可见文字；模板示例中的题签、徽章、印章、任务牌、英文栏头、等级文字不能复制进课件。
5. 模板可以适配 `media-block` 的颜色变量，但**禁止**修改内容图片本身的色相、亮度、对比度、透明度、混合模式和原始素材 URL；禁止为了贴模板色系而对图片直接加滤镜，也禁止把图片组件改成新相框/贴纸结构。
6. 互动页也必须完成模板化，但只允许替换 `page-shared`、`page-root`、标题/正文/辅助文字字体族、标题/正文/辅助文字颜色、按钮颜色、反馈颜色和组件颜色变量；**禁止**重排 `interactive-root` 内部 DOM、禁止改组件结构/形状、禁止改壳脚本、禁止改页数和 `data-id`。
7. 互动页在适配按钮、选项按钮、切换按钮颜色时，必须保留原有事件绑定关系；禁止修改 `onclick`、事件监听目标、函数名、节点 `id`、关键 `class` 与脚本查询选择器，禁止出现“按钮调用的函数名与脚本定义不一致”的情况。
8. 替换完成后，必须再次调用 `publish_resource`，向用户重新发布替换版面后的新版课件。
9. 若用户仍要求换版面，继续基于**最新一次 edit_file 返回的新 resourceId** 迭代，禁止回退到旧版本。
...
- [ ] 若用户选择了模板，全部页面都已有 `data-cw-template` 和 `data-cw-variant`，不存在“只换了部分页面”的情况
- [ ] 若页面存在卡片、题框、面板、按钮，则已挂上 `component-shell` / `button-skin`
- [ ] 模板没有新增原课件不存在的题签、徽章、印章、英文栏头、任务牌或等级文字
- [ ] 模板没有改变组件 DOM、尺寸、圆角、阴影、边框粗细、布局、位置和可见文案，只做颜色级适配
...
- [ ] 首次交付使用最新 resourceId 单独调用了 `publish_resource`
- [ ] 首次交付后已立刻调用 `ask_user` 询问版面是否满意
- [ ] 若执行过版面替换，二次交付同样使用最新 resourceId 单独调用了 `publish_resource`
