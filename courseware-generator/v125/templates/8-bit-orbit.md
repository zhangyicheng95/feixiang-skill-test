...
- 根据 AI 教案/大纲页面意图，在每一个 `page-root` 上添加一个 `data-cw-template="8-bit-orbit"` 和 `data-cw-variant` 视觉变体；视觉变体只改变页面背景、标题/正文/辅助文字字体族、标题/正文/辅助文字颜色、文字阴影、颜色变量和页面氛围，**不得移动或重排模型已生成的组件位置，也不得改变组件样式**。
...
- `8-bit-orbit` 的关键价值正是原版 10 页的多背景像素范式：深蓝、粉色、青色、薰衣草都必须保留为可用配方。禁止把整份课件统一成一种深蓝皮肤。
- 变体选择必须按页面功能确定性映射，不是随机混色；但只要页面功能匹配，就应使用对应原版 slide 的固定背景色。
- `slide-02-mission-brief`、`slide-06-roadmap` 的粉色大底必须保留，用于导入/目标/任务说明/步骤流程页；不得因为担心割裂而自动改成深蓝。
- `slide-03-core-systems`、`slide-08-quote` 的青色大底必须保留，用于概念模块/知识卡/中心问题/关键结论页。
- `slide-05-resource-allocation`、`slide-09-access-tiers` 的薰衣草大底必须保留，用于数据对比/分层任务/选项组页；不得因此改造原组件为进度条、套餐卡或新按钮样式。
- `slide-01-hero`、`slide-04-analytics-core`、`slide-07-platform-vitals`、`slide-10-closing-cta` 的深蓝像素星空/网格必须保留，用于封面、数据快照、关键数值、结束页。

## 10 页来源范式沉淀表（只套视觉，不套结构）

> 原开源模板 `8-bit-orbit` 是 10 页。本 skill 必须把 10 页都作为可选视觉配方沉淀；套用时按 AI 教案页面意图选择其中一页的背景、颜色、字体和文字层级配色。**禁止**按原模板页码改课件页码，禁止把原模板组件位置、卡片、按钮、图表结构搬进课件。
...
|---|---|---|---|---|
| `native-interaction-shell` | 基于 8-bit 深色页的安全抽象 | 强互动页、练习页、闯关页 | 深蓝网格背景、青色标题、黄色强调色、原变量级按钮/反馈颜色 | 不改 `interactive-root` 内部 DOM / 事件 / 状态协议 / 组件样式 |
...
> 完整范式原则：10 页来源范式是必须使用的视觉候选集。`8-bit-orbit` 本身就是多背景色模板，粉色、青色、薰衣草、深蓝都应按页面功能出现在成品中。禁止随机选色，也禁止为了统一而把所有页面压成同一种深蓝背景。

## 颜色差异说明

`8-bit-orbit` 的原版 10 页包含几组固定背景色，模板注入时必须保留这些来源页差异：
...
二次发布前必须在覆盖表中列出本模板的 `backgroundFamily` 分布：`dark-grid`、`pink-grid`、`cyan-grid`、`lavender-grid`、`native-dark`。若非互动页全部落在同一背景组，说明 10 页来源范式没有真正应用，禁止发布。
...
- `title-block`：只允许按所选 `slide-xx-*` 配方改标题字体族、颜色和文字阴影；不得改标题结构、字号、行高、横杠、边距和下划线，不新增任务牌文案
...
- 这套模板明确包含：10 页来源范式的页面背景、页面文字颜色、标题/正文/辅助文字字体族、标题/正文/辅助文字颜色、文字阴影、颜色系统、`--primary-color` / `--accent-color` 等变量级颜色适配。
