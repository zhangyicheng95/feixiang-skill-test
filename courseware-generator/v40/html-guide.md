...
根据大纲逐页设计表格中每页的**交互设计**列，将所有页面分为两类：

| 类型 | 判定标准 | 典型页面 |
|------|---------|---------|
| **强互动页** | 交互设计列包含详细的"交互剧本"（描述了操作方式、过程反馈、结果反馈、完成态、技术标注等） | 拖拽匹配、连线题、游戏化闯关、实验模拟、Canvas 动画等 |
| **普通页** | 交互设计列为简短标签（如"点击展开"、"逐步显示"、"音频播放"）或无交互 | 封面、导入、讲解、简单选择题、总结、结束等 |

> **简单判定**：交互设计列超过 50 字且包含反馈动画/音效描述 → 强互动页；否则 → 普通页。
...
1. **强互动页必须独占一个批次**——每个强互动页单独生成，不与其他页面混合。这保证 AI 将全部输出预算和注意力集中在该页的丰富交互实现上。
...
| 强互动页与普通页混在同一批次生成 | 强互动页必须独占批次，保证完整的输出预算和注意力。普通页可打包生成。 |
