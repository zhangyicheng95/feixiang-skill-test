...
- 替换策略：全部页面都要换成复古印刷风；普通页替换页面背景、页面文字颜色和主题颜色变量，交互页替换纸张背景和外层颜色变量；不移动、不重排、不改造模型已生成的组件，不删除标题横杠
...
- 替换策略：全部页面都要换成像素霓虹风；普通页替换页面背景、页面文字颜色和主题颜色变量，交互页替换霓虹背景和外层颜色变量；不移动、不重排、不改造模型已生成的组件，不删除标题横杠
...
- 模板根据 AI 教案/大纲页面意图选择 `data-cw-variant` 视觉变体；每套模板必须优先使用 `slide-01-*` 到 `slide-10-*` 的原开源 10 页来源范式。视觉变体只改变页面背景、页面文字颜色、标题/正文/辅助文字字体族、标题/正文/辅助文字颜色、文字阴影和主题颜色变量，**不控制组件位置或组件样式**。
- 强互动页优先保留原始 `v69` 互动结构，但仍必须继承模板的背景和外层颜色变量。
- 模板禁止新增原课件没有的可见文字；模板示例中的题签、徽章、印章、任务牌、英文栏头不能复制到课件里。
- 模板禁止修改内容图片本身的颜色、滤镜、透明度、原始素材和图片组件结构。
- 模板可以通过原课件已有颜色变量让按钮自然换色，但**禁止**改写原有按钮事件绑定、函数名、节点 `id`、关键 `class`、脚本选择器、按钮结构、尺寸、圆角、边框粗细和阴影。
