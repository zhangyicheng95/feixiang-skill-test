...
<!-- ========== 壳框架（固定引用，不要修改） ========== -->
<script src="https://musk-online.fbcontent.cn/pub-musk-ai-studio/user/upload/admin/HTgdKPoRbUn35GvEUXJVz2.js"></script>
...
每页内容在壳框架提供的 **960×540 像素固定画布**（16:9）内渲染。壳框架会自动为每页注入 base CSS：`html, body { margin:0; padding:0; width:960px; height:540px; overflow:hidden; }`。

### 4.1 画布与布局

- 根容器样式须包含：`html, body { margin:0; padding:0; width:100%; height:100%; }`。
- **禁止使用 `100vh`**：在 iframe 中 `100vh` 取的是外层视口高度，会导致错乱。一律使用 `100%`。
- 建议为 body 或根容器设置 `padding` 留出安全边距（如 `padding: 30px 40px`）。

### 4.2 禁止溢出

- **所有元素必须完整落在 960×540 画布内**，禁止出现滚动条或内容被截断。
- **禁止**在根容器或任何父层使用 `overflow: hidden/auto/scroll` 来"隐藏"溢出内容。防溢出的唯一合法手段是**调整布局**或**精简内容**。
- **全程不溢出**：动画前/后、互动前/后（如展开答案、显示解析）的所有状态均不得溢出。设计时须同时校验初始态与所有终态/展开态。
...
- 退出演示时同步主区状态
- 自动为每页注入 base CSS（960×540 + overflow:hidden）
...
| 使用 `overflow: hidden/auto/scroll` 来"隐藏"溢出内容 | 必须通过布局调整解决溢出。 |
