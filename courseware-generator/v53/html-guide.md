...
每页在独立 iframe 中运行，通过 `new Audio()` 创建的音频对象不属于 DOM 树。两种场景会导致音频不受控地继续播放：

1. **翻页**：切换页面时浏览器音频子系统仍持有旧页面 Audio 对象的引用，音频不会自动停止。
2. **放映模式切换**：进入/退出放映模式时壳框架可能创建新 iframe 或改变布局，旧 iframe 中的音频同样不会停止。

解决方案由两部分组成，**缺一不可**：

##### 第一部分：`page-shared` 音频追踪脚本

放在 `page-shared` 的**最前面**（在其他外部脚本之前），确保在任何页面代码创建 Audio 之前完成包装：
...
(function() {
  ...
  function TrackedAudio(src) {
    var a = new _RealAudio(src);
    _allAudio.push(a);
    return a;
  }
  TrackedAudio.prototype = _RealAudio.prototype;
  window.Audio = TrackedAudio;
  function stopAll() {
    _allAudio.forEach(function(a) {
  ...
  window.addEventListener('message', function(e) {
    if (e.data && e.data.type === '__cwStopAudio') stopAll();
  });
})();
</script>
```

**原理**：包装 `Audio` 构造函数，自动追踪所有 `new Audio()` 实例。三个清理触发点：页面卸载（`pagehide` / `beforeunload`，覆盖翻页场景）、收到 `__cwStopAudio` 消息（覆盖放映模式场景）。页面代码无需任何修改，`new Audio(url)` 用法保持不变。

##### 第二部分：父文档全屏检测脚本

放在骨架的 `<!-- CW_PAGES -->` 标记之后、壳框架 `<script>` 之前。进入/退出放映模式（全屏变化）时向所有页面 iframe 发送 `__cwStopAudio` 消息：
...
(function() {
  function notifyStop() {
    document.querySelectorAll('iframe').forEach(function(f) {
      try { f.contentWindow.postMessage({ type: '__cwStopAudio' }, '*'); } catch(e) {}
    });
  }
  document.addEventListener('fullscreenchange', notifyStop);
  document.addEventListener('webkitfullscreenchange', notifyStop);
})();
</script>
```
...
<template class="page-shared">
  <!-- 音频自动清理：追踪 new Audio() 实例，翻页/放映切换时自动停止 -->
  <script>
  (function(){var o=window.Audio,a=[];window.Audio=function(s){var e=new o(s);a.push(e);return e};window.Audio.prototype=o.prototype;function c(){a.forEach(function(e){try{e.pause();e.src=''}catch(x){}});a.length=0}window.addEventListener('pagehide',c);window.addEventListener('beforeunload',c);window.addEventListener('message',function(e){if(e.data&&e.data.type==='__cwStopAudio')c()})})();
  </script>
...
<!-- 放映模式音频清理：进入/退出全屏时通知页面 iframe 停止音频 -->
<script>
(function(){function n(){document.querySelectorAll('iframe').forEach(function(f){try{f.contentWindow.postMessage({type:'__cwStopAudio'},'*')}catch(e){}})};document.addEventListener('fullscreenchange',n);document.addEventListener('webkitfullscreenchange',n)})();
</script>
