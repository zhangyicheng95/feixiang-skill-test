...
| 类型 | 判定标准 | 典型页面 |
|------|---------|---------|
| **强互动页** | 大纲交互设计列以 `[强互动]` 标签开头 | 拖拽匹配、连线题、游戏化闯关、实验模拟、Canvas 动画等 |
| **普通页** | 交互设计列无 `[强互动]` 标签 | 封面、导入、讲解、简单选择题、总结、结束等 |

> **判定方式**：检查大纲交互设计列是否包含 `[强互动]` 前缀标签，有则为强互动页，无则为普通页。大纲专家已在设计阶段完成标注。

### 8.3 强互动页的交互剧本扩充（生成前必做）

大纲中强互动页的交互设计列通常只有简短描述（如「[强互动] 拖拽匹配练习」）。**在生成该页 HTML 之前**，须先按以下五个维度将简短描述扩充为详细的交互剧本：
