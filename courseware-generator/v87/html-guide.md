...
```
步骤 -1 装载模板：如 Phase 2.5 选定了模板，读取模板的 template.md / shared.css / layouts / examples
  ↓
步骤 0  复杂度评估：根据已确认大纲，评估各页生成复杂度权重
  ↓
步骤 1  生成：创建 HTML 骨架（注入模板 CSS）→ 按复杂度分批生成页面内容（按模板版式）→ 逐批注入骨架
  ↓
...
> 3. Phase 3 已完成素材准备，图片和音频素材已获取 URL。生成 HTML 时须使用这些素材 URL，**严禁使用 base64 编码替代**。
...
| `templates/<选定模板>/layouts/*.html` | 版式骨架库 | ✅ 必读 |
| `templates/<选定模板>/svg-library.md` | SVG 图标库索引 + 何时使用什么图标 | ✅ 必读 |
| `templates/<选定模板>/svg/*.svg` | 矢量图标库（AI 主动挑选 inline） | ✅ 必读 |
| `templates/<选定模板>/examples/sample.html` | 完整样张 | ✅ 必读 |

### 0.2 CSS 装载规则

1. **必须把 shared.css 的全部内容**写入课件 HTML 文件的 `<template class="page-shared">` 的 `<style>` 标签中
...
每生成一页时，**先决定该页用哪个 layout**。读取选定模板的 `template.md` 查看可用 layouts 列表和决策树（每个模板的版式不同）。

**版式使用方法**：

1. 复制选定 layout 文件里的 `<template class="page-data">` 块作为该页起点
2. 把占位变量（如 `{{title}}`、`{{step_1_text}}`）替换成大纲里的真实内容
3. **AI 可微调元素位置、大小、增删元素以适应内容长短**
4. **不要破坏**：配色变量、字体变量、关键 class 名
...
### 0.5 SVG 图标库使用（AI 主动选择）

模板的 `svg/` 目录提供**学科专属矢量图标**。AI 在生成每页时**主动**挑选合适的 SVG：

1. 读取 `templates/<选定模板>/svg-library.md` 了解每个图标的用途、适合的页面、建议尺寸
2. **每页**根据内容判断是否需要装饰图标（不是每页都用，按需即可）
3. **inline 嵌入**到 layout 已预留的 `.svg-deco` 容器（或自由位置）
4. **颜色自动绑定**：SVG 内的 `data-c="primary|accent|..."` 会被 `shared.css` 规则自动绑定模板色，AI 不需要改 fill 属性

**禁止**：
- ❌ 用 `<img src="svg/...">` 引用（部署后路径失效）
- ❌ 修改 SVG 内部结构和颜色定义
...
| 用 generate_image 生成的图替换模板的 SVG 装饰 | ❌ **禁止** |

### 0.7 排版避碰撞规则（AI 必须自检）

**模板的版面是"基础骨架"，AI 在填入内容后必须自检排版**：

1. **元素无重叠**：图片不能盖文字、装饰不能盖内容、SVG 不能盖正文
2. **内容不溢出 960×540 画布**
3. **图片自适应**：
...
- [ ] SVG 装饰使用 inline 方式（不是 `<img src>`）
- [ ] 课件第一眼视觉调性与 `examples/sample.html` 接近
...
<!-- 🚨 如果 Phase 2.5 选了模板（红线 2）：
     ...
     对应版式的骨架写：
       ...
       - 章节起始页 → 用 layouts/part.html
       - 大部分知识点/正文页 → 用 layouts/content.html
       - 末页 → 用 layouts/summary.html
       - 练习/强互动页 → 自由写但必须用模板 CSS 变量（var(--c-red) 等）
...
- **配色规则（根据 Phase 2.5 用户选择而不同）**：
  - **【若选了模板】**：配色、字体、装饰**严格由模板的 `shared.css` 决定**，必须使用模板的 CSS 变量（`var(--c-red)` / `var(--ff-title)` 等），**禁止**自行决定颜色和字体，**禁止**套用下面那条"白底深文字"的默认建议。
...
- **【若选了模板】**：**必须**从 `templates/<选定模板>/layouts/<对应版式>.html` 复制 `<template class="page-data">` 块作为该页起点，替换占位变量为大纲真实内容。**禁止**自行决定 `<style>` 中的配色、字体、布局——这些必须由模板决定。
