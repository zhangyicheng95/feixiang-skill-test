...
### 红线 2：选定模板后，必须按模板渲染整份课件（不只是生图）

当 Phase 2.5 用户选定了某个模板（如 `splash-pop`），**整份课件 HTML 必须严格按模板规范生成**：

| 必做动作 | 在哪做 | 不做的后果 |
|---------|-------|-----------|
| 把 `templates/<选定模板>/shared.css` **完整复制**到课件 HTML 的 `<template class="page-shared"><style>` 中 | Phase 4 创建骨架时 | 整份课件用不上模板字体/配色变量，视觉上像没装模板 |
| 每一页都用 `templates/<选定模板>/layouts/` 里的版式骨架（封面用 cover.html，章节页用 part.html，正文用 content.html，末页用 summary.html） | Phase 4 生成每页时 | 内页风格回到默认课件样式，与封面割裂 |
| 颜色全部走 CSS 变量（`var(--c-red)` 等）禁止硬编码 | 每页生成时 | 配色飘忽不统一 |
| `generate_image` 调用追加 `image-prompt.txt` 后缀 | Phase 3 / Phase 4 | 插图画风与课件 HTML 不统一 |
...
Phase 3: 素材准备
  ...
  图片（generate_image，调用时追加模板的 image-prompt.txt 后缀）→ 音频（generate_voice）→ 知识搜索（search_knowledge）等
     ↓
Phase 4: HTML 课件生成
...
6. **【若 Phase 2.5 选定了模板】图片生成必须追加模板的 image-prompt 后缀**：
   - 读取 `templates/<选定模板>/image-prompt.txt`
   - 每次调用 `generate_image` 时，在用户级 prompt 末尾以 `, ` 连接 image-prompt.txt 的全部内容
   - 这能让所有插图画风与模板保持一致
   - 如果选了"不使用模板"则跳过此规则
...
使用 Phase 3 中准备的素材 URL：生成 HTML 时，将图片和音频的 URL 嵌入对应页面，不要自行创建 base64 内容。**强互动页在生成过程中可按需调用 `generate_image`、`generate_voice` 补充素材**（若选了模板，仍需按模板规则追加 image-prompt 后缀）。
...
4. Phase 3 负责「用什么」——根据大纲准备图片、音频、题目等素材（若选了模板，图片生成需追加模板的 image-prompt 后缀）。
