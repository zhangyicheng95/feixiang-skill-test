...
每生成一页时，**先决定该页用哪个 layout**：

| 页面用途 | 优先使用的 layout |
|---------|------------------|
| 第 1 页课件总封面 | `layouts/cover.html` |
| 章节/单元起始页（PART ONE / TWO …） | `layouts/part.html` |
| 知识点讲解、对比、列表、要点等 | `layouts/content.html` |
| 末页谢谢观看、总结回顾 | `layouts/summary.html` |
| 其他特殊页面（练习页、互动游戏页） | 没有现成 layout 时，**自由生成，但必须使用模板的 CSS 变量**（如 `var(--c-red)`），保持视觉一致 |

**版式使用方法**：复制 layout 文件里的 HTML/CSS 骨架，把占位变量（如 `{{title}}`）替换成真实内容；可根据内容量增减元素，但**不要破坏配色、字体、关键 class 名**。
