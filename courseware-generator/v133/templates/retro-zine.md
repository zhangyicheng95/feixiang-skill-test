...
- 根据 AI 教案/大纲页面意图，在每一个 `page-root` 上添加一个 `data-cw-template="retro-zine"` 和 `data-cw-variant` 视觉变体；视觉变体只改变页面背景、组件框外标题/正文/辅助文字字体族、组件框外标题/正文/辅助文字颜色、文字阴影和页面氛围，**不得移动或重排模型已生成的组件位置，也不得改变组件样式或组件框内文字颜色**。
...
- 第 1 页若已有 `data-cover-visual="full-bleed-background"`，说明封面生图是整页主视觉；此时本模板不得用纸张背景覆盖封面图，只能应用标题字体、标题颜色、文字阴影、独立遮罩和弱装饰。若封面是 `side-visual` 或 `inline-card`，模板背景可保留，但封面图必须作为 `media-block` 保护。
- `retro-zine` 的 10 页来源范式都必须保留并按页面功能使用；它本身是两色印刷/纸张感模板，很多来源页会共享暖纸底，但不能因此只用一个变体覆盖所有普通页。
...
> 完整范式原则：10 页来源范式是必须使用的视觉候选集。`retro-zine` 的很多来源页同属暖纸色系，但仍要按页面功能使用不同配方；禁止只用一个暖纸底变体覆盖所有普通页。

## 颜色差异说明

`retro-zine` 原版是两色印刷/纸张感模板，不是 10 页 10 种背景色；但 10 页来源范式仍要完整沉淀并按功能使用：
...
二次发布前必须在覆盖表中列出本模板的 `backgroundFamily` 分布：`paper`、`green-field`、`dark-paper`、`black-closing`、`native-paper`。若非互动页全部落在同一个 `data-cw-variant`，说明 10 页来源范式没有真正应用，禁止发布。
...
- 这套模板明确包含：10 页来源范式的页面背景、组件框外页面文字颜色、标题/正文/辅助文字字体族、组件框外标题/正文/辅助文字颜色、颜色系统和文字阴影。
