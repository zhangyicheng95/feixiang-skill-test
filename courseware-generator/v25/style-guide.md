...
```
创建骨架 → 生成 HTML 文件，含 <!-- CW_PAGES --> 注入标记
  ↓
批次1 → 生成第1–3页 <template>，multi_edit 注入到标记位置
  ↓
批次2 → 生成第4–7页 <template>，multi_edit 注入到标记位置
  ↓
批次3 → 生成第8–10页 <template>，multi_edit 注入到标记位置
  ↓
验收 → 逐页核对大纲 → 交付
```
...
每批生成后，用 `multi_edit` 将标记替换为「本批 `<template>` 标签 + 标记」：
...
用 `multi_edit` 匹配需修改页面中**短小、唯一的片段**进行精准替换。如果修改范围较大，可对该页的 `<template>` 整体替换（以 `<template class="page-data" data-id="N"` 到 `</template>` 为匹配范围）。
