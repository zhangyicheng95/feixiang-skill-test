...
| `templates/<选定模板>/image-prompt.txt` | 生图风格后缀（Phase 3 已用，此处用于强互动页内的临时生图） | ✅ 必读 |
...
强互动页在 HTML 生成过程中按需调用 `generate_image` 时，prompt 末尾必须以 `, ` 连接模板的 `image-prompt.txt` 全部内容。例如：

```
generate_image(prompt = "<原本的 prompt>, <image-prompt.txt 内容>")
```
