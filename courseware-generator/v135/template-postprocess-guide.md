...
8. 第 1 页若存在 `data-cover-visual="full-bleed-background"`，说明封面生图是主视觉背景。此时模板背景不能覆盖该封面图；模板只允许作用于字体、文字颜色、文字阴影、独立半透明遮罩和弱装饰。第 1 页若为 `side-visual` 或 `inline-card`，模板背景可保留，但 `data-cover-role="cover-visual"` 的媒体层必须作为内容图片保护，禁止滤镜、换图、裁切主体或移动位置。
