...
> 本文件是 courseware-generator 技能的 Phase 4 子流程。进入本流程时，课件大纲已由 Phase 1（outline-guidance.md）生成并经 Phase 2 用户确认，**模板已在 Phase 2.5 由用户选定**（或选了"不使用模板"），所需素材已由 Phase 3 准备完毕。

```
步骤 -1 装载模板：如 Phase 2.5 选定了模板，读取模板的 template.md / shared.css / layouts
  ↓
步骤 0  复杂度评估：根据 Phase 3 已搭好的空课件框架和填充清单，评估各页填充与微调复杂度
  ↓
步骤 1  生成：若 Phase 3 已搭好模板空框架，则只替换占位符 + 做少量版面微调；若未使用模板，按原流程创建骨架并分批注入
  ↓
...
> 2. **Phase 2.5 已完成模板选择**，用户的选择（模板 ID 或"不使用模板"）已记录在上下文中。
> 3. Phase 3 已完成素材准备；若选了模板，Phase 3 还应已完成空课件框架搭建，并输出占位符填充清单。生成 HTML 时须使用这些素材 URL，**严禁使用 base64 编码替代**。
...
4. **若 Phase 2.5 选定了模板**：必须严格遵守"§ 关于模板的硬性规则"（见下方第 0 节）。若选了"不使用模板"则跳过该节，按本文件其余章节自由生成。

---

## 〇、关于模板的硬性规则（仅当 Phase 2.5 选定了模板时生效）

**触发条件**：Phase 2.5 中用户**未选**"不使用模板，AI 自由发挥"。本节规则在该情况下**最高优先级**，与本文件其他章节冲突时以本节为准。
...
### 0.1.1 Phase 3 空框架前置条件

选了模板时，Phase 4 的输入不再是「只有大纲和素材」，而是 Phase 3 已经搭好的**空课件框架**：

- 外层 HTML 已包含正确壳脚本和完整 `page-shared`
- 每一页已经有 `<template class="page-data" data-id="N" data-name="页面名">`
- 每一页已经套入对应 layout 结构
- layout 内的 `class`、`data-*`、`<script>` 已固定
- 需要填的内容全部是带页码前缀的占位符（如 `{{P3_TITLE}}`、`{{P6_QUESTION_TEXT}}`）

若 Phase 4 开始时发现没有空课件框架 resourceId，或 page-data 尚未按大纲页数搭好，**必须回到 Phase 3 补齐框架**，禁止在 Phase 4 临时重建。
...
layout 选择应在 Phase 3 完成。Phase 4 不再重新决定 layout，只检查 Phase 3 的页面 layout 映射表是否合理。读取选定模板的 `template.md` 查看可用 layouts 列表和决策树（每个模板的版式不同）。

**Phase 3 版式使用方法**：

1. 复制选定 layout 文件里的 `<template class="page-data">` 块作为该页起点
2. 写死 `data-id` 与 `data-name`
3. 将通用占位符改为带页码前缀的唯一占位符（如 `{{title}}` → `{{P3_TITLE}}`）
4. 保留完整结构、class、script
5. 必要时添加少量微调占位符（如 `{{P2_IMAGE_STYLE}}`）

**Phase 4 内容填充方法**：

1. 读取 Phase 3 的占位符填充清单
...
### 0.5 练习题页（exercise layout）硬性要求

复制 `layouts/exercise.html` 时（仅当当前选定模板实际提供该文件）：

1. **必须保留完整 `<script>`**（`qzAnswer` + `postMessage saveState` + `restoreState` 监听）
2. **禁止**只复制 `.qz-page` 的 HTML 而丢掉 script
3. 版式 CSS（`.qz-feedback { display:none }` 等）已在 `shared.css`，无需在 page-data 再写 `<style>`

如果当前模板**不提供** `exercise.html`：

1. **不要伪造一个模板练习页 layout**
2. 改用 v69 标准互动结构实现练习逻辑
3. 仍然要继承当前模板的 CSS 变量、标题区和基础装饰

### 0.6 generate_image 的使用（核心边界）
...
<template class="page-shared">
  ...
  <!-- 🚨 如果 Phase 2.5 选了模板（红线 2）：
       必须把 templates/<模板路径>/shared.css 的全部内容粘贴在 <style> 标签里。
       不做这一步整份课件视觉不会变样！-->
  <style>
    /* TODO: 如选了模板，把模板 shared.css 全部内容写在这里 */
  </style>
</template>
...
<!-- 🚨 如果 Phase 2.5 选了模板（红线 2）：
     ...
     当前模板实际存在的版式骨架写：
       ...
       - 选择题练习 → 仅当模板提供 layouts/exercise.html 时使用（保留完整 <script>）
       - 末页 → 优先用 layouts/summary.html
       - 若模板缺少所需 layout → 改用 v69 标准互动结构，但必须继承模板 CSS 变量与标题区
       - 强互动页 → 自由写但必须用模板 CSS 变量（var(--c-primary) 等）
     禁止回退到"白底黑字微软雅黑"的默认样式！
-->

<template class="page-data" data-id="1" data-name="封面">
  <!-- 第1页内容：如选了模板，复制 layouts/cover.html 的 <template> 块作为起点 -->
</template>

<template class="page-data" data-id="2" data-name="页面名称">
  <!-- 第2页内容：如选了模板，根据用途从 layouts/ 选对应骨架 -->
</template>
...
- **配色规则（根据 Phase 2.5 用户选择而不同）**：
  - **【若选了模板】**：配色、字体、装饰**严格由模板的 `shared.css` 决定**，必须使用模板的 CSS 变量（`var(--c-primary)` / `var(--ff-title)` 等），**禁止**自行决定颜色和字体，**禁止**套用下面那条"白底深文字"的默认建议。
  - **【若选了"不使用模板"】**：可使用教育课件常见方案：白底深文字、左侧色块装饰、圆角卡片等；每页可有轻微差异但整体风格统一。
...
> **若 Phase 2.5 选了模板**：本节的「骨架创建 + page-data 注入」应在 **Phase 3 空课件框架搭建**阶段完成。Phase 4 不再重新创建骨架，也不再重新注入 page-data，只替换 Phase 3 预留的 `{{P*_...}}` 占位符。
...
- Phase 4 只替换占位符，不再改骨架、page-shared、壳脚本
...
<template class="page-shared">
  <!-- 🚨 如 Phase 2.5 选了模板：必须把 templates/<模板路径>/shared.css 的全部内容
       完整粘贴到下面的 <style> 标签里。这是模板生效的关键步骤，跳过这一步整份
       课件的字体、配色、装饰都不会变样。 -->
  <style>
    /* TODO: 选了模板 → 粘贴 shared.css 全部内容；没选模板 → 留空或按需引入资源 */
  </style>
</template>
...
**🚨 关于 `<template class="page-data">` 内容的来源（根据 Phase 2.5 用户选择不同）**：

- **【若选了模板】**：**Phase 3 必须**从 `templates/README.md` 中该模板对应的实际目录路径读取 `layouts/<对应版式>.html`，复制 `<template class="page-data">` 块作为该页空框架起点，固定 `data-id` / `data-name`，并把通用占位符改为页码前缀占位符（如 `{{P3_TITLE}}`）。**Phase 4 只替换这些页码前缀占位符**。若当前模板没有对应 layout，则该页改用 v69 标准互动结构，但仍必须继承模板变量、标题区和基础装饰。禁止自行决定 `<style>` 中的配色、字体、布局——这些必须由模板决定。
- **【若选了"不使用模板"】**：自由编写每页的 `<style>` 和内容，遵守第四节的画布与内容预算规则。
...
edit_file:
  ...
  <!-- 选了模板：复制 templates/<模板路径>/layouts/cover.html 的 <template> 块内容到此 -->
  <!-- 没选模板：自由编写，遵守第四节规则 -->
</template>
