...
3. 普通页必须替换 `page-shared` 和每个 `page-root` 的页面级视觉；`title-block` 只改已有标题文字的字体/颜色；`content-block`、`media-block`、`component-shell`、`button-skin`、`feedback-layer` 只做保护识别，不改组件内联样式、组件结构或组件框内文字颜色。
...
9. 替换完成后，必须再次调用 `publish_resource`，向用户重新发布替换版面后的新版课件。
10. 若用户仍要求换版面，继续基于**最新一次 edit_file 返回的新 resourceId** 迭代，禁止回退到旧版本。
