...
> 本文件用于 Phase 1 首次信息确认中的模板选择字段，并在 Phase 7 后置模板注入前再次核对。用户一开始选择模板，但模板只在原版互动课件首次发布后注入。
...
1. 首次信息确认 `ask_user` 中必须包含一个**单选**模板字段。
2. 只能展示本文件中列出的真实模板选项，不得临时编造新模板名。
3. 模板选项的用户可见文案建议使用中文展示名：
   - `复古印刷风`
   - `像素霓虹风`
4. 首次确认后必须记录 `selectedTemplateId`：`retro-zine` 或 `8-bit-orbit`。
5. 若 Phase 7 前发现 `selectedTemplateId` 缺失或无效，才允许再次使用 `ask_user` 只补选模板，不能由模型自行代选。
6. 用户选定模板后，必须继续读取对应的模板说明书，再执行后置版面替换。
...
- 模板共享 CSS 必须作为最终视觉层生效；若模板资产后面还残留原课件的全局 `page-root` 背景、背景图或 `::before` 遮罩规则，会把模板背景盖成原课件米灰/浅灰/水墨底，必须先修正顺序或追加模板背景锁，禁止发布。
- 若原课件把背景图或底色写在 `page-root` 内联 `style` 中，只允许删除这个 `page-root` 上的背景相关属性；禁止因此改动页面内部卡片、按钮、图片框、媒体框、标题或反馈层样式。
- 模板根据 AI 教案/大纲页面意图选择 `data-cw-variant` 视觉变体；每套模板必须优先使用 `slide-01-*` 到 `slide-10-*` 的原开源 10 页来源范式。视觉变体只改变页面背景、组件框外文字颜色、组件框外标题/正文/辅助文字颜色和文字阴影；字体继承原版 `--cw-courseware-*` 决策，**不控制组件位置、组件样式或组件框内文字颜色**。
...
- `tokens.css`：颜色、页面级主题变量，以及引用原版 `--cw-courseware-*` 的模板字体 fallback。
- `page-shared.css`：基于 `data-cw-template="<slug>"` 和 `data-cw-variant="<slide-xx-*>"` 的页面背景、标题/正文/辅助文字颜色、文字阴影和变量级颜色适配代码；不得直接重写组件角色样式、位置、尺寸、边框、圆角、阴影、字体或媒体结构。
...
- 不包含“深度固化”“视觉重构”“霓虹强化”“控制台风格”等失败修复式二次设计。
