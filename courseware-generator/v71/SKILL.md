...
1. 读取 **html-guide.md**。
2. **【若 Phase 2.5 选定了模板】装载模板资源**：
   - 读取 `templates/<选定模板>/template.md`（风格说明书，必读）
   - 读取 `templates/<选定模板>/shared.css`（待装入 page-shared）
   - 读取 `templates/<选定模板>/layouts/*.html`（按页面用途选用版式骨架）
   - 读取 `templates/<选定模板>/examples/sample.html`（作为 few-shot 视觉参考）
   - 详细的模板装载与遵守规则见 html-guide.md 中"关于模板的硬性规则"章节
3. 读取 **interaction-design** 技能，获取动画与交互的设计参考（缓动、时长、交互模式、代码模板）。读取后，将其中的共享 CSS 变量（缓动函数等）写入 `<template class="page-shared">` 的 `<style>` 中，使所有页面可直接使用。如果 Phase 2.5 选了模板，模板的 shared.css 与 interaction-design 的变量**合并写入** page-shared，二者均生效。
4. 从 html-guide.md 的**步骤 0（页面分类）**开始执行：
   - 根据大纲中各页的交互设计列，将页面分为**强互动页**（详细交互剧本）和**普通页**（简短交互标签）
   - 强互动页独占批次单独生成（追求游戏级交互体验），普通页打包生成
   - 创建 HTML 骨架 → 分批生成 → 逐批注入
   - **若选定了模板**：每页生成时必须从 `templates/<选定模板>/layouts/` 选最匹配的版式骨架，把内容填入，颜色严格使用模板 CSS 变量（如 `var(--c-red)`），禁止硬编码颜色
5. **使用 Phase 3 中准备的素材 URL**：生成 HTML 时，将图片和音频的 URL 嵌入对应页面，不要自行创建 base64 内容。**强互动页在生成过程中可按需调用 `generate_image`、`generate_voice` 补充素材**（若选了模板，仍需按模板规则追加 image-prompt 后缀）。
