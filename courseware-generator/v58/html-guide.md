...
`new Audio()` 创建的音频不在 DOM 中，翻页或切换放映模式时不会自动停止。需要两段脚本配合，**均已包含在骨架模板中（见 9.2 节），生成时直接复制即可**：

1. **`page-shared` 音频追踪脚本**（放在 `page-shared` 最前面）：包装 `Audio` 构造函数以追踪所有实例，在页面卸载（`pagehide`/`beforeunload`）或收到 `__cwStopAudio` 消息时自动停止全部音频。页面代码无需改动，`new Audio(url)` 用法不变。
2. **父文档全屏检测脚本**（放在 `<!-- CW_PAGES -->` 之后、壳框架 `<script>` 之前）：监听 `fullscreenchange` 事件，进入/退出放映模式时向所有页面 iframe 发送 `__cwStopAudio` 消息。
