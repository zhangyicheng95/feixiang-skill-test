...
<!-- ========== 壳框架（固定引用，不要修改） ========== -->
<script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/user/upload/admin/pvpDPHDo2EACkrQQD4saLB.js"></script>
...
> 本节仅适用于「需要采集学习数据」的课件（例如练习课、答题闯关课、需要看作答报告的课）。**纯讲解类、纯展示类课件无需接入数据收集 SDK，跳过本节即可。** 是否需要数据收集，由 `data-collect` 技能决定，本技能只负责：当需要时，按本节规范在 HTML 中正确接入 SDK。

数据收集 SDK 用于把学生的答题、互动数据上报到平台，让教师查看作答报告。SDK 的全局类名是 `MuskCollect`，提供 `save(collection, data)` 和 `query(collection)` 两个 API。

### 7.1 架构铁律（违反必出 bug）

数据收集 SDK 在多页课件中的接入有严格的架构要求，AI 必须遵守以下三条铁律：

#### 铁律 1：SDK **已由壳框架自动加载**，禁止手动引入

壳框架（`courseware-shell.js`）会**自动**在主页加载 `musk-collect` SDK 到 `window.MuskCollect`。

- ✅ 在主页 `<body>` 末尾的 `<script>` 中直接 `new MuskCollect()` 即可使用。
- ❌ **禁止**在任何位置手动写 `<script src=".../musk-collect.js"></script>` 或类似 SDK 引入语句，包括 `<template class="page-shared">` 内部和主页 `<head>` 中。

> 原因：壳已经在主页装好 SDK；AI 重复引入容易写错位置（例如错放在 `<template class="page-shared">` 内 → 该 template 是惰性的，主页根本不会执行其中的 `<script>`，最终主页里 `new MuskCollect()` 会报 `ReferenceError: MuskCollect is not defined`）。

#### 铁律 2：SDK 实例与消息监听必须在主页（不能写进任何 `<template>` 内）

`new MuskCollect()`、`window.addEventListener('message', ...)` 这类**业务代码**必须写在课件 HTML 的主页 `<body>` 末尾的普通 `<script>` 块中，**不能**写进 `<template class="page-shared">` 或 `<template class="page-data">` 任何一种 `<template>` 内。

- ✅ 主页 body 末尾、所有 `<template>` 之后、`<script src="...courseware-shell...">` **之前**，放一段 `<script>` 写业务逻辑。
- ❌ **禁止**把 `new MuskCollect()` 或 `addEventListener('message', ...)` 写进 `<template class="page-shared">`。
- ❌ **禁止**把上述代码写进 `<template class="page-data">`。

> 原因：`<template class="page-shared">` 的内容会被壳框架注入到**每个子页 srcdoc 的 `<head>`**，相当于每个子页都跑一份独立 SDK 实例和独立消息监听。子页用 `window.parent.postMessage(...)` 把事件发给主页时，主页没有监听器，消息就会被丢弃，导致 `sdk.save` 永远不被触发。这是真实出现过的 bug，**务必避免**。

#### 铁律 3：子页只发不收（业务消息）

子页（`<template class="page-data">` 内的脚本）作为"事件源"，**只通过 `window.parent.postMessage(...)` 把数据事件发给主页**，由主页统一处理；子页**不监听**业务消息。

- ✅ 子页发送：`window.parent.postMessage({ type: 'quizResult', pageId: 5, correct: true, selected: 'B' }, '*')`
- ❌ 子页**禁止**注册 `window.addEventListener('message', ...)` 监听业务消息（如 `quizResult`、`pageEnter`）。
- 唯一例外：第六节的壳协议消息 `restoreState`，子页可以监听（这是壳→子页的状态恢复推送，跟数据收集无关）。
...
```html
<!-- 数据收集业务脚本：必须放在所有 <template> 之后、壳框架 <script> 之前 -->
<script>
(function () {
  // SDK 实例（壳已自动加载 SDK，这里直接实例化即可）
  var sdk = new MuskCollect();

  // 全局聚合状态
  var quizResults = {};         // { [pageId]: { correct, selected } }
  var visitedPages = new Set(); // 已访问页 id 集合
  var studentInfo = null;       // { name, clazz }
  var studentInfoPromise = null;

  // 监听子页消息
  window.addEventListener('message', function (e) {
    if (!e.data || typeof e.data !== 'object') return;

    if (e.data.type === 'pageEnter' && e.data.pageId != null) {
      visitedPages.add(String(e.data.pageId));
      trySync();
    }

    if (e.data.type === 'quizResult' && e.data.pageId != null) {
      quizResults[e.data.pageId] = {
        correct: e.data.correct === true,
        selected: e.data.selected
      };
      ensureStudentInfo().then(trySync);
    }
  });

  // 学生身份采集（首次答题时弹层询问，后续自动复用）
  function ensureStudentInfo() {
    if (studentInfo) return Promise.resolve();
    if (studentInfoPromise) return studentInfoPromise;
    studentInfoPromise = new Promise(function (resolve) {
      // AI 自定义弹层 UI（输入姓名 + 班级），用户提交后调用：
      // studentInfo = { name: '张三', clazz: '六(1)班' };
      // resolve();
      // 注意：禁止使用 alert/confirm/prompt（见禁止事项清单）
      showLoginOverlay(function (info) {
        studentInfo = info;
        resolve();
      });
    });
  ...
  function trySync() {
    ...
    var totalQuiz = Object.keys(quizResults).length;
    var correctCount = 0;
    Object.keys(quizResults).forEach(function (k) {
      if (quizResults[k].correct) correctCount++;
    });
    var accuracy = totalQuiz > 0 ? Math.round(correctCount / totalQuiz * 100) : 0;

    sdk.save('submissions', {
      name: studentInfo.name,
      clazz: studentInfo.clazz,
      quizResults: quizResults,
      visitedPages: Array.from(visitedPages),
      accuracy: accuracy,
      submittedAt: Date.now()
    }).catch(function (e) { console.warn('[collect] save failed', e); });
  }

  // showLoginOverlay 由 AI 实现：HTML 弹层（不可用 alert/prompt），输入姓名+班级，提交后调用回调
...
子页（`<template class="page-data">` 内）的标准发送代码：

```html
<template class="page-data" data-id="5" data-name="选择题">
  <!-- ...题目 UI... -->
  <script>
  (function () {
    // 答题判定后，把结果发给主页
    function onAnswer(option, isCorrect) {
      window.parent.postMessage({
        type: 'quizResult',
        pageId: 5,                  // 当前页 data-id
        correct: isCorrect,
        selected: option
      }, '*');

      // ↓ 同时按第六节规范上报 saveState（页内状态恢复用），与 quizResult 是两件事
      window.parent.postMessage({
        type: 'saveState',
        state: { answered: true, selected: option }
      }, '*');
    }
    // ... 绑定选项点击事件
  })();
...
### 7.3 instanceId 自动解析（无需 AI 处理）

`new MuskCollect()` 不传参数时，SDK 会自动按以下顺序查找 `instanceId`：

1. 当前页 URL 的 `?instanceId=`
2. `window.__MUSK_INSTANCE_ID__` 全局变量
3. `<meta name="musk-instance-id">` 标签
4. `window.top.location.search` 中的 `?instanceId=`

平台分发课件时会在 URL 中带上 `?instanceId=`，**AI 不需要手动处理 instanceId**，直接 `new MuskCollect()` 即可。

### 7.4 报告页（如果大纲规划了"作答报告"页）

如果大纲规划了一个"看作答报告"的页面（通常作为单独一页，由教师查看），该页需要通过 `query` 拉取所有学生的作答数据。**报告页代码写在 `<template class="page-data">` 内**，通过 `window.parent` 访问主页的 SDK 实例：

```html
<template class="page-data" data-id="20" data-name="作答报告">
  <div id="report-root">加载中...</div>
  <script>
  (function () {
    // 子页内不能直接 new MuskCollect()（会拿不到主页累计的状态）
    // 应通过 window.parent 访问主页的 SDK 实例（同源 srcdoc 可访问）
    var parentSdk = new (window.parent.MuskCollect)();
    parentSdk.query('submissions').then(function (records) {
      // 按 name+clazz 分组，每组取最新一条（SDK 不支持 docId 覆盖，需端上去重）
      var latestByStudent = {};
      records.forEach(function (r) {
        var key = (r.name || '') + '|' + (r.clazz || '');
        if (!latestByStudent[key] || r.createdAt > latestByStudent[key].createdAt) {
          latestByStudent[key] = r;
        }
      });
      var rows = Object.values(latestByStudent);
      // ... 渲染表格 / 图表
    });
  })();
  </script>
...
- [ ] **未**手动引入 `musk-collect.js`（壳已自动加载）
- [ ] `new MuskCollect()` 写在主页 `<body>` 末尾的 `<script>` 中，**不**在任何 `<template>` 内
- [ ] `window.addEventListener('message', ...)` 写在主页，**不**在 `<template>` 内
- [ ] 子页（练习页、答题页等）通过 `window.parent.postMessage({type:'quizResult', pageId, correct, selected}, '*')` 发送结果
- [ ] 子页**未**注册业务消息的 `addEventListener('message', ...)`
- [ ] 身份采集弹层用 HTML 实现，**未**使用 `alert`/`prompt`/`confirm`
- [ ] 主页业务 `<script>` 位于 `<script src="...courseware-shell...">` **之前**
...
| 手动引入 `musk-collect.js` SDK（任何位置） | 壳框架已自动加载到主页 `window.MuskCollect`，重复引入容易写错位置导致 ReferenceError。直接 `new MuskCollect()` 即可。 |
| 在 `<template class="page-shared">` 内写 `new MuskCollect()` 或 `addEventListener('message', ...)` | page-shared 内容会被注入到每个子页 srcdoc，导致每个子页各跑一份独立 SDK 实例和监听器，子页 `window.parent.postMessage` 发出的消息没人接，`sdk.save` 永远不触发。 |
| 在 `<template class="page-data">` 内写 `new MuskCollect()` 或业务消息的 `addEventListener('message', ...)` | 每个子页是独立 srcdoc 文档，跨页状态无法聚合，`save` 调用也是孤立的。SDK 实例和聚合状态必须在主页唯一存在。 |
| 子页（`<template class="page-data">` 内）注册业务消息的 `addEventListener('message', ...)` | 子页只发不收，业务消息（如 `quizResult`、`pageEnter`）由主页统一处理。子页只能监听壳协议的 `restoreState`。 |
| 报告页中直接 `new MuskCollect()` 而非通过 `window.parent.MuskCollect` | 子页 srcdoc 的 SDK 实例独立于主页，拿不到主页累计的状态。同源 srcdoc 可通过 `window.parent` 访问主页 SDK，应使用 `new (window.parent.MuskCollect)()`。 |
