...
> 3. 进入本流程前，必须已经读取 `typography-guide.md` 并确定 1 套字体 preset；若第 1 页有封面图或特殊封面排版，必须已经读取 `cover-layout-guide.md` 并确定 `data-cover-layout` 与 `coverImageSlot`。
...
- **封面页例外**：第 1 页若使用 `cover-layout-guide.md`，封面根容器必须固定为 `.cover-page { width:960px; height:540px; min-width:960px; max-width:960px; min-height:540px; max-height:540px; flex-shrink:0; }`。封面所有固定 px 坐标都以这个固定画布为基准，禁止把封面根容器写成 `min-height:100%`、`width:100%`、`height:auto`、`aspect-ratio` 或响应式尺寸。
...
- 封面页必须按 `cover-layout-guide.md` 的 7 种封面版式之一生成，并在第 1 页根节点记录 `data-cover-visual`、`data-cover-layout`；若使用封面图，还必须在媒体层记录 `data-cover-slot`。
- 封面页根节点必须是固定尺寸 `.cover-page`，实际显示尺寸固定 `960×540`，否则 `cover-layout-guide.md` 的固定 px 坐标无效。
- 封面页不是自由排版任务。生成第 1 页时必须复制 `cover-layout-guide.md` 第四节对应版式的固定版式骨架（固定 CSS/HTML 骨架），只替换字段值；禁止自己写一个“相似”的 flex/grid 封面，禁止改变标题、图片、教材版本/年级/作者信息的槽位。
- 标题字号、位置、说明文字位置、页脚信息位置和图片槽位大小以 `cover-layout-guide.md` 的固定 px 坐标为准；禁止再套用“封面标题必须居中”的旧规则。
- 封面图必须落在 `coverImageSlot` 指定区域内。局部封面图必须使用对应固定槽位：`cover-05-left-visual=480×540`、`cover-06-organic-visual` 可见遮罩 `415×540` 且源图目标 `415×709`、`cover-07-image-card=900×357`；若图片比例与槽位不匹配，应使用 `contain` 保护主体或重新按槽位 prompt 生图，禁止用 `object-fit: cover` 裁切主体。
- 封面版式属于原版课件首次生成结构，后续模板替换 skill 不得重新选择版式、移动标题或移动图片。
...
| 元素 | 上限 |
|------|------|
| 主标题 | 按 `cover-layout-guide.md` 对应版式控制，长标题须自动缩放或换行，不能溢出文字安全区 |
| 副标题 | 按对应版式文字区控制；年级、教材版本等小字优先放入页脚/辅助信息区 |
| 封面图片 | 必须按 `coverImageSlot` 固定区域落位，不得随内容撑开，不得裁切主体 |
...
<template class="page-shared">
  <!-- 必须写入 typography-guide.md 选定字体 preset 的 @font-face、CW_TYPOGRAPHY_DECISION 和 --cw-courseware-* 字体变量；按需补充其他共享资源 -->
</template>
...
- [ ] **未使用** `100vh`，一律用 `100%`
- [ ] 第 1 页封面已按 `cover-layout-guide.md` 选择版式，记录 `data-cover-visual` / `data-cover-layout`；如有封面图，媒体层记录 `data-cover-slot` 且图片未被裁切主体
- [ ] `page-shared` 已包含 `CW_TYPOGRAPHY_DECISION`、所选字体 preset 的 `@font-face` 和 `--cw-courseware-*` 字体变量
