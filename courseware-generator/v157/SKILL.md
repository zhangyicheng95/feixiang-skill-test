...
7. **封面图先定槽位**：若第 1 页需要封面图，调用 `generate_image` 前必须先读取 `cover-layout-guide.md`，确定 `data-cover-visual`、`data-cover-layout` 和 `coverImageSlot`，再把目标尺寸、比例、横竖方向和构图安全区写入该条图片描述。若封面图不是 `full-bleed-background`，prompt 必须明确它是局部图片槽位，并写清 `cover-05:480×540`、`cover-06:415×709`、`cover-07:900×357` 等目标尺寸，禁止写成 16:9 整页背景图。禁止生成任意比例图片后在 HTML 中用 `object-fit: cover` 裁切主体。
...
2. 读取 **cover-layout-guide.md**，根据第 1 页内容和封面图角色选择设计老师给出的 7 种封面版式之一，确定 `data-cover-visual`、`data-cover-layout` 和 `coverImageSlot`。封面根容器必须固定显示为 `960×540` 的 `.cover-page`，再在这个固定坐标系内使用标题位置、字号、图片槽位大小和图片位置；这些必须在原版课件生成时完成，禁止留到模板替换时重排。**封面版式必须复制 `cover-layout-guide.md` 第四节的固定 CSS/HTML 骨架，只替换 `{{title}}`、`{{subtitle}}`、`{{meta}}`、`{{coverImageUrl}}` 等字段；禁止自由重写相似结构、移动教材/年级/作者信息、改变图片槽位。**
3. 读取 **html-guide.md**。
...
6. **使用 Phase 3 中准备的素材 URL**：生成 HTML 时，将图片和音频的 URL 嵌入对应页面，不要自行创建 base64 内容。封面图必须按 `cover-layout-guide.md` 的固定 px 槽位落位，不得用 `flex: 1`、自动填充或随内容撑开的方式代替图片槽位；若图片比例明显不匹配槽位，应使用 `contain` 保护主体或重新按槽位 prompt 生图，禁止裁切主体。**强互动页在生成过程中可按需调用 `generate_image`、`generate_voice` 补充素材；若补调 `generate_image`，同样必须先按 `image-generation-guide.md` 做逐条命中式增强。**
...
- **固定使用 v145 测试壳**：原版互动课件 HTML 必须在所有 `<template class="page-data">` 之后引用 `<script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/user/upload/admin/HgSiredEejFXx94ofdiCZ8.js"></script>`，禁止使用 `musk-online` 壳或其他壳 JS。
