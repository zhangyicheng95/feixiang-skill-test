...
☐ 动作 3：read templates/<选定模板>/layouts/*.html
         （当前数学探索风共有 8 个版式骨架，决定每页怎么写）
         - layouts/cover.html    → 用于第 1 页课件总封面
         - layouts/intro.html    → 用于情境导入 / 古题 / 生活实例
         - layouts/concept.html  → 用于概念讲解 / 要点并列
         - layouts/formula.html  → 用于公式呈现 / 推导过程
         - layouts/example.html  → 用于例题讲解 / 解题步骤
         - layouts/exercise.html → 用于选择题练习（含完整互动 script）
         - layouts/compare.html  → 用于方法对比 / 概念辨析
         - layouts/summary.html  → 用于课堂小结 / 末页

☐ 动作 4：在创建课件 HTML 骨架时，把 shared.css 的全部内容
...
| 情境导入 / 古题 / 生活实例 | `templates/<选定模板>/layouts/intro.html` |
| 概念讲解 / 要点并列 | `templates/<选定模板>/layouts/concept.html`（可按内容量改成 2/3/4 卡） |
| 公式呈现 / 推导过程 | `templates/<选定模板>/layouts/formula.html` |
| 例题讲解 / 解题步骤 | `templates/<选定模板>/layouts/example.html` |
| 选择题练习 | `templates/<选定模板>/layouts/exercise.html`（必须保留完整 `<script>`） |
| 方法对比 / 概念辨析 | `templates/<选定模板>/layouts/compare.html` |
| 课堂小结 / 末页 | `templates/<选定模板>/layouts/summary.html` |
| 强互动游戏页（现有 layouts 没法承载时） | **自由写 HTML，但必须使用模板的 CSS 变量**（`var(--c-primary)`、`var(--ff-title)` 等），保持视觉一致 |
...
5. 根据内容量增删元素（如 concept.html 的卡片可改成 2/3/4 张），但**不要**修改配色变量、字体变量、关键 class 名
