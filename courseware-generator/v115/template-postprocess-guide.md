...
- 每页标题的原有结构、字号、字体、下划线/横杠、`border-bottom`、`display`、`margin`、`padding`
- 直接给 `component-shell`、`media-block`、`button-skin`、`title-block` 写新的组件外观规则；尤其禁止新增或覆盖 `display`、`position`、`width`、`height`、`margin`、`padding`、`gap`、`flex`、`grid`、`border`、`border-width`、`border-style`、`border-radius`、`box-shadow`、`transform`、`overflow`、`object-fit`、`clip-path`、`font-size`、`line-height`
...
8. **先铺模板代码，再改单页**：
   ...
   - 根据 AI 教案/大纲页面意图，为页面根节点补一个 `data-cw-variant="<visualVariant>"`；该字段只选择视觉范式，不能改变组件位置、字体结构、标题横杠或组件样式。
...
10. **普通页做完整页面级视觉替换，但不重排模型输出**：
   - 普通展示页、讲解页、总结页允许替换页面背景、页面文字颜色和页面级颜色变量。
...
11. **互动页做轻量模板替换，但不能漏替换**：
   - 互动页必须替换页面背景和外层色彩变量；按钮、反馈只通过原变量自然换色。
...
3. **批次 1-N：按连续页码套模板**
   ...
   - 本批只给本批页面的 `page-root` 补 `data-cw-template`、`data-cw-variant`。
