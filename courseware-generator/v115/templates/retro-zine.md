...
- 根据 AI 教案/大纲页面意图，在每一个 `page-root` 上添加一个 `data-cw-template="retro-zine"` 和 `data-cw-variant` 视觉变体；视觉变体只改变页面背景、页面文字颜色、颜色变量和页面氛围，**不得移动或重排模型已生成的组件位置，也不得改变组件样式**。
...
|---|---|---|---|---|
| `hero-cover` | 原模板 Slide 1 hero title | 封面、章节开场 | 暖纸底、绿色强调色、页面氛围 | 不新增题签/印章/英文栏头，不把其他组件强行居中，不改组件样式，不删除标题横杠 |
...
| `visual-feature` | 原模板 Slide 5 full visual | 图片/图示/实验现象页 | 页面背景和颜色变量 | 不改图片本身颜色、滤镜、URL、图片位置或图片组件结构，不给图片容器新增大边框/底色 |
...
- `title-block`：默认不改标题结构、字体、字号、横杠、边距和下划线；若原标题颜色使用 `--primary-color`，可通过变量自然换色，不新增题签文案
...
- 封面：使用 `hero-cover` 的纸张底和绿色强调色；不新增章编号签条、印章或英文栏头，不强行重排标题
- 目录 / 概念卡：使用 `info-grid`
