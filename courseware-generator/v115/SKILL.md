...
Phase 6: 发布后版面满意度确认
  ...
  → 若不满意：读取 templates/README.md，调用 ask_user 让用户单选模板
     ↓
Phase 7: 模板替换与二次交付
  读取 template-postprocess-guide.md → 读取所选模板的代码资产 → 追加共享模板层 → 按连续页码只给 page-root 补模板标识 → 做覆盖表与结构指纹校验
...
5. **Phase 7 必须像 Phase 4 分批生成课件一样分步执行**，禁止把“注入模板”当成一次性自由改造。固定顺序如下：
   ...
   - **Step 7.4 每批只做本批页面的模板标识**：为本批所有 `page-root` 增加 `data-cw-template="<selectedTemplateId>"` 与 `data-cw-variant="<visualVariant>"`。默认不修改页面内部 DOM；颜色适配优先通过模板 CSS 在 `page-root` 上设置 `--primary-color`、`--accent-color`、页面背景和文字颜色完成，不得直接重写组件、按钮、图片容器或标题结构样式。
...
10. **根据 AI 教案/大纲页面意图匹配视觉变体**：在 `page-root` 上补 `data-cw-variant="<visualVariant>"`，只选择背景、颜色变量和页面氛围，不允许模板约束或重排模型已生成的组件位置。
11. **禁止新增模板文案**：模板里的示例题签、徽章、英文栏头、印章文字只表示原版视觉来源，不能写进课件。严禁新增 `ANCIENT PROBLEM`、`CLASSROOM ZINE`、`MISSION`、`READY`、`LEVEL` 等原课件没有的可见文字。
12. **替换深度按页面类型区分**：
   - 普通页：必须替换页面背景、页面文字颜色、页面级装饰和整体色彩变量，但不得为了贴合模板截图移动组件、重排卡片、改组件形状、删除标题横杠或强行改成图表/时间线。
   - 互动页：也必须替换为模板风格，但只允许替换页面背景、页面文字颜色、按钮颜色变量、反馈颜色变量和外层色彩变量；禁止改动互动主体结构、组件样式和按钮样式。
