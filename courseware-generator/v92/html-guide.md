...
如果 Phase 2.5 用户选了某个视觉模板（如 `math-explorer`），整份 HTML 生成流程改为「**skeleton + 填槽**」模式，本节规则**覆盖**本文件后续部分内容（特别是 § 9 骨架/分批的"自写"部分）。
...
### 0.2 页内容来源（覆盖 § 9.4 的"自由写"部分）

**禁止凭记忆默写每页 HTML**。每生成一页的标准动作：

```
1) 根据该页用途（来自大纲），按 template.md「layout 决策树」选一个 layout
2) read templates/<选定模板>/layouts/<选定 layout>.html
3) 在 reasoning 中，把 layout 字符串里所有 {{XXX}} 替换为该页真实内容
   - 文字占位符 → 大纲字段
   - 图片占位符 → Phase 3 generate_image 返回的 URL
   - exercise 的 {{CA}}/{{CB}}/{{CC}}/{{CD}} → 正确选项填 "1"、其余填 "0"
4) 把【替换后的整段 <template class="page-data"> 块】作为本页输出
```
...
- 替换 layout 已有的占位符以外的文本

### 0.3 分批注入（§ 9.3 / § 9.4 / § 9.6 流程仍适用）
...
「分批策略」「resourceId 链式跟踪」「批次串行」等规则与本文件 § 9.3 / § 9.4 / § 9.6 完全一致。
