...
**禁止从零开始凭记忆写整份 HTML**。当用户选了模板（`math-explorer` 等，非 `none`）：

1. `read templates/<选定模板>/template.md` → 了解占位符表与 layout 决策树
2. `read templates/<选定模板>/skeleton.html` → 拿到完整骨架（已含壳脚本 + 完整 shared.css + `<!-- CW_PAGES -->` 标记）
3. `create_file` 课件.html → content **原样**复制步骤 2 的字符串，**禁止增删任何字符**（仅把 `<title>{{COURSEWARE_TITLE}}</title>` 中的占位符替换为实际标题）
4. 对每一页：`read templates/<选定模板>/layouts/<选定 layout>.html` → 在 reasoning 中替换 `{{占位符}}` → `edit_file` 注入到 `<!-- CW_PAGES -->` 之前
5. **禁止**修改 layout 内的 `class` / `<script>` / `data-*` 属性
6. **禁止**省略 layout 内任何 `<div>` 块
7. **禁止**在 page-data 内再写一份 `<style>`（样式已在 skeleton 的 page-shared 中）
...
4.1 创建骨架文件
   ☐ create_file 课件.html
       ...
       仅把 <title>{{COURSEWARE_TITLE}}</title> 中 {{COURSEWARE_TITLE}} 换为大纲标题
   ☐ 记录 resourceId_A
...
4.2 读取 html-guide.md 与 interaction-design
   ...
   ☐ 注意：模板的 shared.css 已在 skeleton.html 中，禁止再写一份；
            interaction-design 的 CSS 变量可追加到 page-shared 的 <style> 中（如有需要）

4.3 按 html-guide § 步骤 0 做页面分类（强互动 vs 普通页）

4.4 分批注入（每批 1-5 页）
   对每一页：
     1) 按 template.md「layout 决策树」选一个 layout
     2) read templates/<选定模板>/layouts/<layout>.html
     3) reasoning：把 layout 字符串里所有 {{XXX}} 替换为大纲该页的真实值
        - 文字占位符 → 大纲字段
        - 图片占位符 → Phase 3 准备的 URL
        - exercise 的 {{CA}}/{{CB}}/{{CC}}/{{CD}}：正确填 "1"、其余填 "0"
     4) 强互动页：layout 决策树没有命中 → 不用 layout，自由写 HTML，
        但仍套 <template class="page-data" data-id="N" data-name="...">，
