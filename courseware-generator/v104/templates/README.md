...
> 本文件由 SKILL 的 Phase 3 读取，列出当前所有可用的视觉模板。
...
3. templates/<模板路径>/layouts/*.html     ← 仅供静态展示页选择 layout；交互页不要直接套 template layout
4. 输出：空课件 HTML resourceId + 页面 layout 映射表 + 占位符填充清单

Phase 5:
1. 不再重新选择 layout，不再重写 page-data
2. 只根据 Phase 4 的占位符填充清单替换 {{P*_...}}
3. 所有交互页都保留 Phase 4 已确定的 v69 原生互动结构，只替换内容和少量微调占位符
```
...
│  ✅ 空课件框架规则（Phase 4 先固定壳和 page-data）│
│  ✅ 交互页的背景壳 / 标题区 / 非阻挡装饰          │
└────────────────────────────────────────────────┘
┌─ 模板【不管】的事 ───────────────────────────────┐
│  ❌ generate_image 画风                          │
