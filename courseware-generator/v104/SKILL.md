...
| 静态展示页优先使用 `templates/<模板路径>/layouts/` 里的版式骨架；所有交互页一律使用 v69 原生互动结构并继承模板背景壳 | Phase 4 / Phase 5 | 内页风格回到默认课件样式，或互动能力被模板误伤 |
| 颜色 / 字体 全部走 CSS 变量（`var(--c-primary)` / `var(--ff-title)` 等）禁止硬编码 | 每页生成时 | 配色字体飘忽不统一 |
| 交互页中必须完整保留 v69 原生互动组件的 `<script>`、状态恢复协议与 `data-*` 结构，模板不能替换这些协议 | 练习题/互动页 | 选项无法点击、状态无法恢复 |
...
- ✅ 强互动页只需要：使用模板的 CSS 变量配色 + 背景壳 + 标题区
...
1. 先做页面分流，再匹配 layout
   ...
   - B 类：交互页（选择、拖拽、连线、翻牌、配对、排序、点击判定、含状态恢复脚本的练习页）→ 一律使用 v69 原生互动结构
   ...
   - B 类交互页不要直接套 template layout（即使模板提供 exercise.html 也只作视觉参考）
   - 若模板缺少所需静态 layout：使用 v69 标准结构，但必须套模板变量、背景层、标题区和基础装饰
   - B 类交互页只允许模板接管背景壳，不允许模板改写互动组件 DOM / script / 状态协议
...
4. 输出「占位符填充清单」
   ...
   - 若某页是 B 类交互页，必须在清单里标注「native-interaction」以及所使用的背景壳类名（如 `rz-native-shell` / `bo-native-shell`）
...
6. 对 B 类交互页，只允许修改模板背景壳的标题、背景、弱装饰，不允许把互动组件重新塞进模板卡片阵列或复杂分栏
...
- **模板约束视觉**：若选定模板，颜色/字体/版式由模板规范，AI 不自由发挥配色；但交互页只继承模板背景壳，互动组件本体仍走 v69 原生协议；`generate_image` 的画风仍由题目内容自由决定。
