...
> 本文件是 courseware-generator 技能的 Phase 5 子流程。进入本流程时，课件大纲已由 Phase 1（outline-guidance.md）生成并经 Phase 2 用户确认，**模板已在 Phase 3 由用户强制选定**，所需素材与空课件框架已由 Phase 4 准备完毕。

```
步骤 -1 装载模板：读取 Phase 3 已选模板的 template.md / shared.css / layouts
  ↓
...
> 3. Phase 4 已完成素材准备与空课件框架搭建，并输出占位符填充清单。生成 HTML 时须使用这些素材 URL，**严禁使用 base64 编码替代**。
...
进入步骤 -1 后，必须按顺序读取**选定模板的全部入口文件**：

| 文件 | 用途 | 是否必读 |
|------|------|----------|
| `templates/<模板路径>/template.md` | 风格说明书 | ✅ 必读 |
| `templates/<模板路径>/tokens.json` | 颜色/字体/间距 token | ✅ 必读 |
| `templates/<模板路径>/shared.css` | 待装入 `<template class="page-shared">` 的样式 | ✅ 必读 |
| `templates/<模板路径>/layouts/*.html` | 版式骨架库（复制时保留 exercise 等页的 `<script>`） | ✅ 必读 |
...
layout 选择应在 Phase 4 完成。Phase 5 不再重新决定 layout，只检查 Phase 4 的页面 layout 映射表是否合理。读取选定模板的 `template.md` 查看可用 layouts 列表和决策树（每个模板的版式不同）。
...
**模板模式下**：这里注入的是「空模板页」而不是最终内容。每页必须先复制对应 layout，再把通用占位符改为带页码前缀的唯一占位符，例如 `{{title}}` → `{{P3_TITLE}}`。最终内容在 Phase 4 通过替换 `{{P*_...}}` 填入。

**🚨 关于 `<template class="page-data">` 内容的来源（模板已在 Phase 3 强制选定）**：

- **【固定规则】**：**Phase 4 必须**从 `templates/README.md` 中该模板对应的实际目录路径读取 `layouts/<对应版式>.html`，复制 `<template class="page-data">` 块作为该页空框架起点，固定 `data-id` / `data-name`，并把通用占位符改为页码前缀占位符（如 `{{P3_TITLE}}`）。**Phase 5 只替换这些页码前缀占位符**。若当前模板没有对应 layout，则该页改用 v69 标准互动结构，但仍必须继承模板变量、标题区和基础装饰。禁止自行决定 `<style>` 中的配色、字体、布局——这些必须由模板决定。
