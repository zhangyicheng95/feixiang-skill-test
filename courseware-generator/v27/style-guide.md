...
采用**骨架 + 按复杂度分批增量注入**模式：先创建包含注入标记的 HTML 骨架，根据大纲中各页的生成复杂度权重分批（每批权重预算 ≤ 10，总权重 ≤ 10 时一批生成全部页面），每批通过 `multi_edit` 将 `<template>` 标签注入到标记位置。
...
> **⚠️ 分批的目的是保证生成质量，不是为了分批而分批。** 简单课件（总权重 ≤ 10）应一批生成全部页面。分批策略详见 content-guide.md 第 5.3 节。
>
