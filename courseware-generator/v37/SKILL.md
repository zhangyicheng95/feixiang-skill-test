...
3. content-guide.md 中原来的参数确认、检索、大纲规划步骤**全部跳过**——这些工作已由 Phase 1 完成。
4. 从 content-guide.md 的**步骤 0（复杂度评估）**开始执行：
   - 首先根据已确认大纲中各页的页面类型和交互设计，评估每页的**生成复杂度权重**（轻量1/中等2/重量4/超重6）
   - 然后按权重进行分批装箱
   - 创建 HTML 骨架 → 分批生成 → 逐批注入
5. **使用 Phase 3 中准备的素材 URL**：生成 HTML 时，将图片和音频的 URL 嵌入对应页面，不要自行创建 base64 内容。
6. style-guide.md 负责 HTML 的编写规范（`<template>` 结构、画布尺寸、壳框架等）。
