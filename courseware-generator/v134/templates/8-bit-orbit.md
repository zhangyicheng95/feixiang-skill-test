...
- 根据 AI 教案/大纲页面意图，在每一个 `page-root` 上添加一个 `data-cw-template="8-bit-orbit"` 和 `data-cw-variant` 视觉变体；视觉变体只改变页面背景、组件框外标题/正文/辅助文字字体族、组件框外标题/正文/辅助文字颜色、文字阴影和页面氛围，**不得移动或重排模型已生成的组件位置，也不得改变组件样式或组件框内文字颜色**。
...
- 第 1 页若已有 `data-cover-visual="full-bleed-background"`，说明封面生图是整页主视觉；此时本模板不得用像素背景覆盖封面图，只能应用标题字体、标题颜色、文字阴影、独立遮罩和弱装饰。若封面是 `side-visual` 或 `inline-card`，模板背景可保留，但封面图必须作为 `media-block` 保护。
...
- `content-block`：只改组件框外正文字体族和文字颜色，不拆分/重排内容，禁止让颜色规则穿透到组件框内
...
- 互动页标题结构、标题横杠、字号、行高和边距默认保持原样；只允许按模板配方改组件外标题字体族、颜色和文字阴影
...
- 这套模板明确包含：10 页来源范式的页面背景、组件框外页面文字颜色、标题/正文/辅助文字字体族、组件框外标题/正文/辅助文字颜色、文字阴影和颜色系统。
