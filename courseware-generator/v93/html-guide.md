...
## 〇、关于模板的硬性规则（Phase 2.5 选了模板时必读）

如果 Phase 2.5 用户选了某个视觉模板（如 `math-explorer`），整份 HTML 生成流程改为「**skeleton + multi_edit 填槽**」模式，本节规则**覆盖**本文件后续部分内容（特别是 § 9 骨架/分批的"自写"部分）。

### 0.1 骨架来源（覆盖 § 9.2 自写骨架）

**禁止凭记忆写骨架**。骨架由模板提供：

```
1) read templates/<选定模板>/skeleton.html   # 拿到完整骨架字符串
2) create 课件.html
   content = 上一步 read 到的字符串【原样复制，禁止增删任何字符】
   仅把 <title>{{COURSEWARE_TITLE}}</title> 中的 {{COURSEWARE_TITLE}} 换为课件实际标题
```

skeleton.html 已经包含：

- 正确的壳框架 `<script src="https://musk-test.fbcontent.cn/.../HgSiredEejFXx94ofdiCZ8.js">`，**禁止替换为 metis-lectio 等其他脚本**
- 完整的 `<template class="page-shared"><style>`，含模板 shared.css 全套样式（含 8 个版式样式 + Tailwind 兼容子集）
- `<!-- CW_PAGES -->` 注入标记
- 加载占位 `<div id="cw-loading">`

### 0.2 每页内容生成方式（覆盖 § 9.4 的"自由写"部分）

**核心原则**：AI **只负责给模板填值**（输出键值对），**HTML 结构永远来自 layout 文件**，由 `multi_edit` 工具做精确替换。**严禁**在 reasoning 里把整段 layout 替换好再注入。

```
对大纲每一页（按 data-id 顺序，逐页串行）：
  1) 根据该页用途，按 template.md「layout 决策树」选 1 个 layout
  2) read templates/<选定模板>/layouts/<layout>.html
     → 拿到带 {{XXX}} 的 layout 字符串
  3) reasoning：根据大纲准备该页所有占位符的真值
     ★ 仅准备「键 → 真值」的映射表，不要碰 HTML 结构
  4) multi_edit（一次调用 = 注入 + 填槽，N+1 个 editItems）：
     editItems:
       # 第 1 项：先把 CW_PAGES 替换为 layout 原样字符串（占位符未替换）
       - oldString: "<!-- CW_PAGES -->"
         newString: "<layout 字符串原样，含 {{XXX}}>\n\n<!-- CW_PAGES -->"
       # 第 2~N+1 项：把刚注入的所有占位符替换为真值
       - oldString: "{{DATA_ID}}"
         newString: "3"
       - oldString: "{{TITLE}}"
         newString: "化繁为简"
       - oldString: "{{QUESTION_TEXT}}"
         newString: "上有 35 头，下有 94 足..."
       # ... 列出该 layout 的全部占位符
```

**关键点**：

- multi_edit 的 editItems **按顺序串行执行**：第 1 项注入 layout 后，第 2~N+1 项才能在新文本中匹配到 `{{XXX}}`
- 必须**逐页处理**（不要把 5 页打包在一次 multi_edit 里）：否则同名占位符（如多个 `{{TITLE}}`）会冲突。逐页处理后，每页处理完占位符即被替换为真值，下一页注入新占位符不冲突
- AI 的输出**只有键值对**，不再有「整段 HTML newString」

**严禁**：

- 修改 layout 内的任何 `class="..."` 值
- 删除 layout 内的任何 `<div>` / `<script>` 块
- 给 layout 添加新的 `<style>` 块（样式已在 shared.css 中）
- 在 reasoning 中把 `{{XXX}}` 提前替换为真值，再把整段 layout 作为 newString 注入（这就是"重写"，应该用 multi_edit 替换）

### 0.3 同名占位符与跨页冲突

每页执行完 multi_edit 后，该页的 `{{XXX}}` 占位符已全部替换为真值。下一页注入新 layout 时，**新的 `{{XXX}}` 在文件里是唯一存在的**（其他页已无占位符），multi_edit 的 oldString 唯一性约束自然满足。

**资源 ID 跟踪**：每次 multi_edit 执行后会生成新的 resourceId，下一次操作必须使用最新的 resourceId。各页**严格串行**，禁止并行处理多页（同一 resourceId 的并行修改会分叉）。

### 0.4 强互动页例外

如果某页的教学需求**没有命中** template.md「layout 决策树」的任意一种（如拖拽匹配、Canvas 动画、闯关游戏），按本文件 § 八「强互动页」处理：

- 自由写 HTML、自由写 `<script>`
- 但仍套 `<template class="page-data" data-id="..." data-name="...">` 外壳
- **必须**使用 shared.css 提供的 CSS 变量（`var(--c-primary)` / `var(--c-accent)` / `var(--ff-title)` 等），**禁止硬编码颜色和字体**
- 仍按本文件 § 八「JS 复杂度要求」等约束执行
- 此时 multi_edit 只需 1 个 editItem：把 `<!-- CW_PAGES -->` 替换为 `<该页完整 HTML>\n\n<!-- CW_PAGES -->`

### 0.5 LaTeX / 外部字体 / 外部脚本禁忌

- ❌ LaTeX 公式语法（`\(...\)` / `$$...$$` / `\frac{}{}` / `\sqrt{}`）—— 没有渲染器，会显示原文乱码。改用 HTML + Unicode（详见 template.md § 五）
- ❌ `<template class="page-shared">` 内加 `@import url(...)` 或外部 `<link>` —— 安全机制会过滤为 `[unsafe-url-removed]`，整段 CSS 失效
- ❌ 在 `<head>` 里加 metis-lectio / anime / Tailwind CDN 等脚本 —— 模板已自带需要的样式，外部脚本可能与壳冲突

### 0.6 自检 checklist（Phase 5 验收时执行）

- [ ] 每页都用了某个 layout（或属于「强互动页」例外）
- [ ] layout 内所有 `{{XXX}}` 占位符均已替换为真实值，无残留
- [ ] 没有修改任何 layout 的 class / script / data-* 属性
- [ ] 壳脚本 URL 是 `HgSiredEejFXx94ofdiCZ8`（不是 metis-lectio）
- [ ] `<template class="page-data">` 数量 = 大纲页数
- [ ] `<!-- CW_PAGES -->` 标记仍在文件尾部（注入后保留）

### 0.7 与本文件其他章节的优先级

当本节规则与本文件其他章节冲突时，**以本节为准**。但通用规则（壳框架不动、960×540 画布、不溢出、互动状态 saveState/restoreState 等）保持一致。
