...
### 红线 3：选定模板后，必须按模板规范生成整份课件

当 Phase 3 用户选定了某个模板（如 `retro-zine`），**整份课件 HTML 必须严格按模板规范生成**：

| 必做动作 | 在哪做 | 不做的后果 |
|---------|-------|-----------|
| 把 `templates/<模板路径>/shared.css` **完整复制**到课件 HTML 的 `<template class="page-shared"><style>` 中 | Phase 4 创建骨架时 | 整份课件用不上模板字体/配色变量，视觉上像没装模板 |
| 标准讲解页优先使用 `templates/<模板路径>/layouts/` 里的版式骨架；若模板未提供对应互动 layout，则继续使用 v69 标准互动结构并继承模板视觉变量 | Phase 5 生成每页时 | 内页风格回到默认课件样式，或互动能力被模板误伤 |
...
- ✅ 如果模板不提供 `exercise.html` / `intro.html` 等所需版式，对应页面继续使用 v69 标准互动结构，但必须继承模板 `shared.css` 的变量、标题区和基础装饰
...
| **[html-guide.md](html-guide.md)** | 课件**HTML生成**完整指南：`<template>` 标签用法、960×540 画布规则、壳框架约束、互动状态管理、复杂度评估、骨架创建→分批生成页面→验收交付，以及**按模板生成的硬性规则** | Phase 5 执行，将大纲转化为 HTML |
...
Phase 4: 素材准备 + 空课件框架搭建
  ...
  根据 Phase 3 已选模板逐页匹配 layout，先搭出含壳、page-shared、全部 page-data 的空课件框架
     ↓
Phase 5: HTML 课件生成
...
- 模板选择完成后，进入 Phase 4 前先读取该模板在 `templates/README.md` 中对应路径下的 `template.md`，了解该模板的风格规范，作为 Phase 4 生成素材和 Phase 5 生成 HTML 时的全局风格约束

---

## Phase 4 详细说明：素材准备 + 空课件框架搭建

大纲确认且模板选择完成后、HTML 生成前，根据大纲中各页的教学内容和交互设计，提前准备所需的各类素材。**本阶段还必须先搭出空课件框架**：壳、`page-shared`、全部 `page-data`、`data-id`、`data-name`、layout 结构、exercise 的互动 `<script>` 都先固定好；Phase 5 只负责填内容。
...
## Phase 5 详细说明：HTML 课件生成

### Phase 5.0 装载模板（模板已在 Phase 3 强制选定；必须在任何 HTML 生成动作之前完成）

**⚠️ 这是 Phase 5 的强制前置步骤。如果你跳过本步骤直接开始写 HTML，整份课件视觉就会和模板脱节，违反「红线 3」。**

**若 Phase 4 已按模板搭好空课件框架**，Phase 5 不再重新创建骨架、不再重新复制 shared.css、不再重新决定每页 layout。Phase 5 只读取 Phase 4 产出的「空课件 HTML resourceId + 占位符填充清单 + 素材绑定表」，然后把内容填入对应占位符。
...
3. 把 interaction-design 中的共享 CSS 变量（缓动函数等）也写入 `<template class="page-shared">` 的 `<style>` 中。
   - 若 Phase 5.0 已经把模板的 shared.css 写进去，则**追加** interaction-design 的变量到同一个 `<style>` 块下面，二者并存。
...
- 强互动页：若 Phase 4 已生成强互动空容器，则只填入互动内容；若必须补写强互动代码，仍须保留外层 page-data、data-id、page-title 与模板 CSS 变量

**由于 Phase 3 已强制完成模板选择（红线 3）**：

**操作方式**：
1. 使用 Phase 4 的空课件 HTML resourceId 作为修改目标
2. 使用 Phase 4 的占位符填充清单作为唯一内容来源
...
4. Phase 4 负责「用什么」——根据大纲准备图片、音频、题目等素材。**图片生成完全自由**，由 AI 根据题目内容自由编写 prompt，不受模板约束。
5. html-guide.md 负责「怎么做 + 怎么写」——将大纲和素材转化为 HTML 页面，同时规范 HTML 代码的格式与样式，并强制遵守已选模板规范（Phase 5）。
