...
> 本文件只在 Phase 4 生成 HTML 前读取，用于第 1 页封面首次生成。它沉淀设计稿最上方 7 列封面排版范式：文字大小、文字相对位置、图片相对位置和封面图角色。使用本文件前必须已按 `typography-guide.md` 选择字体 preset；本文件只规定字号层级和位置，不重新选择字体家族。Phase 7 模板注入只能读取已生成封面的标记做视觉适配，禁止重新选择版式、移动标题或移动图片。
