...
11. **字体必须可加载或可降级**：字体不是模型自带能力。若目标字体没有已确认 Web Font / 内部 CDN / 系统字体来源，必须使用 fallback 栈，禁止伪造 `<link>` 或 `@font-face`。
...
5. 若目标字体有确定可用的 `<link>` 或 `@font-face` 来源，放在 `page-shared`；若没有，禁止写假链接，只写 fallback 栈。
...
4. 按 `cover-layout-guide.md` 从 7 种版式中选择一个 `data-cover-layout`。
5. 输出一行封面版式决策记录，至少包含：封面页码、`data-cover-visual`、`data-cover-layout`、选择理由、图片是否需要避让文字区。
6. 生成第 1 页封面 HTML 时必须把上述决策写入 `page-root`：
