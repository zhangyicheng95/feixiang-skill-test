...
> 本文件是 courseware-generator 技能的 Phase 7 子流程。基础互动课件已经首次发布为原版后，必须根据 Phase 1 首次信息确认中记录的 `selectedTemplateId` 读取并执行，用于生成第二份模板版互动课件。
...
2. 该课件已经完成首次 `publish_resource`，用户已经拿到了原版互动课件。
3. Phase 1 首次信息确认中已经记录模板选择 `selectedTemplateId`。
4. 原版课件最新 HTML 的 `<template class="page-data">` 数量已经等于 Phase 1 的 `lockedPageCount`，且 `data-id` 从 1 连续到 `lockedPageCount`。
...
- 保留壳框架接入方式
- 保留页数、`data-id`、`data-name`，且模板版页数必须继续等于 `lockedPageCount`
- 保留原始教学内容与素材引用
- 保留交互逻辑、事件绑定、状态保存恢复
- 对**全部页面**替换页面级视觉系统
- 对组件只做保护性识别：保留原组件结构、形状、尺寸、圆角、阴影、边框粗细、布局、事件、标题横杠、文案和组件框内文字颜色
- 对互动页只替换安全的模板层：页面背景、组件外页面文字颜色和外层页面氛围；互动区内部文字颜色保持原样
- 继承原版课件 Phase 4 的字体决策：保留 `CW_TYPOGRAPHY_DECISION` 与 `--cw-courseware-*` 字体变量，模板字体只作为 fallback，禁止重新选择或覆盖整课件字体 preset
- 对封面只做视觉适配：读取第 1 页已有 `data-cover-visual` / `data-cover-layout`，禁止重新选择封面版式，禁止移动标题、说明、页脚或封面图位置
- 不触发生图：不读取 `image-generation-guide.md`，不调用 `generate_image`，不重绘、不替换、不筛选已有图片素材
...
```text
Phase 1 已确认 selectedTemplateId
  ↓
原版 publish_resource 完成
  ↓
读取原版最新 HTML，确认 page-data 数 = lockedPageCount
  ↓
读取 templates/README.md，校验 selectedTemplateId
  ├─ selectedTemplateId 有效 → 继续
  └─ selectedTemplateId 缺失/无效 → ask_user 只补选模板
       ↓
     读取对应模板说明书
       ↓
     读取对应模板代码资产
...
3. 禁止伪造完整资产标记：不能把自己手写的短版 CSS 包在 `CW_TEMPLATE_ASSET` / `CW_TEMPLATE_VARIANT` 注释里冒充真实模板资产。只要 CSS 不是从模板资产文件原文复制而来，就视为假套版。
4. 注入后必须检查模板资产标记：
   ...
   - 最终 HTML 中必须保留 `page-shared.css` 文件头里的 `REQUIRED_VARIANTS:` 行；缺失该行说明复制的不是原文资产，禁止进入下一批。
   - 对于保留旧别名的模板，最终 CSS 中必须保留旧别名选择器，例如 `hero-cover`、`split-mission`、`closing`、`panel-grid` 等。缺失别名通常说明 page-shared 被压缩/改写，禁止发布。
5. 建立两张集合并做包含校验：
...
6. 即使当前课件暂时不用某个来源范式，也必须完整保留该模板的 10 页来源范式 CSS。后续页面映射可能会选择任一范式，不能让某些范式缺规则。
7. 若 `edit_file` 因 oldString 匹配失败、内容过长或返回空结果而未产生新 `resourceId`，必须 `read_file` 最近一次成功资源并继续原文注入；可以把 `tokens.css` 和 `page-shared.css` 拆成连续小批次原文追加，但不能改写成短版。
8. 若原版 HTML 中存在 `CW_TYPOGRAPHY_DECISION` 或 `--cw-courseware-*` 字体变量，批次 0 注入模板资产后这些标记和变量必须仍存在。模板 `tokens.css` 中的标题/正文/标签字体变量应优先引用 `--cw-courseware-*`，只有原版变量缺失时才使用模板自带字体 fallback。
9. 若最终文件中某页有 `data-cw-template` / `data-cw-variant`，但背景、标题颜色或正文颜色仍被原页 CSS 明显覆盖，优先修正模板共享 CSS 的安全选择器；若是 `page-root` 内联背景造成冲突，只允许删除 `page-root` 的 `background` / `background-image` / `background-color` / `background-size` / `background-position`。但第 1 页若 `data-cover-visual="full-bleed-background"`，必须优先保护独立 `cover-visual` 媒体层，禁止删除、替换或遮盖封面主视觉图。禁止通过改组件 DOM、改组件 class、删原页组件样式、重选字体或重写组件外观来“修好视觉”。
10. 模板资产标记之后禁止再出现未限定模板的页面背景规则，例如 `[data-cw-role="page-root"] { background... }`、`background-color`、`background-image`、`[data-cw-role="page-root"]::before { background... }`。若原课件样式中存在这类规则，必须保证模板 `tokens.css` / `page-shared.css` 位于它们之后；否则会出现原课件米灰、浅灰、水墨底图覆盖像素风/复古风背景的假套版。
11. 第 1 页若存在 `data-cover-visual="full-bleed-background"`，说明封面生图是主视觉背景。此时模板背景不能覆盖该封面图；模板只允许作用于文字颜色、文字阴影、独立半透明遮罩和弱装饰，并继承原版字体变量。第 1 页若为 `side-visual` 或 `inline-card`，模板背景可保留，但 `data-cover-role="cover-visual"` 的媒体层必须作为内容图片保护，禁止滤镜、换图、裁切主体或移动位置。
...
“沉淀模板版面”指把开源模板每一页的**安全视觉配方**都沉淀出来：页面背景、纹理/网格/星点等纯背景层、组件外标题/正文/辅助文字颜色、文字阴影和字体变量引用方式。它不包含原模板的组件位置、两栏结构、图表 DOM、按钮形状、媒体框结构、组件框内文字颜色、重新选字体或任何模板文案。

1. 每套模板的 `slide-01-*` 到 `slide-10-*` 都必须作为独立候选配方存在，不能把 10 页压缩成一种“主皮肤”。
2. 模板注入时按 AI 教案页面功能选择最合适的候选配方；这是确定性映射，不是随机混色，也不是强制混用。
3. 一份课件不需要覆盖全部 10 个来源范式，也不需要覆盖全部背景色组。若多页教学功能相同，可以使用同一个 `data-cw-variant`。
4. `data-cw-variant` 只选择安全视觉配方：页面背景、组件外标题/正文/辅助文字颜色、文字阴影和字体变量引用；禁止据此移动、重排或重做组件。
5. 二次发布前覆盖表只需列出每页 `data-cw-template`、`data-cw-variant` 和选择理由；`backgroundFamily` 可作为排查信息，但不作为发布硬闸门。
6. 若用户明确要求某页换成指定来源页或明显换色，再在当前模板候选集中选择对应变体；仍只应用视觉配方，不迁移原模板结构。
...
- 基于选中模板 10 页来源范式的安全文字配方：组件框外标题/正文/辅助文字的颜色、文字阴影和字体变量引用；只能通过 `title-block`、`content-block` 中不属于组件保护区的文字节点生效，禁止改字号、行高、边距、布局和 DOM
- 模板字体变量只能通过 `--cw-template-title-font` / `--cw-template-body-font` / `--cw-template-label-font` 继承原版 `--cw-courseware-*` 变量；若原版变量缺失，才使用模板自带 fallback
...
- 原课件 Phase 4 字体决策，包括 `CW_TYPOGRAPHY_DECISION` 注释、字体 `<link>` / `@font-face`、`--cw-courseware-title-font`、`--cw-courseware-body-font`、`--cw-courseware-label-font`、`--cw-courseware-note-font`、`--cw-courseware-latin-accent-font`
...
- 每页标题的原有结构、字号、下划线/横杠、`border-bottom`、`display`、`margin`、`padding`；标题允许按模板页级配方修改颜色和文字阴影，并继承原版标题字体变量，但不得改变标题 DOM、字号、行高、位置或装饰线
- Phase 7 重新挑选整课件字体 preset、替换原版字体资源、删除原版 fallback 栈，或把模板自带字体强行覆盖到已存在的 `--cw-courseware-*` 变量上
...
2. **先读取选中模板的说明书**：根据 Phase 1 确认的 `selectedTemplateId`，必须先读取对应的模板说明文件（如 `templates/retro-zine.md` / `templates/8-bit-orbit.md`）。只有当 `selectedTemplateId` 缺失或无效时，才允许额外调用一次 `ask_user` 只补选模板。
...
8. **先铺模板代码，再改单页**：
   ...
   - 必须完整追加，不得删减未使用变体；批次 0 后立刻检查全部 `CW_TEMPLATE_VARIANT` 标记是否仍在最终 HTML 中，并检查 `REQUIRED_VARIANTS:` 文件头和旧别名选择器仍存在。
   - 若批次 0 后发现 `REQUIRED_VARIANTS:` 缺失、旧别名选择器缺失、或 CSS 明显短于源模板资产，说明模型写成了短版 CSS，必须回到最近一次成功 `resourceId` 重做原文注入，禁止继续批次 1。
   ...
   - 根据 AI 教案/大纲页面意图，为页面根节点补一个 `data-cw-variant="<visualVariant>"`；优先从选中模板 `slide-01-*` 到 `slide-10-*` 的 10 页来源范式中选择。该字段只选择背景、组件外文字颜色、文字阴影和文字层级配色；字体变量必须继承原版 `--cw-courseware-*` 决策，不能重新选择字体，也不能改变组件位置、组件框内文字颜色、标题 DOM、标题横杠或组件样式。
...
9. **按页面意图匹配视觉变体，而不是套结构版式**：
   - 每套模板必须以原开源模板的 10 页来源范式为完整候选集；可以按页面功能使用其中一个或多个候选，禁止随机选色，也禁止使用未沉淀的临时变体。
...
10. **普通页做完整页面级视觉替换，但不重排模型输出**：
   - 普通展示页、讲解页、总结页允许替换页面背景、组件外页面文字颜色、组件外标题/正文/辅助文字颜色和文字阴影；字体变量继承原版 `--cw-courseware-*` 变量，模板字体只做 fallback。
...
11. **互动页做轻量模板替换，但不能漏替换**：
   - 互动页必须替换页面背景、组件外标题/正文/辅助文字颜色、文字阴影和页面氛围；字体变量继承原版 `--cw-courseware-*` 变量，互动主体、按钮、反馈层内部文字颜色和字体保持原样。
...
1. **读取最新 HTML 并建立页面清单**
   ...
   - 读取 Phase 1 的 `lockedPageCount`，统计最新 HTML 中全部 `<template class="page-data">`。数量必须等于 `lockedPageCount`，且 `data-id` 必须从 1 连续到 `lockedPageCount`；若少页、多页、跳号或重复，禁止进入模板替换，必须回到 HTML 生成流程补齐/修复原版课件。
...
5. **全量覆盖表**
   ...
   - 逐页列出全部页面的覆盖表，确认页数等于页面清单，等于 `lockedPageCount`，且每页都有 `data-cw-template` / `data-cw-variant`。
   - 逐页对比全量结构指纹，确认模板替换前后的组件数量、媒体数量、按钮数量、标题数量、图片数量和保护区内联 `style` 完全一致。唯一例外是 `page-root` 自身背景属性按规则被清理。
   - 只有全量覆盖表无缺失，才能进入最终检查。
6. **最终安全检查**
   - 重新检查壳协议：`<template class="page-data">` 数量等于页面清单页数且等于 `lockedPageCount`，`data-id` 从 1 连续到 `lockedPageCount`，壳脚本存在且位于页面之后，`<!-- CW_PAGES -->` 保留，不存在自写 `#cw-root` / 父级 iframe/srcdoc / 普通网页展示容器。
   - 检查原有共享 CSS 是否被完整保留；特别是标题横杠、按钮基础样式、组件基础样式、图片尺寸/圆角规则不能减少或消失。
   - 检查原版字体决策是否被完整保留；若原版 HTML 有 `CW_TYPOGRAPHY_DECISION` 或 `--cw-courseware-*` 变量，模板版必须仍保留，且模板 token 只能引用这些变量或 fallback，不能删除或覆盖。
   - 检查选中模板共享 CSS 是否完整保留；`tokens.css`、`page-shared.css` 的 `CW_TEMPLATE_ASSET` 标记、`REQUIRED_VARIANTS:` 文件头、旧别名选择器和全部 `CW_TEMPLATE_VARIANT` 标记不能减少或消失。只有手写短版 CSS 或压缩版 CSS 时，禁止发布。
...
7. **二次发布模板版**
   ...
   - 该发布是模板版互动课件；不得覆盖或隐藏此前已经发布给用户的原版互动课件。

---

## 七、发布要求

- 替换完成后，必须再次调用 `publish_resource`，把模板版互动课件发给用户。
- 禁止在 `create_file` / `edit_file` 同一轮并行调用 `publish_resource`。
- 用户最终应看到两份正式成品：首次发布的原版互动课件、二次发布的模板版互动课件。中间 edit_file 版本不要作为正式成品展示。
- 二次发布前必须确认：原版互动课件已先完成首次发布，Phase 1 已有有效 `selectedTemplateId`，模板替换覆盖了全部页面，覆盖表无缺失，组件框外文字已跟随模板，且没有新增模板文案、改动组件样式或改动组件框内文字颜色。
- 二次发布前必须确认：模板版最新 HTML 的 `<template class="page-data">` 数量仍等于 `lockedPageCount`，`data-id` 仍连续；如果原版本身少于 `lockedPageCount`，禁止用模板流程补救或直接发布，必须先回到原版 HTML 生成阶段补齐。
