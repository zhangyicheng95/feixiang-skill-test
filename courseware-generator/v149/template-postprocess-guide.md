...
读取原版最新 HTML，确认 page-data 数 = lockedPageCount
  ↓
读取 README.md，校验 selectedTemplateId
...
2. **先读取选中模板的说明书**：根据 Phase 1 确认的 `selectedTemplateId`，必须先读取对应的模板说明文件（如 `retro-zine.md` / `8-bit-orbit.md`）。只有当 `selectedTemplateId` 缺失或无效时，才允许额外调用一次 `ask_user` 只补选模板。
