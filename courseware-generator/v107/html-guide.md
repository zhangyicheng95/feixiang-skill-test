...
步骤 2  验收与首次交付：逐页核对大纲 → 调用 publish_resource 首次发布
  ↓
步骤 3  发布后版面替换（按需）：用户确认版面是否满意 → 选择模板 → 安全替换 → 调用 publish_resource 二次发布
```
...
<template class="page-data" data-id="3" data-name="重点讲解">
  ...
  <div class="lesson-page" data-cw-role="page-root">
    ...
    <main data-cw-role="content-block">
      <section data-cw-role="media-block">
        <img src="..." alt="">
      </section>
...
- **输出语义钩子**：每页必须按 3.5 节输出 `data-cw-role` 结构；交互页必须包含 `interactive-root`，有反馈浮层的页必须包含 `feedback-layer`。
...
2. 后置替换只允许修改 `page-shared`、`page-root`、`title-block`、`content-block`、`media-block` 等视觉层；**禁止**重排 `interactive-root` 内部 DOM、禁止改壳脚本、禁止改页数和 `data-id`。
3. 替换完成后，必须再次调用 `publish_resource`，向用户重新发布替换版面后的新版课件。
4. 若用户仍要求换版面，继续基于**最新一次 edit_file 返回的新 resourceId** 迭代，禁止回退到旧版本。
