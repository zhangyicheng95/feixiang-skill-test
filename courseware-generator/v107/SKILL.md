...
首次发布后，调用 `ask_user` 询问用户对当前课件版面是否满意。该确认环节必须分两步执行：
...
4. 替换时必须保留原有的壳框架、页面数量、互动逻辑、状态管理和素材引用，只允许修改受控的视觉层与版式层。
5. 替换完成后，**必须再次调用 `publish_resource`**，把替换版面后的新版互动课件重新发给用户。
6. 若用户继续要求更换版面或微调视觉，可重复执行 Phase 6 → Phase 7；但每一轮都必须基于**最近一次 edit_file 返回的最新文件版本**进行修改。
