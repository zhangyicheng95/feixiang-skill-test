...
> 本文件是 courseware-generator 技能的 Phase 4 子流程。进入本流程时，课件大纲已由 Phase 1（outline-guidance.md）生成并经 Phase 2 用户确认，所需素材已由 Phase 3 准备完毕，且 Phase 1 已记录 `selectedTemplateId`。
...
步骤 1  生成：创建 HTML 骨架 → 按复杂度分批生成页面内容 → 逐批注入骨架
  ↓
步骤 2  验收与基础稿留存：逐页核对大纲 → 保留当前最新 HTML 作为基础稿
  ↓
步骤 3  发布前模板应用（按需）：若 selectedTemplateId != default-base → 对全部页面做安全套版
  ↓
步骤 4  最终交付：调用 publish_resource，仅发布一次最终版互动课件
```
...
4. **验收通过后的基础稿只在后台保留，不对用户发布**；即使用户选择 `默认基础版`，也要等全部流程结束后统一调用一次 `publish_resource`。
5. **若 `selectedTemplateId` 为真实模板，必须在当前最新 HTML 文件上做发布前增量替换**；禁止抛弃当前文件重做一份新的整课件。
6. **本流程不再在发布后调用 `ask_user` 追问版面满意度**；模板选择必须在 Phase 1 的信息确认阶段完成。
7. **若用户选择模板，模板替换必须覆盖全部页面**；普通页做完整模板替换，互动页做轻量模板替换，但同样不能漏页。
...
- **为支持发布前安全套版，每页必须输出最小语义钩子**，见 3.5 节。
...
### 3.5 发布前模板套版所需的最小语义钩子（必做）

为保证最终发布前的模板套版只改视觉层、不误伤互动与壳兼容性，**每页 `<template class="page-data">` 内都必须带上以下 `data-cw-role` 语义钩子**：

| 钩子 | 是否必需 | 用途 |
|------|---------|------|
| `data-cw-role="page-root"` | 必需 | 页面最外层可视容器，模板替换时只允许改它的背景、边框、阴影、内边距与弱装饰 |
| `data-cw-role="title-block"` | 必需 | 标题区容器，模板替换时可调整标题布局、题签样式与装饰条 |
| `data-cw-role="content-block"` | 必需 | 主内容区容器，模板替换时可调整排版、分栏、卡片样式 |
| `data-cw-role="media-block"` | 按需 | 图片、图表、音频播放器等媒体区容器，模板替换时可调整外框和摆位，但**禁止**改变内容图片本身的颜色和素材 |
| `data-cw-role="component-shell"` | 有卡片/面板/题框时必需 | 普通页和互动页的组件框、面板框、题框、信息卡外壳；模板替换时应统一替换成模板组件框 |
| `data-cw-role="button-skin"` | 有可点击按钮时必需 | 普通页和互动页的按钮、选项按钮、提交按钮、切换按钮的视觉皮肤目标 |
| `data-cw-role="interactive-root"` | 交互页必需 | 互动 DOM 的根容器，模板替换时**禁止重排其内部结构**，只允许改外围壳与视觉 token |
| `data-cw-role="feedback-layer"` | 有反馈层时必需 | 正误反馈、解析弹层、提示层容器，模板替换时可换皮肤但禁止改事件逻辑 |

#### 使用规则

- 每页只能有 **1 个** `page-root`。
- 每页应有且仅有 **1 个**主 `title-block` 和 **1 个**主 `content-block`；若存在多列或多卡片布局，使用内部 class 继续细分，不要额外复制同名钩子。
- 普通页中的信息卡、题框、讲解框、图片区外框，尽量都包在 `component-shell` 内，便于模板统一换组件框。
- 所有真实可点击的按钮、选项块、切换按钮、提交按钮，尽量都挂上 `button-skin`，便于模板统一替换按钮组件样式。
- 含拖拽、选择、排序、Canvas、闯关等交互的页面，必须提供 `interactive-root`，并让交互脚本以内聚方式绑定其内部节点。
- `media-block` 若包含 `<img>`、`<svg>`、题图、插图等内容素材，模板替换只允许改其容器、外框、底板、圆角、阴影和摆位；**禁止**对素材节点本身施加 `filter`、`hue-rotate`、`mix-blend-mode`、`backdrop-filter`、着色遮罩或替换原图。
- `button-skin` 只负责按钮视觉皮肤；模板替换时必须保留原按钮节点的事件绑定关系，包括 `onclick`、监听函数名、`id`、脚本查询选择器和 `data-*` 标识。
...
- **基础稿只做内部留存，不对外发布**：使用最近一次 `edit_file` 返回的最新 `resourceId` 继续进入模板判断与最终交付流程。
- **本阶段不再额外询问版面满意度**：模板是否需要应用、应用哪套模板，都应以前置确认得到的 `selectedTemplateId` 为准。

### 10.4 发布前模板替换（按预选结果）

若 Phase 1 中记录的 `selectedTemplateId` 为真实模板（而非 `default-base`）：

1. 不重新走大纲与整课件生成流程，直接基于**当前最新 resourceId 对应的 HTML 文件**进行发布前替换。
2. 模板替换必须覆盖**全部页面**，禁止只替换封面、目录或部分普通页。
3. 普通页必须替换 `page-shared`、`page-root`、`title-block`、`content-block`、`media-block`、`component-shell`、`button-skin`、`feedback-layer` 等视觉层。
4. 模板可以美化 `media-block` 的容器，但**禁止**修改内容图片本身的色相、亮度、对比度、透明度、混合模式和原始素材 URL；禁止为了贴模板色系而对图片直接加滤镜。
5. 互动页也必须完成模板化，但只允许替换 `page-shared`、`page-root`、`title-block`、`component-shell`、`button-skin`、`feedback-layer`；**禁止**重排 `interactive-root` 内部 DOM、禁止改壳脚本、禁止改页数和 `data-id`。
6. 互动页在替换按钮、选项按钮、切换按钮时，必须保留原有事件绑定关系；禁止修改 `onclick`、事件监听目标、函数名、节点 `id`、关键 `class` 与脚本查询选择器，禁止出现“按钮调用的函数名与脚本定义不一致”的情况。
7. 模板替换完成后，不在本阶段发布；应继续进入 10.5，仅对当前最新成品调用一次 `publish_resource`。
8. 若 `selectedTemplateId = default-base`，则跳过本节，直接进入 10.5。

### 10.5 最终交付

- **只发布一次最终版**：使用**最近一次 `edit_file` 返回的最新 resourceId** 单独调用 `publish_resource`，向用户发布最终版互动课件。
- **最终交付说明**：向用户说明「直接浏览器打开即可，无需其他文件或服务器」。
- **禁止双重发布**：禁止先发布基础稿、再发布模板稿；整个流程只允许一次面向用户的发布动作。
...
- [ ] 若用户选择了模板，全部页面都已完成模板替换，不存在“只换了部分页面”的情况
...
- [ ] 基础稿仅在后台留存，未提前对用户调用 `publish_resource`
- [ ] 模板选择已在 Phase 1 的信息确认阶段完成，流程中未额外 ask_user 追问版面满意度
- [ ] 最终交付仅使用最新 resourceId 单独调用了一次 `publish_resource`
...
| 删除、伪造或复用 `data-cw-role` 语义钩子 | 会导致发布前模板套版误判结构边界，误伤互动或内容布局。 |
