...
| 文件 | 用途 | 是否必读 |
|------|------|----------|
| `templates/<选定模板>/template.md` | 风格说明书 | ✅ 必读 |
| `templates/<选定模板>/tokens.json` | 颜色/字体/间距 token | ✅ 必读 |
| `templates/<选定模板>/shared.css` | 待装入 `<template class="page-shared">` 的样式 | ✅ 必读 |
| `templates/<选定模板>/layouts/*.html` | 版式骨架库（复制时保留 exercise 等页的 `<script>`） | ✅ 必读 |
...
- [ ] 最终 HTML 不含任何 `{{P` 或 `{{` 占位符残留
- [ ] 最终 HTML 不含 `metis-lectio`
- [ ] 每个 `<template class="page-data">` 都含 `data-id` 和 `data-name`
...
<template class="page-shared">
  ...
  <!-- 🚨 如果 Phase 2.5 选了模板（红线 2）：
       必须把 templates/<选定模板>/shared.css 的全部内容粘贴在 <style> 标签里。
       不做这一步整份课件视觉不会变样！-->
  <style>
...
<!-- 🚨 如果 Phase 2.5 选了模板（红线 2）：
     每个 <template class="page-data"> 块必须按"templates/<选定模板>/layouts/" 下
     对应版式的骨架写：
       - 第 1 页（封面）→ 用 layouts/cover.html
       - 情境导入 / 古题 / 生活实例 → 用 layouts/intro.html
       - 概念讲解 / 要点并列 → 用 layouts/concept.html
       - 公式呈现 / 推导过程 → 用 layouts/formula.html
       - 例题讲解 / 解题步骤 → 用 layouts/example.html
       - 选择题练习 → 用 layouts/exercise.html（保留完整 <script>）
       - 方法对比 / 概念辨析 → 用 layouts/compare.html
       - 末页 → 用 layouts/summary.html
...
<template class="page-shared">
  <!-- 🚨 如 Phase 2.5 选了模板：必须把 templates/<选定模板>/shared.css 的全部内容
...
- **【若选了模板】**：**Phase 3 必须**从 `templates/<选定模板>/layouts/<对应版式>.html` 复制 `<template class="page-data">` 块作为该页空框架起点，固定 `data-id` / `data-name`，并把通用占位符改为页码前缀占位符（如 `{{P3_TITLE}}`）。**Phase 4 只替换这些页码前缀占位符**。禁止自行决定 `<style>` 中的配色、字体、布局——这些必须由模板决定。
...
edit_file:
  ...
  <!-- 选了模板：复制 templates/<选定模板>/layouts/cover.html 的 <template> 块内容到此 -->
  <!-- 没选模板：自由编写，遵守第四节规则 -->
</template>
