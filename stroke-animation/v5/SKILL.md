...
const handle = StrokeTrace.mount({
  ...
  // ★ v1.6 / v1.7：整字写完后的"作品感" + 圈点系统
  artwork: {
    enabled: true,             // 衬底淡出，保留田字格 + 学生笔迹
    message: '🎉 这是你写出来的字！',
    // ★ v1.7 圈点系统
    praiseCircles: true,       // 3 星笔画在作品态用红色手绘圈圈出来表扬
    maxPraiseCircles: 3,       // 最多圈几笔（避免画面太乱，按 final 分降序选）
...
**v1.7 圈点系统 = "老师批改习字本"反馈**：

- **红圈**只圈学生最终笔迹中**通过且 3 星**的部分；不会触碰已淡出的标准衬底
- **数量上限**：`maxPraiseCircles`（默认 3），3 星笔画很多时按 `final` 分降序选
- **重写鼓励**：仅针对"曾经失败 ≥1 次后通过"的笔（`_strokeRetries[i] > 0`）；当 ≥4 笔重写时，前 2 笔详细 + 第 3 行聚合为"还有 N 笔也成功改正了"
- **状态隔离**：`reset()` / `setChar()` / 点击「再写一次」时全部清空（红圈 SVG 节点 + 重写计数 + 鼓励文案）

`artwork.enabled: false` 可一次性关掉作品感 + 圈点系统，回退到旧的"完成提示 + 重新开始按钮"。`praiseCircles: false` / `retryPraise: false` 可单独关其中一项（教研老师可以选），核心错笔重写逻辑不受影响。
...
| v1.6.1 | 32 笔画提示语全面切换为硬笔小学语文版（用"从左往右写，基本写平"替代"侧锋顿入"等毛笔术语） |
| **v1.7** | **★ 圈点系统：作品态 3 星笔画红色手绘圈表扬（仅圈学生笔迹，不圈已淡出的衬底）+ 重写笔画"改正成功"鼓励文案（不再显示旧错误轨迹）+ 严格的 reset/setChar 状态清理 + 15 个新单测** |
