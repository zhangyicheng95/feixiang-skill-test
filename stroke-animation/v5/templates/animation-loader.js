/*!
 * 飞象老师 · animation-loader.js (StrokeTrace) v1.7.0
 * Built: 2026-05-06
 *
 * 用途：在飞象老师 AI 互动课件中提供田字格跟练能力。
 * 一行 <script> 引入即可使用，无外部依赖（自带 UI、SVG、Canvas、DTW 评分）。
 *
 * 最小用法：
 *   <div id="trace"></div>
 *   <script src=".../stroke-order/v57/templates/stroke-loader.js"></script>
 *   <script src=".../stroke-animation/v1/templates/animation-loader.js"></script>
 *   <script>
 ...
 *   - 笔画名权威：飞象老师 stroke-order v57（2842 字教研审核 · data v56-2026-04-15）
 *
 * 严禁（违反将污染数据源）：
...
const _DataLoader = (function () {
  const DEFAULT_PATH_URL = 'https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-animation/v1/assets/stroke-path.json';
  const DEFAULT_NAMES_URL = 'https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v57/assets/stroke-data.json';
  ...
  /**
   * 返回字的 tier：
   *   - 'textbook' : 在 path 库内（2842 字，几何 + 笔画名都有）
   *   - 'extended' : 不在 path 库但在 names 库（4976 小学范围外字，仅有笔画名）
   *   - null       : 不在任何库
...
const _StrokeTrace = (function () {
   ...
   * 原则：不用毛笔书法术语（顿、出锋、回锋、铺毫、侧锋等），重点提示
   *       方向、长短、斜正、折角和易错点。
  ...
  function _injectStyles() {
    ...
    const css = [
      ...
      // ★ v1.7 圈点系统：作品态对三星笔画画红色手绘圈（仅在 .st-finished 时可见）
      '.st-trace .st-pad svg .st-circles { pointer-events: none; }',
      '.st-trace .st-pad svg .st-circles .st-circle { fill: none; stroke: #E74C3C; stroke-width: 6; stroke-linecap: round; stroke-linejoin: round; stroke-dasharray: 9 5 16 6; opacity: 0; }',
      '.st-trace.st-finished .st-pad svg .st-circles .st-circle { animation: st-circle-pop 0.7s cubic-bezier(0.34,1.56,0.64,1) forwards; }',
      '@keyframes st-circle-pop { 0% { opacity: 0; stroke-dashoffset: 120; } 60% { opacity: 1; } 100% { opacity: 0.92; stroke-dashoffset: 0; } }',
      // ★ v1.7 重写笔画"改正成功"鼓励文案（白底贴纸样式，叠在 st-finish 文案下方）
      '.st-trace .st-msg.st-finish .st-retry-praise { display: flex; flex-wrap: wrap; gap: 6px; justify-content: center; margin-top: 10px; padding-top: 8px; border-top: 1px dashed rgba(180,120,60,0.3); }',
  ...
  function TraceInstance(opts) {
    this.opts = Object.assign({
      ...
      hints: {
        ...
        showStrokeTip: true                 // v1.4 新增：在 hint 下方显示笔画口诀
      },
      // v1.4 新增：写前示范（Watch Mode）
      // 一支虚拟毛笔沿标准 median 滑过，速度按"起笔慢-行笔快-收笔顿"自然曲线，
      ...
      artwork: {
        ...
        maxPraiseCircles: 3,        // ★ v1.7：最多圈几笔（避免画面太乱）
  ...
  TraceInstance.prototype._loadAndStart = function () {
    ...
    dataLoader.loadAll().then(function () {
      ...
      if (!data) {
        self._setMsg('「' + self.opts.char + '」不在字库内（仅支持 2,842 教材字）', 'err');
  ...
  TraceInstance.prototype._enterArtworkMode = function (agg) {
    ...
    this._setMsg('<b>' + msg + '</b><br>「' + this.opts.char + '」综合 <b>'
      + score + '</b> 分，' + tail + retryHtml, 'ok st-finish');

    // ★ v1.7：圈点系统 — 红色手绘圈圈出三星笔画的学生笔迹
    this._drawPraiseCircles();
  };
  ...
  TraceInstance.prototype._exitArtworkMode = function () {
    ...
    // ★ v1.7：清空圈点系统（红圈与点评文案随消息重写自然消失，但 SVG 节点要主动清）
  ...
  /**
   * 生成"手绘风"椭圆 path：36 段折线 + 抖动 + stroke-linejoin=round
   * 比纯 <ellipse> 更接近老师批改时画的不规则圈。
   */
  TraceInstance.prototype._wigglyEllipsePath = function (cx, cy, rx, ry, jitter) {
    const segs = 36;
    const pts = [];
    for (let i = 0; i <= segs; i++) {
      const t = (i / segs) * Math.PI * 2;
      const wx = (Math.random() - 0.5) * jitter;
      const wy = (Math.random() - 0.5) * jitter;
      pts.push([cx + Math.cos(t) * rx + wx, cy + Math.sin(t) * ry + wy]);
    }
  ...
  TraceInstance.prototype._drawPraiseCircles = function () {
    ...
    for (let k = 0; k < picks.length; k++) {
      const c = this._strokeBoundingCircle(picks[k].result.userPts);
      if (!c) continue;
      // 包围圈再扩大一些，让圈不挤到笔迹（笔触本身宽 22px）
      const padX = 28, padY = 22;
      const rx = c.r + padX;
      const ry = c.r + padY;
      const angle = (Math.random() - 0.5) * 12; // -6° ~ +6°
      const cx = c.cx + (Math.random() - 0.5) * 6;
      const cy = c.cy + (Math.random() - 0.5) * 6;
      const path = document.createElementNS(NS, 'path');
      path.setAttribute('d', this._wigglyEllipsePath(cx, cy, rx, ry, 6));
      path.setAttribute('class', 'st-circle');
      path.setAttribute('transform',
        'rotate(' + angle.toFixed(1) + ' ' + cx.toFixed(1) + ' ' + cy.toFixed(1) + ')');
      // 错落出现：每个圈延迟一点点
      path.setAttribute('style', 'animation-delay: ' + (k * 0.18).toFixed(2) + 's');
  ...
  /**
   * 写前示范：虚拟毛笔沿标准 median 滑过，速度按"起笔慢-行笔快-收笔顿"自然曲线。
  ...
  /**
   * 速度曲线：起笔（前 15% 时间走 8% 长度）+ 行笔（70% 时间走 80%）+ 收笔（15% 时间走 12%）
   * 即起笔 0.53x, 行笔 1.14x, 收笔 0.8x，模拟真实毛笔节奏
   */
  ...
  TraceInstance.prototype._drawWatchFrame = function (now) {
    ...
    // 虚拟毛笔（金色发光圆点；起笔/收笔时半径放大表示"顿"）
    if (w.showBrush) {
      let r = 14;
      if (t < 0.12) r = 14 + (1 - t / 0.12) * 8;       // 起笔顿
      else if (t > 0.88) r = 14 + ((t - 0.88) / 0.12) * 8; // 收笔顿
      ctx.save();
      ctx.globalAlpha = opacity;
  ...
  // 金色虚拟毛笔沿 medians 滑过 + ✍️ 握笔 emoji 跟随 + 蓝色发光墨迹
  ...
  AnimateInstance.prototype._onStrokeFinished = function () {
    ...
    if (this.opts.onStrokeEnd) {
      this.opts.onStrokeEnd({ index: idx + 1, name: this._strokeNameAt(idx), total: this.charData.outlines.length });
    }
    const total = this.charData.outlines.length;
  ...
  const REASON_TEXT = {
    ...
    'not-in-textbook': '该汉字不在小学教材 2842 字内（仅小学范围外字库有笔画名，没有跟练几何数据）',
