/*!
 * 飞象老师 · animation-loader.js (StrokeTrace) v1.0.0
 * Built: 2026-04-27
 *
...
const _StrokeTrace = (function () {
  ...
  /**
   * 32 种标准笔画口诀（v1.4 新增）
   * 每条 8-14 字，朗朗上口，小学生听一遍就能记住。
   * 来源：飞象老师教研团队整理 + 字帖通用口诀
   */
  const STROKE_TIPS = {
    // 基本（6）
    '点':       '侧锋顿入，瞬时收笔',
  ...
  TraceInstance.prototype._onStrokeEnd = function (userPts) {
    ...
    // 红笔标注
    if (this.opts.scoring.showSegmentErrors && result.errors && result.errors.length && result.stars < 3) {
  ...
  TraceInstance.prototype._showFeedback = function (result) {
    if (result.stars === 3) {
      this._setMsg('完美！第 ' + result.index + ' 笔「<b>' + result.strokeName + '</b>」收笔到位。', 'ok');
    } else if (result.stars === 2) {
      this._setMsg('不错，再稳一些就是三星了。', 'ok');
    } else if (result.stars === 1) {
      const hint = (result.errors[0] && result.errors[0].hint) || '请按提示重写';
      this._setMsg('一笔基本到位，但<b>' + hint + '</b>，建议点「重写」再试。', 'err');
    } else {
      const hint = (result.errors[0] && result.errors[0].hint) || '差距较大';
      this._setMsg('差距较大：<b>' + hint + '</b>，点「重写」再来。', 'err');
    }
  };
  ...
  TraceInstance.prototype._finish = function () {
    ...
    this._updateProgress();
    this._setMsg('「<b>' + this.opts.char + '</b>」完成，综合 <b>' + Math.round(agg.final * 100)
      + '</b> 分。' + (agg.stars >= 2 ? '点「重新开始」再试一次。' : '再来一次会更好！'),
      agg.stars >= 2 ? 'ok' : 'err');
  ...
  TraceInstance.prototype.redoStroke = function () {
    ...
    this.tracer.clearCurrent();
    this._clearFx();              // 当前笔如果触发过完美闪光，重写时清掉
    this._resetStars();
    this._updateProgress();
    this._setMsg('请重新书写第 ' + (this.curStroke + 1) + ' 笔：<b>' + this._strokeName(this.curStroke) + '</b>', 'ok');
    this._dom.btnNext.disabled = true;
    // v1.4.1 失败累计：连续 N 次未达 ≥2 星 → 自动慢速重播示范
    // 默认 retryAfterFails=0（关闭，避免打断学生节奏）；老师场景可设 2
    if (lastStars < 2) this._consecutiveFails++;
  ...
  TraceInstance.prototype.nextStroke = function () {
    ...
    this.curStroke++;
    this._consecutiveFails = 0;        // v1.4 进入新一笔，连续失败归零
    this._refreshHint();
    ...
    this._dom.btnNext.disabled = true;
    // v1.4.1 默认不自动播；仅 watchMode.autoPlay === true 时才在切笔时自动播
  ...
  TraceInstance.prototype.reset = function () {
    ...
    // v1.4.1 重新开始也按 autoPlay 设置决定（first/true 才播；默认不播）
  ...
  TraceInstance.prototype.destroy = function () {
    ...
    this.container.innerHTML = '';
    this.container.classList.remove('st-trace');
  };
...
if (typeof window !== 'undefined') {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () {
      if (window._ST && window._ST.dataLoader) window._ST.dataLoader.loadAll();
    });
  } else {
    if (window._ST && window._ST.dataLoader) window._ST.dataLoader.loadAll();
  }
