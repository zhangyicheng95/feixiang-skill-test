...
**必须使用当前 skill 版本的附件 CDN。源码示例默认是测试基线 `stroke-animation/v9`；发布包会由 `publish.sh` 渲染为目标环境版本。严禁跨环境引用 CDN，也不要回退到同一环境的旧版 `animation-loader.js`。**

```html
<script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-order/v57/templates/stroke-loader.js"></script>
<script src="https://musk-test.fbcontent.cn/pub-musk-ai-studio/skills/stroke-animation/v10/templates/animation-loader.js"></script>
```
...
```bash
# 测试环境：按测试环境真实版本指定，当前测试基线可用 v9 / v57
