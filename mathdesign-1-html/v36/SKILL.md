...
description:【强制执行指令】必须基于“整体语义/图片”判断，当用户意图涉及数学相关知识点时，必须强制执行提供的色彩匹配机制，必须读取所有色彩方案，包含【A-色彩01-11】和【B-色彩01-20】，共31种色彩编号。匹配机制：针对属于小学阶段相关知识（1-6年级），必须随机调用【活力高饱和】色彩体系中【A-色彩01-11】种的一个色彩方案来进行html设计，以实现高吸引力的视觉效果；针对属于初中及高中阶段相关知识（7-12年级），必须随机调用【智性低饱和】色彩体系中【B-色彩01-20】的一个色彩方案来进行html设计，以确保视觉风格呈现理性克制且具备益智感，从而提升高年级的理解效率。
---
...
要求：
- 必须读取所有色彩方案，包含A-色彩01-11和B-色彩01-20 共31种色彩编号
- 禁止固定选择第一个方案
- 禁止偏向前几个方案

###A【活力高饱和】色彩体系，共11种适配于小学阶段知识点（1-6年级），重要：必须随机选择以下色彩。

####A-色彩01
- 背景色 (Background): #F3F3F3
- 所有字体颜色 (Text): #000000
- 主色调 (Primary): #FFDF5E
...
02:知识演示区域色彩（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
  01
background: #FFDF5E;
...
- 设计关键词 / 标签：
如果用户有建模需求，使用【极简、几何风格、干净、块面感、低细节】处理，如果没有则忽略。
标签：高对比配色 极简教育可视化 无阴影平涂 结构拆解教学

#### 色彩A-02
- 背景色 (Background): #E3E3E3
- 所有字体颜色 (Text): #2F0400
- 主色调 (Primary): #2F0400
...
border: 5px solid #CD8702;
  02
background: rgba(250, 155, 249, 0.90);
...
02:知识演示区域色彩（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
  01
background: rgba(251, 102, 140, 0.95);
border: 4px solid #FF466C;
  02
background: rgba(9, 122, 53, 0.95);
...
02:知识演示区域色彩（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
  01
background: rgba(106, 250, 119, 0.95);
...
01:演示区域背景：
背景色 (Background): #FFFFFF，border: 1px solid rgba(0, 0, 0, 0.30);
背景网格：
...
02:知识演示区域色彩（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
  01
background: rgba(255, 232, 181, 0.95);
border: 2px solid #9D1700;
  02
background: rgba(94, 126, 180, 0.95);
border: 2px solid #31415C;
  03
background: rgba(248, 152, 31, 0.95);
...
border: 2px solid #2782C9;

- 设计关键词 / 标签：
如果需要建模，使用【极简、几何风格、干净、块面感、低细节】处理，暖米教育风 扁平伪3D几何体 结构拆解交互课件 数学可视化UI 低干扰教学界面 过程化交互演示
B-
#### 色彩A-11
- 背景色 (Background): #E9D9BD
- 所有字体颜色 (Text): #251310
- 主色调 (Primary): #ED7D4B

- 卡片色彩/样式：
除了图示区域外。尽量减少在卡片里展示内容，直接在背景上进行排版展示。
...
### B【智性低饱和】色彩体系，适配于初中及高中阶段知识点（7-12年级），必须随机选择【智性低饱和】的色彩方案，避免分析知识点适合哪个颜色。

#### 色彩B-01（低饱和 单色）
- 背景色 (Background): #DCDCDC
- 背景网格 (Background): 
...
background: #000000;
border: 1px solid rgba(0, 0, 0, 0.40);
color: #FFFFFF（按钮上白色文字）
02:次要按钮/未选中按钮
background: #F1F1F1;
border: 1px solid rgba(0, 0, 0, 0.40);
color: #000000（按钮上黑色文字）

- 知识点演示色彩/样式：（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
主模型（几何体）
background: rgba(249, 250, 252, 0.95);
border: 2px solid #8B9094;

- 设计关键词 / 标签：
Low-Poly Wireframe Triangulated Mesh 线框拓扑 几何骨架 低多边形建模 科技感几何表现

#### 色彩B-02
- 背景色 (Background): #E2ECD3
- 所有字体颜色 (Text): #012B1F
- 主色调 (Primary): #405E28
...
- 知识点演示区色彩/样式：
01:演示区域背景：
背景色 (Background): #F7FDF6，border: 1px solid rgba(0, 0, 0, 0.30)
border-radius: 8px;
背景网格：
...
border: 2px solid #168D76;
  02
background: #549B22;
border: 2px solid #1C402A;
  03
background: #405E28;
border: 2px solid #001912;

- 设计关键词 / 标签：
如果需要建模，使用【极简、几何风格、干净、块面感、低细节】处理，暖米教育风 扁平伪3D几何体 结构拆解交互课件 数学可视化UI 低干扰教学界面 过程化交互演示

#### 色彩B-03
- 背景色 (Background): #E4DEBD
- 所有字体颜色 (Text): #3A1D09
- 主色调 (Primary): #3A1D09
...
- 设计关键词 / 标签：
暖棕米色调 低饱和度配色 极简几何 复古数学风 学术风 柔和无阴影 单色描边 高对比度文字
...
02:知识演示区域色彩（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
  01（如果是用于建模，作为定面色彩，如果不是建模就忽略）
background: rgba(36, 164, 116, 0.90);
...
- 设计关键词 / 标签：
如果用户有建模需求，使用【极简、几何风格、干净、块面感、低细节】处理，如果没有建模需求则忽略。
标签：高对比低饱和配色 极简教育可视化 无阴影平涂 结构拆解教学
...
02:知识演示区域色彩（3d/图形/图表/几何等演示必须要用到以下100%的颜色，重要：避免因光照不足而显得过暗或不清晰的问题。确保色彩还原更加鲜艳、真实。）
  01
background: rgba(53, 233, 254, 0.95);
border: 2px solid #137F8B;
  02
background: rgba(223, 144, 18, 0.95);
...
- 设计关键词 / 标签：
如果需要建模，使用【极简、几何风格、干净、块面感、低细节】处理，莫兰迪绿调教育风 扁平伪3D几何体 低干扰数学可视化 结构拆解交互课件 同色系极简UI 学术感矢量几何表现
