...
- 可视区域参考比例：16:7，优先一屏展示所有交互，尽量不要滑动。如果一屏幕展示不全，可内容溢出处理规则：当内容超过可视区域时，不要通过页面高度扩展解决内容溢出问题，建议通过以下方式解决：
   - Step Navigation（步骤切换）
   - Card Tab 切换
- 禁止 Emoji：全篇严禁出现任何表情符号，确保学术严谨感。禁止图片**：页面中严禁使用 `<img>` 标签或任何背景图片。
- 整体使用描边+填充的风格，UI卡片带有1px描边且描边颜色和字体色彩一致，描边透明度是40%，卡片统一圆角20px
- 演示图示部分必须通过色彩填充保持视觉冲击力。必须根据我的色彩要求设计页面
- 按照我的字号系统设计页面
- 按钮统一高度：80px
- 使用 grid 布局
