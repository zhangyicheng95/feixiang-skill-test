...
description:数学单页互动 HTML 视觉设计规范。生成或修改数学演示动画、几何互动、算理可视化、数学互动课件等单页 HTML 时启用；按学段自动选配色（小学1-6年级→活力高饱和，初中高中7-12年级→智性低饱和），统一字号、布局、按钮与安全区。触发词：数学动画、几何演示、互动课件、算理演示、勾股定理、分数可视化、坐标变换、图形变换、面积推导、方程演示、数学可视化、制作数学HTML。不适用于：多页PPT课件、小棒教具、立体几何3D、纯文字答疑无HTML、非数学学科。启用后仅随机读取匹配学段的1套色板，禁止全量加载31套。
---
