...
理解需求 → 追问收集指标（不清楚时用 continue_ask）
         ...
         → create 生成 HTML × N    收集页/报告页（URL 参数读 instanceId）
...
HTML 始终从 URL 参数读 `instanceId`，**不要硬编码**。

```javascript
// 读取 instanceId：必须读浏览器最外层 url 上的 instanceId
var instanceId;
try {
...
- **不要在 HTML 里硬编码假数据**：演示效果由 `mockSeeds` 在后端预置完成，前端代码不维护两份
