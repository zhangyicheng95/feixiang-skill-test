...
3. create 生成 HTML_1（收集页）（`role=collect`）：身份收集 + 业务交互 + 提交反馈，给参与者使用
     → 内部从 URL 参数读 instanceId，使用 new MuskCollect(instanceId) 读写
     
4. create 生成 HTML_2（报告页，可选）（`role=report`）：数据统计 + 明细列表 + 刷新按钮，给发起者使用
     → 同上
...
```html
<script src="https://www.feixianglaoshi.biz/fedebug/musk-workbench-web/feature/app-center/musk-collect/musk-collect.js?v=1.2.0"></script>
```
...
8. **生成完成后，必须向用户输出访问链接，每个链接带上 `instanceId`**
