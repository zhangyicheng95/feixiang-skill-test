---
name: data-collect
description: 当用户生成或改造的 HTML/课件包含较多填写、选择、答题等交互项时，先询问用户是否需要开启“数据回收”功能；**只有用户明确确认需要时，才调用本 skill**。用于在页面/课件内容已基本打磨完成后，将纯前端交互资源升级为带后端数据存取的收集页和报告页；不用于仅需本地交互、静态展示、单人体验，或用户尚未确认需要数据回收的场景。
---
...
```
1. ask_user 
     → 单选题1：必须先确认“当前动画/课件是否已打磨完成”，并提示“如果尚未完成，可能影响后续生成质量”，提供两个选项“继续打磨”，“确认”
        → 如果选择“继续打磨”，则中止当前流程，等待用户进一步输入
        → 如果选择“确认”，再次调用ask_user，询问以下问题
     → 多选题2：明确“是否需要收集学生身份信息”，提供选项：姓名、学号、班级、匿名等
     → 多选题3：明确“数据结构”，确认用户所需要的所有数据指标
     → 多选题4：明确“是否需要数据导出功能”，并确认用户需要的导出格式：csv、xls、json
     → 补充（可选）：其他需要确认的问题
...
7. 向用户展示访问链接（使用 instanceId）
```

## 联通性自检（bind_resources 之后、发送链接之前必做）

为防止"参与者填了数据但看板不更新"的常见 bug（根因是收集页与报告页的数据通道不一致），调用 `bind_resources` 成功后，必须按以下格式输出**数据通道一致性对照表**，否则禁止发送访问链接。

> 自检前需要回看自己写的代码，对比两端是否使用相同的数据集合名 / 数据字段。

### 数据集合一致性

| 数据集合（业务含义） | 收集页 | 报告页 | 是否一致 |
|---|---|---|---|
| 参与者提交 | ✓ 写入 | ✓ 读取 | ✓ |

> 多个数据集合时逐行列出。每个集合在两端必须使用完全相同的内部名称（大小写、复数严格一致）。
...
【硬约束】未输出此对照表 / 表格仍有 ✗ 时直接发送链接 = 违反 skill 规则。
