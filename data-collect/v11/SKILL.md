...
```
理解需求 → 追问收集指标（不清楚时用 ask_user）
         → create_instance 获得 instanceId
         → create_file 生成 HTML × N（内嵌 mock 数据，从 URL 参数读取 instanceId）
...
2. create_file 生成 HTML_1             → 收集页（从 URL 参数读取 instanceId）
3. create_file 生成 HTML_2（可选）      → 报告页（同上）
...
```javascript
// mock 数据：仅在预览模式下使用，真实模式下禁止混入
var MOCK_DATA = [
  {docId: "mock_1", data: {name: "张三", score: 85}},
  {docId: "mock_2", data: {name: "李四", score: 92}},
  {docId: "mock_3", data: {name: "王五", score: 78}}
];

// 读取 instanceId：必须使用这种方法，必须读取浏览器上的url上的instanceId
var instanceId;
try {
  instanceId = new URLSearchParams(window.top.location.search).get('instanceId');
} catch(e) {
  instanceId = new URLSearchParams(location.search).get('instanceId');
}

if (instanceId) {
  // 真实模式：只使用后端数据，即使返回空列表也不能用 MOCK_DATA 填充
  var sdk = new MuskCollect(instanceId);
  sdk.query('submissions').then(function(list) { render(list); });
} else {
  // 预览模式（无 instanceId）：展示 mock 数据供预览排版效果
  render(MOCK_DATA);
}
```
