...
4. create_file 生成 HTML_2（报告页）（`role=report`）：数据统计 + 明细列表 + 刷新按钮，给发起者使用
     → 同上
...
7. **发送链接前必须输出"联通性自检对照表"**（见上文「联通性自检」章节），全部 ✓ 才能发链接；任一行 ✗ 必须先用 `edit_file` 修复后重做自检
8. **create_file生成完成后，必须向用户输出HTML_1（收集页）和HTML_2（报告页）文件**
