...
4. create_file 生成 HTML_2（报告页，可选）（`role=report`）：数据统计 + 明细列表 + 刷新按钮 + 每条数据的删除按钮，给发起者使用
     → 同上
...
```html
<script src="https://www.feixianglaoshi.biz/musk-collect/musk-collect.js?v=1.4.0"></script>
```
...
| `sdk.delete(collection, docId)` | `collection`: String, `docId`: String | `Promise<Boolean>` | 硬删单条记录，删除后 query 不可见。docId 从 query 返回的 CollectData.docId 获取 |
...
> 删除必须使用同一个 `collection` 和 `query(collection)` 返回的 `item.docId`：`await sdk.delete(collection, item.docId)`；禁止使用 `item.id`，删除失败必须提示失败并保留数据，不能吞掉异常后提示成功。
...
4. **报告页明细默认提供单条删除按钮**：每行按钮只删除本行记录，调用 `sdk.delete(collection, item.docId)`，成功后重新 `query(collection)` 刷新列表
5. **instanceId 不硬编码在 HTML 中**，由 `new MuskCollect()` / `MuskCollect.getInstanceId()` 自动解析
6. **不在 HTML 里硬编码假数据**：始终展示参与者真实数据，没有数据时显示空状态
7. **生成收集页前必须先判断原始 HTML 是否命中「高风险页面结构的特殊分支」**；命中时必须走该分支，不能直接原地改造
8. **答题/测验类收集页默认不即时批改**：单题作答后只记录选择，不显示正确/错误、正确答案、解析或实时得分；最终提交后再展示分数/结果。除非用户明确要求即时反馈；此规则同样适用于上传 HTML / 已有互动页改造：必须检查并移除或禁用原页面中的单题级反馈逻辑（正确/错误提示、正确答案、解析、实时得分、选项红绿高亮等），只保留选择状态。
9. **联通性自检对照表必须在 `publish_resource` 之前输出**（见上文「联通性自检」章节）：全部 ✓ 才能进入 `publish_resource` → `bind_resources` → 发链接；任一行 ✗ 必须先用 `edit_file` 修复后重做自检
10. **`edit_file` 每次返回新的 resourceId**（旧 resource 作为历史保留）：自检失败修复后，后续 `publish_resource` / `bind_resources` **必须**使用 `edit_file` 这一轮 observation 返回的**最新** resourceId，绝不能沿用最初 `create_file` 的旧 id
11. **`publish_resource` 是用户可见性的唯一入口**：`bind_resources` 只负责"页面 ↔ instance 数据回传通道"，**不能**让用户在前端看到成品卡片。所有最终交付页（收集页、报告页等）都必须各自调用一次 `publish_resource`，未 publish 的 resource 的 internal_url 绝不贴给用户
12. **`publish_resource` 绝不能与 `create_file` / `edit_file` 同轮 tool_calls 并行调用**：tool_calls 并行执行，publish 跑得早就拿不到本轮 create/edit 即将返回的 resourceId。必须 create/edit 那一轮完成、observation 里看到 resourceId 之后，**下一轮**再单独发 `publish_resource`
13. **生成完成后，必须向用户输出访问链接，每个链接必须带上 `instanceId`**；前置条件：所有最终交付 resource 都已 `publish_resource` 且已 `bind_resources`
...
- **报告页**（`role=report`）：数据统计 + 明细列表 + 刷新按钮 + 每条数据的删除按钮，给发起者使用
