...
- Added a narrow angle-degree typography rule: angle/tilt readouts should use `^\circ` or `{}^\circ`, not `\mathrm{^\circ}`, because the latter can render visibly as `circ`.
- Added source-scan and Python/Playwright self-check examples for angle contexts: after interaction, visual text should not contain an independent `circ` artifact.
- Added a fix-existing reminder to rewrite old spec comments such as `forbid=$...$` to `forbid=dollar-math-delimiter` when editing that area.
- Kept the rule scoped to angle notation only; ordinary units such as `kg`, `N`, `m/s`, `T`, and `C` still use `\mathrm{...}`, and no physics-model constraints were added.
