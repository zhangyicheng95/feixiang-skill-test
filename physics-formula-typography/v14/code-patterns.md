...
async function setAngleValue(el, value, symbol = '\\theta') {
  await setFormula(el, `${symbol} = ${value}^\\circ`);
}
...
await setAngleValue(document.querySelector('#theta-readout'), theta.toFixed(0));
```
...
### 2.2 角度单位不要写成 `\mathrm{^\circ}`

角度的度符号不是普通文本单位，不要放进 `\mathrm{...}`。`kg`、`N`、`m/s`、`T`、`C` 等普通单位仍使用 `\mathrm{...}`；这里只针对角度/倾角的 `^\circ`。
...
<div id="theta-readout">\(\theta = 30^\circ\)</div>
<div id="angle-readout">\(\alpha = 45\,{}^\circ\)</div>
```

```js
await setFormula(thetaReadout, `\\theta = ${theta.toFixed(0)}^\\circ`);
await setFormula(angleReadout, `\\alpha = ${alpha.toFixed(0)}\\,{}^\\circ`);
```
...
<div>\(\theta = 30\,\mathrm{^\circ}\)</div>
```

交互后如果视觉文本里出现独立的 `circ`，通常说明度符号写法错了。
...
function hasMutableDomInsideMathDelimiter(html) {
  const visibleHtml = stripScriptStyle(html);
  const mathSegments = visibleHtml.match(/\\\([\s\S]*?\\\)|\\\[[\s\S]*?\\\]/g) || [];
  return mathSegments.some((segment) => /<(span|output|input)\b/i.test(segment));
}

function hasBadDegreeUnitLatex(html) {
  return /\\mathrm\s*\{\s*\^\\circ\s*\}|\\mathrm\s*\{\s*\\circ\s*\}/.test(html);
}
...
function scanFormulaTypographySource(html) {
  ...
  return {
    ...
    hasMutableDomInsideMathDelimiter: hasMutableDomInsideMathDelimiter(html),
    hasBadDegreeUnitLatex: hasBadDegreeUnitLatex(html),
...
def assert_no_angle_circ_leak(page):
    # Only flags the independent MathJax artifact "circ" in angle contexts.
    visual_text = page.locator("body").inner_text()
    angle_context = any(token in visual_text for token in ["倾角", "角度", "θ", "𝜃", "theta"])
    if angle_context:
        assert re.search(r"\\bcirc\\b", visual_text, re.I) is None, "角度单位渲染成了可见 circ，请使用 ^\\circ 或 {}^\\circ，不要用 \\mathrm{^\\circ}"
...
assert_no_angle_circ_leak(page)
```
