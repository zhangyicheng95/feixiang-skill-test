...
function hasDollarMathDelimiter(html) {
  const sourceForDollarScan = stripJsTemplateInterpolations(html);
...
MathJax 会改写 DOM，因此测试必须区分“源码检查”和“渲染后视觉检查”：

```js
// Source check: 看原始 HTML/DOM 源码里是否使用了正确 LaTeX 写法和测试钩子。
const html = await page.content();
expect(hasDollarMathDelimiter(html)).toBe(false);
expect(html).toContain('data-physics-label="G"');
expect(await page.locator('[data-physics-label="G"]').first().innerHTML()).toContain('\\(G\\)');

// Visual check: 等待 MathJax 渲染后，看用户可见文本，不要求保留 \\( \\) 或 \\mathrm。
await page.evaluate(() => window.MathJax?.startup?.promise);
await expect(page.locator('[data-physics-label="G"]')).toBeVisible();
await expect(page.locator('label[for="mass-slider"]')).toContainText(/质量.*m.*单位.*kg/);
```
