...
Phase 3 ┃ 选择 1 套稳定色板
  ...
  **Step 4 — 只写入产物元数据**
  ...
  色板查表和兜底计算属于内部准备，不向用户播报，也不延迟文件写入。把最终的 `keyword`、`pool`、`source` 和 `palette_id` 写入 `artifact-spec.mathDesign`，并确保 CSS 变量与之相符。
...
Phase 5 ┃ 交付硬约束（缺一即需要修复后复测）
  ① **mathDesign 元数据（最高优先级）**
     - `<!DOCTYPE html>` 保持为文件开头
     - 在 `<head>` 的 `artifact-spec` JSON 内写入 `mathDesign.pool`、`palette`、`layout` 和 `keyword`
     - `palette` 与 Phase 3 实际 palette_id 一致，`layout` 与 §2.2 实际结果一致
     - **未写入或 JSON 不可解析 → 先修复，再进入验收**
  ② 字号/按钮：标题和主按钮在目标视口可读、可达且不挤压主舞台；按 §3 的响应范围实现
  ③ palette_id 与 Phase 3 输出一致（查表命中→精确匹配；hash 兜底→与算式结果一致）
  ④ Stage 主演示容器中心与 Stage 几何中心可视化对齐（§2.3）
  ⑤ 布局变体与互动原型、控件类型和主舞台重心匹配（§2.2）
  ⑥ **控件可达 / 不用滚动掩盖关键操作**：核心 controls、当前反馈和 Stage 主体应保持易见易操作；内容超屏时优先用 Step Navigation、Card Tab、Accordion 或重排，不用滚动隐藏关键控件
  ...
  ⑨ **无整图方格背景**：数学场景默认禁止在 `#stage`、`.stage`、`.stage-area`、`.plot-area`、`.chart-area`、`.svg-container`、`#plot-container`、Canvas/SVG 容器上用 CSS 背景或 SVG/JS 循环生成方格纸；出现 §1.1 任一命中项 → 必须删除后复测
...
- 内容超屏时：Step Navigation、Card Tab、Accordion 或重排；只有长说明/长表格等阅读型内容才考虑滚动，互动主体和关键控件不靠滚动隐藏
- 演示图示必须通过色彩填充保持视觉冲击力（遵循 visual-impact.md）
- 主按钮使用 `clamp()` 或响应断点保持 48–64px 最小高度；投屏视口可适当放大，但不能挤压主舞台
...
**选择规则：**
- 连续步骤、上一题/下一题、检查/重置等横向动作链优先 L1；
- 多个滑块、参数或对象选择需要边看边调时优先 L2；
- 演示主导、参数较少、解释或反馈需要贴近右侧时优先 L3；
- 若某种布局让视觉主角小于支持面板，或把核心操作/反馈推到首屏外，应改用更适合的变体并压缩支持内容。

选择结果只写入 `artifact-spec.mathDesign.layout`，不向用户输出布局计算过程。

**Controls 尺寸约束：**
- L1：通常高度 ≤ 128px，主按钮最小高度 48–64px，横向 flex
- L2 / L3：桌面宽度通常为 22%–30%，主按钮最小高度 48–64px，纵向 flex；窄屏重排到 Stage 下方
- 三种变体都要让主舞台占主体区域大部分注意力，并让主操作与当前反馈相邻

**Controls 内容硬约束（关键，防止控件不可达）：**
- `#controls` / `.controls` 不应变成需要长距离滚动才能操作的控件堆；如果控件数量过多，优先通过 Tabs、Accordion、分组、压缩文案或布局重排解决
- L2 / L3 单栏控件数 **≤ 5 块**（一个滑块组、一个按钮组、一个公式卡、一个数值显示 = 4 块）
- 控件多 → 用 Tabs / Accordion 内部折叠；不要让学生通过长滚动寻找关键控件
- 全局布局优先保证 Stage、controls 和反馈可达；内容真超出时先重排或改成交互分步
...
- 本地验收必须覆盖本节：读取页面内容并断言 Stage/plot/chart 容器的 `backgroundImage` 不含 `linear-gradient`/`repeating-linear-gradient`，源码不含 `grid-layer`/`coord-grid`/`initGrid`/`drawGrid`；任一失败必须删除后重测
...
不要让以上任何 `aside#controls` 变成需要长距离滚动才能操作的控件堆；超量控件优先用 Tabs/Accordion 替代。

## 三、字号系统（Typography）

- H1 = `clamp(28px, 3.2vw, 42px)` / font-weight:700–800
- H2 = `clamp(22px, 2.4vw, 30px)` / font-weight:600–700
- H3 = `clamp(18px, 2vw, 26px)` / font-weight:600
- Body = `clamp(16px, 1.6vw, 22px)` / font-weight:400–600
- Caption ≥ 14px，核心标注在常用视口下不低于 16px
- 按钮字号 = `clamp(16px, 1.7vw, 24px)` / font-weight:600–700

字号服务于可读性与一屏任务闭环。不得用超大标题、固定 80px 控件或大段说明挤压视觉主角；也不得用缩小到难读的文字换取表面无溢出。

## 四、与 html-authoring 通用规范的衔接

数学场景下，以下 `html-authoring/content-guide.md` 通用规则**仍适用**：
