...
> 入口：`SKILL.md`。下图展示 Read / Write / 内联的调用顺序与分支。
...
**生图闸门**：调用生图前必读 `references/cover.md`（封面）+ `references/image-generation.md`；工具不可用则自绘并声明。
...
flowchart TD
    ...
    W_HTML --> COPY[Read assets/courseware-shell.js 到结尾<br/>原样内联到最终 script]
    COPY --> SELF[对照 artifact-spec.outline 逐页自检<br/>references/html.md §八]
...
flowchart TD
    ...
    R_TEST --> R_GUIDE[按需 Read ../test-html/guide.md]
    R_GUIDE --> MUST[从 spec / core-loop 抽 must-cover]
    MUST --> STATIC[create_file 写入后回读<br/>结构 / 依赖 / artifact-spec / 内联壳]
...
**共用 Skill**：`../test-html/SKILL.md`（Step 3 验收）
...
flowchart LR
    ...
    subgraph step2 [Step 2 · Read + Write + 内联]
        ...
        SHELL[(assets/courseware-shell.js)]
        INDEX_W[(slug.html)]
    end

    subgraph step3 [Step 3 · Read + 浏览器]
        TEST[../test-html/SKILL.md]
        GUIDE[../test-html/guide.md]
    end
