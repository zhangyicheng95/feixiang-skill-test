...
> 路径：`courseware-generator/references/cover.md`  
> Step 2 生成第 1 页封面前必读。7 种固定坐标版式；字体变量见 `references/typography.md`。  
> 若第 1 页需要外链/生成封面图，在 **Step 1** 定 `coverImageSlot`（见 §三），再读 `references/image-generation.md` 做生图 prompt 增强。
...
- [ ] 封面生图前已读 `references/image-generation.md`；prompt 含槽位尺寸/比例；未在图内生成标题文字。
