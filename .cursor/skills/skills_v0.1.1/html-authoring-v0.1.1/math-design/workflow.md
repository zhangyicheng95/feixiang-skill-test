...
> 目录：`math-design/`  
> **仅当** `../html-authoring-v0.1.1/SKILL.md` 判定为数学场景时启用。
...
| [color-palettes-a.md](color-palettes-a.md) | `math-design/color-palettes-a.md` | Phase 3 选定 A 编号后，**仅读该编号段落** |
| [color-palettes-b.md](color-palettes-b.md) | `math-design/color-palettes-b.md` | Phase 3 选定 B 编号后，**仅读该编号段落** |
| [visual-impact.md](visual-impact.md) | `math-design/visual-impact.md` | Phase 4 编写演示区前 |
...
Phase 4 ┃ 按需读取（禁止全量加载 31 套）
  - **必须使用 `Read` 打开** `math-design/` 下对应文件
  - pool=A → 读取 `math-design/color-palettes-a.md` 中对应编号段落
  - pool=B → 读取 `math-design/color-palettes-b.md` 中对应编号段落
  - read visual-impact.md
  - 将选定色板的 hex **映射**为 CSS 变量：`--primary` `--secondary` `--accent` `--background` `--foreground`

Phase 5 ┃ 交付硬约束（缺一即需要修复后复测）
  ① **palette 注释（最高优先级）**
     - 写入或修改 HTML 后，HTML **第一行**（`<!DOCTYPE html>` 之前）必须是：
...
数学场景下，以下 `../content-guide.md` 通用规则**仍适用**：
