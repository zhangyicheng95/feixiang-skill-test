...
- **画布底色（必做）**：`page-shared` 声明 `--canvas-bg`（`--bg` 与 `#eef1f5` **各半混合约 50%**；`--bg` 近白时用 `--card`，见 [feixiang-style.md](../feixiang-style.md) §4.6）；`html, body, .page-container` 均使用 `background: var(--canvas-bg)`。禁止画布根层纯白——白色仅用于 `.card`、选项块等组件。
...
采用**骨架 + 按复杂度分批增量注入**模式：先创建包含注入标记的 HTML 骨架，根据大纲中各页的生成复杂度权重分批（每批权重预算 ≤ 20，总权重 ≤ 20 时一批生成全部页面），每批通过 `multi_edit` 将 `<template>` 标签注入到标记位置。
...
创建骨架（create）→ 生成 HTML 文件，含 <!-- CW_PAGES --> 注入标记 → 记录 resourceId_A
  ↓
批次1 → multi_edit(resourceId_A) 注入本批页面 → 得到新文件 resourceId_B
  ↓
（如有更多批次）批次2 → multi_edit(resourceId_B) → 得到 resourceId_C → ...
  ↓
验收 → 基于最终 resourceId 逐页核对 → 交付
```

> **⚠️ 分批的目的是保证生成质量，不是为了分批而分批。** 课件总权重 ≤ 20 时应一批生成全部页面。分批策略详见 content-guide.md 第 5.3 节。
>
...
| 总权重超过 20 时一次性生成全部页面 | 总权重 > 20 的课件须按装箱规则分批生成、逐批注入骨架文件。总权重 ≤ 20 时允许一批生成全部页面。 |
...
| 向用户暴露 resourceId 或文件 ID | resourceId 是内部操作标识，禁止在任何面向用户的输出中提及。进度汇报用自然表述（如「正在生成第 X 批页面」）。 |
| 用文件名或技术标识称呼课件 | 在整个工作流中提及课件文件时，统一使用 `<title>` 标签中的课件标题（如「《少年闰土》」），而非文件名或 resourceId。 |
