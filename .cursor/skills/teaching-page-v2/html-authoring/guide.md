...
> **teaching-page-v2 本地副本**。编排入口见 [SKILL.md](SKILL.md)；飞象 `create_file`/`read_url` 等 → Cursor `Write`/`Read`。
...
- 平面直角坐标系/函数坐标图默认**无完整方格纸背景**：只画坐标轴、刻度、标签、曲线/点位和必要语义辅助线；只有用户原文明确要求“方格线/网格线/坐标网格/坐标纸/方格纸”时才可画完整坐标方格，且方格/轴/刻度/标签/曲线必须共用同一 `unit/origin`。统计直方图、柱状图、条形图等图表内部参考线不受此条禁止
- 所有数学场景默认禁止整页/整图方格背景：不得在 `#stage/.stage/.stage-area/.plot-area/.chart-area/.svg-container/#plot-container` 上用 `background-image`、成对 `linear-gradient` + `background-size`、`repeating-linear-gradient`、SVG `<pattern>` 或 SVG/JS 横竖线循环生成方格纸；未明确要求网格时，源码不得出现 `grid-layer`、`coord-grid`、`gridLayer`、`initGrid()`、`drawGrid()`；统计图只允许绘图区内部少量水平参考线/分箱线，Stage 本身不得有背景方格
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；Phase 3 推理已声明 `keyword / pool / source(table|hash) / palette_id`；布局推理已声明 `layout_hash` 数字且与算式一致（禁止默认 L1）；Stage `display:flex; align-items:center; justify-content:center`；H1=40px；按钮 80px；用户未明确要求“方格线/网格线/坐标网格/坐标纸/方格纸”时，HTML 中不得存在整页/整图方格背景：`#stage/.stage/.stage-area/.plot-area/.chart-area/.svg-container/#plot-container` 的 CSS 背景网格、`background-image` 纹理、成对 `linear-gradient(...1px...)` + `background-size`、坐标背景 `repeating-linear-gradient`、SVG `<pattern>`、`grid-layer`、`coord-grid`、`gridLayer`、`initGrid()`、`drawGrid()` 或覆盖全宽全高的横竖线循环；坐标图只保留轴/短刻度/标签/曲线/必要语义辅助线；统计图只允许少量图表内部水平参考线/分箱线，Stage 本身不得有背景方格；`test_html` 必须加入无方格背景断言；**`#controls` 未设 `overflow-y:auto|scroll`**；`<body>` 仅允许 `overflow:hidden`
