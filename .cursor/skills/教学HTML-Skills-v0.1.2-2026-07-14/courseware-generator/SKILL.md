...
description: K12 单文件多页翻页课件 HTML 生成 skill。用于 PPT 式课件、翻页演示、缩略图预览、练习页状态保存、成绩消息、下载或 SCORM；沿用 spec、outline、page-data 和验收流程，把素材、大纲、页面和官方课件壳完整内联进一个 HTML，并通过 create_file 写入和回读验证。
metadata:
  version: v0.1.2
...
本 skill 负责生成“一个 HTML 文件承载多页课件”的翻页型产物。它不是单页互动页面生成器；如果用户要的是教学动画、小游戏、海报、单页练习或 A4 打印材料，应改用 `../../html-authoring/html-authoring-v0.1.2/SKILL.md`。多页课件的核心差异是：页面内容写在多个 `<template class="page-data">` 中，运行时由内联的官方课件壳提供缩略图、主预览、翻页、状态恢复和打包入口。
...
需求、素材、单文件写入和课件验收规则在本入口对应步骤中定义；动态验收细节由 `../../test-html/test-html-v0.1.2/SKILL.md` 定义。
...
Step 3 前必读：
- ../../test-html/test-html-v0.1.2/SKILL.md
```
...
读取 `../../test-html/test-html-v0.1.2/SKILL.md`。调用 `create_file` 前先检查完整 HTML：只有一组 doctype/html/head/body；包含 UTF-8 charset 和 title；style、script、字符串、括号闭合；`artifact-spec` 可解析；所有核心数据和资源已内联；DOM 查询与真实 id/class 一致；无模板占位、空事件函数、可见的 `undefined/null/NaN` 或本地运行依赖。
