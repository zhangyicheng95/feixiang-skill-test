...
> 入口 `html-authoring/SKILL.md`；飞象 `create_file`/`read_url` 等 → Cursor `Write`/`Read`。
...
|---|---|---|---|---|
| **数学 → math-design** | 命中数学知识点（见下）；且无步骤 1 的非数学学科前缀 | **必读** `html-authoring/math-design/workflow.md` → 色板 → `html-authoring/math-design/visual-impact.md` | 31 套 A/B 色板；H1=40px；按钮 80px | 禁止「参考配色方案」 |
| **非数学 → 通用** | 步骤 1 学科前缀；或步骤 3 默认 | **仅读** `html-authoring/SKILL.md` + `guide.md` | 「参考配色方案（5 个）」+ CSS 变量 | **禁止** read `math-design/`；**禁止** `html-authoring:math-design` 注释 |
...
| 文件 | 路径 |
|---|---|
| 工作流 + 布局/字号 | `html-authoring/math-design/workflow.md` |
| 小学色板 A-01~11 | `html-authoring/math-design/color-palettes-a.md` |
| 初高中色板 B-01~20 | `html-authoring/math-design/color-palettes-b.md` |
| 演示区多色强化 | `html-authoring/math-design/visual-impact.md` |
...
- **内容组织模式**（**先于布局决定**，见"内容组织模式"章节）：
  ...
  - 禁止直接把大量内容摊平成一条滚动长页
- **配色方案**（**按学科路由二选一，禁止混用**）：
  - **数学场景**：按 `html-authoring/math-design/workflow.md` Phase 2–4 机械抽选色板 → Read 对应色板文件 + `html-authoring/math-design/visual-impact.md`
...
**数学 — math-design 规则（摘要，完整见 `html-authoring/math-design/workflow.md`）：**
...
- 布局变体（**L1底栏 / L2左栏 / L3右栏**）按 `(ord(keyword[0]) + len(prompt)) mod 3` 选定，详见 `html-authoring/math-design/workflow.md` §2.2；**严禁未声明算式直接落 L1**
- Stage 必须 flex 居中（`align-items:center; justify-content:center`），演示主体居中渲染，详见 `html-authoring/math-design/workflow.md` §2.3
...
- **Controls 禁滚动**：`#controls` 不得设 `overflow-y:auto|scroll`；超量控件用 Tabs/Accordion 折叠（详见 `html-authoring/math-design/workflow.md` §2.2）
...
**注入位置：**
- 所有 `<script>` 直接放在 `<head>` 中（多页课件注入方式不同，由 `courseware-generator/SKILL.md` 管辖）
...
**Agent 推理：**
- 学科路由：数学 → Read `html-authoring/math-design/workflow.md`
- 学段：初中 → pool B
