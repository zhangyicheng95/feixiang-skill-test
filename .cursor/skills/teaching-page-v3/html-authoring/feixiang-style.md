...
| 4 | **统一圆角（飞象最新）** | 飞象最新版产物**一律圆角**，全课件统一 `--radius:16px`（可 12–20px，但同一课件只用一个值）。**禁止 0 / 直角。** |
...
--radius:16px; 演示多色:#35E9FE / #DF9012 / #00404D
```
...
--radius:16px; 演示多色:#83FE91 / #278DEA / #232323
```
...
--radius:16px; 演示多色:#B5FF8B / #549B22 / #405E28
```
...
--radius:16px; 演示多色:#F8981F / #5E7EB4 / #3D3334
```
...
--radius:16px; 演示多色:#06B17A / #B6FCE5 / #EEFFEB
```
...
--radius:16px; 演示多色:#F672AD / #F8EF50 / #03DEDE / #6AFA77
```
...
**预览壳**（多页课件）：`courseware-generator/assets/courseware-shell.js`
...
- ❌ 直角（`radius:0`）或圆角忽大忽小 → ✅ 全课件统一 `--radius:16px`（飞象最新圆角）
...
```html
<!-- fx-style: palette=FX-青(B-08); radius=16; keyword=酸碱中和; source=hash -->
```
