...
1. **禁止手写壳代码**（缩略图、翻页、演示模式等由 `courseware-shell.js` 提供）。
2. **交付物**：`pages/<slug>/index.html` + 同目录 `courseware-shell.js`。
...
- `<script src="./courseware-shell.js">` 放在**所有 template 之后**；从 `assets/courseware-shell.js` 复制到产物目录。
...
3. Read outline.md + assets-manifest.md
4. Write 完整 index.html（所有 page-data 一次到位）
5. 复制 assets/courseware-shell.js → pages/<slug>/courseware-shell.js
6. 对照 outline 逐页验收
```

**页数较多时**：可先 Write 骨架（`page-shared` + 前几页），再追加剩余 `<template>`；**禁止**依赖外部编辑器的 `multi_edit`/`resourceId` 注入机制。
