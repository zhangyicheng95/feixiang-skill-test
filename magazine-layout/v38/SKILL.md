...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级试卷、讲义、练习单、题单、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、好看、美化、可打印、A4、一比一、还原、复原、真实原图、原卷图、按这个格式来。上传试卷的目标是“真实内容一比一保真 + 设计师级美化重排”：真实原图保真是底线，美化排版是核心交付，二者都必须满足。v38 输入判定：只有“题号级结构 + 可编辑正文 + 公式/表格 + 真实逐页图或题内视觉图 URL/bbox + 图片文字 OCR 可编辑还原 + A4 紧凑分页策略”齐全时才生成 HTML；PDF/截图/扫描卷/DOC/DOCX/PPTX 若存在必须保留的视觉图片对象、用户要求读图/带图，或转换结果提示 figure/image/media，但工具没有返回真实视觉图片 URL，必须禁止 create_file 并返回缺少图片提取资源的阻塞说明。纯文字截图可以转为可编辑正文，但必须逐条还原到语义位置并纳入验收。禁止 AI 生成图、SVG/Canvas/CSS 猜画、文字占位、少量 OCR 文本或大量空白分页冒充完成。
---
...
| DOC/DOCX/PPTX/XLSX 等 Office 文档，转换结果或用户要求显示存在图片、图示、截图、媒体、表格图片，且工具返回了真实视觉图片 URL 或可访问裁剪图；纯文字截图已 OCR 成可编辑文字 | 可以 | 生成可编辑重排正文，把真实视觉图片放回语义附近，把图片文字放回原语义位置，并使用紧凑 A4 分页 |
...
| 已知 Office 文档含图片对象，其中部分是纯文字截图，且文字已完整 OCR 成可编辑正文 | 可以 | 纯文字截图不强制保留为 `<img>`；但必须在内容清单里标明“已 OCR 转写”，并在最终 HTML 中出现该文本 |
...
阻塞时只允许回复阻塞说明或调用 `terminate`，不能继续生成“看似完成”的 HTML。
...
- 已知源文档中存在纯文字截图，但最终 HTML 既没有真实图片，也没有把截图文字 OCR 成可编辑文本。
...
10. 用户提到“还原/复原/真实原图/一比一/原卷图/不要随便生成/不要 AI 生成”时，必须进入“一比一原图复原模式”：禁止调用 `generate_image`、`picture_gen`、AI 插画、SVG/Canvas 猜画来替代原图；只能使用真实逐页图、题内裁剪图、截图 URL、原始上传图片或后端 bbox 裁剪结果。
11. 数学、物理、化学、生物、地理等依赖图形准确性的试卷，默认也按“一比一原图复原模式”处理。只有后端明确返回完整 `diagramSpec` / `redrawData` 且用户允许重绘时，才可 SVG 重绘；否则必须真实图片优先。
12. 对 PDF/截图/扫描卷，若没有工具返回的真实图片 URL，必须在生成文件前停止，回复“当前缺少 PDF 转图/图片提取能力，无法完成原图保真排版”；禁止调用 `create_file` 生成伪 HTML。
13. 未满足以上任一项时，必须重新生成或说明当前链路缺能力，禁止假装完成。
...
- 图片里的文字、表格、填空题应优先转成可编辑 HTML 段落或 `<table>`；纯文字截图转写成功后不再强制作为 `<img>` 计数，图片只作为校验或识别失败兜底。
...
- 对 Office 文档，如果已知源文件包含 N 张必须视觉保真的图片，最终 HTML 必须包含不少于 N 个真实 `<img>`；如果其中部分是纯文字截图，则必须 OCR 成可编辑文本并在验收中检查文本片段，不能只放 1 张图通过验收。
...
3. `@media print` 打印样式
4. `.no-print` 打印隐藏
5. `.question, table, figure, img, .keep-together { break-inside: avoid; }`；禁止给 `.section`、`.main`、`.card`、`.page` 这类大容器统一 `break-inside: avoid`
...
12. 禁止给 `.point-card`、`.question-item` 等大量重复小卡片默认 `break-inside: avoid`；只对真实图片、表格、关键题组使用 `break-inside: avoid`
