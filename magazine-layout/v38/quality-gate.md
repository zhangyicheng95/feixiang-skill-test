...
- 已知源文档含纯文字截图时，最终 HTML 既没有真实图片，也没有把截图文字 OCR 成可编辑文本。
...
- A4 产物页面密度明显不足：题块/环节卡片间距过大、页边距过宽、大容器强制 `break-inside: avoid` 导致分页留下大块空白。
- 产物满足真实图片保真，但没有可编辑重排正文、题型结构、标准答题区和高级纸面设计，把一比一复原误做成“原卷图片合集”。
- 产物视觉好看，但题量、公式、题内图或原卷图不准；美化不能覆盖或替代一比一保真。
- MinerU/OCR 已识别出图片文字或表格，但最终 HTML 仍只贴截图，没有转成可编辑段落或 `<table>`。
...
- `--min-questions 30`
- `--min-images 1`
- 带图 DOC/DOCX/PPTX/XLSX：如果已知源文档视觉图片数，必须追加 `--expected-images <源文档视觉图片数>`；纯文字截图必须追加 `--require-text-snippet` 检查 OCR 文本，不能只用 `--min-images 1` 放行。
- `--require-real-images`
- `--require-compact-a4`
- `--require-mathjax`
...
9. 图片中的文字、表格是否已优先转成可编辑 HTML；截图是否只作为校验或识别失败兜底。
...
14. 原卷校验附录是否只在正文之后出现。
15. A4 信息密度是否合格：页边距、字号、行距、题块间距紧凑；大容器没有 `break-inside: avoid`；没有每页只排很少内容的大面积空白。
16. 已知源文档视觉图片数量时，最终 `<img>` 数量是否达到视觉图片数；只读到 1 张图不能算“图都读了”。
17. 已知源文档含纯文字截图时，截图文字是否已转成可编辑正文，并用 `--require-text-snippet` 验证。
