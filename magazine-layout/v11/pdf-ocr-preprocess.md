...
1. 用 OCR/文本转换拿到可编辑文字。
2. 同时保留原图或原页面截图作为视觉保真层。
3. 再基于可确认的文字做重排。

OCR 是辅助提取，不是替代原卷。

重要边界：OCRmyPDF 只负责增加 OCR 文本层，不能把 PDF 页自动变成 HTML 图片。PDF 原图保真还需要“PDF 转图片”或“HTML 嵌入原 PDF”。
...
建立题量清单 + 图片清单
  ↓
真实原图保真层（img/object/iframe） + 可确认文字重排层
  ↓
Paged.js A4 HTML
```
...
3. HTML 中必须放入上传资源的原图/原页截图；如果没有逐页图片，至少嵌入原 PDF URL。
4. OCR 文本只用于辅助重排。
...
2. 生成 `output.ocr.pdf`。
3. 同步用 `pdftoppm` / `pdftocairo` / Ghostscript 将 PDF 每页转为 PNG。
...
- 禁止用“占位说明”代替原图；没有真实资源标签就不能交付。

## 验收

- [ ] PDF 已有可读文本层，或已通过 OCR/文本转换提取内容。
- [ ] HTML 保留原图/原页截图。
- [ ] HTML 使用真实 `<img>`、`<object>`、`<embed>` 或 `<iframe>` 保留原卷。
- [ ] HTML 中没有“此处保留”“图略”“占位”等伪保真文字。
- [ ] OCR 文本与原图题量一致。
