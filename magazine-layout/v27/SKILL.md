...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级讲义、练习单、试卷、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、最好看、准确还原、带图、可打印、A4、讲义、题单、模板复用、按这个格式来。本技能不搜题；内容已给定时先完整复现，再基于 Paged.js 0.4.3 做设计师级重排。生成前必须建立纸面设计系统 token。PDF/截图/扫描卷必须 OCR/文本转换并产出可编辑重排正文；图形题优先使用真实题内图 URL、裁剪图或 bbox。数学/理科试卷禁止凭题干文字或普通 OCR 文本用 SVG 猜画，只有拿到完整 `diagramSpec` / `redrawData` 时才允许 SVG/HTML 重绘；否则必须阻塞并说明缺少题内图片裁剪/定位结果。英语/语文阅读材料、封面或非关键语境图缺失时，应调用可用的 `generate_image`（旧名 `picture_gen`）生成统一风格 AI 辅助配图并插入，标注“AI配图”，不能直接交付无图版。禁止同时调用 mathdesign-1-html、html-authoring、page-optimize、mathdesign-* 等其他排版/HTML Skill；禁止空白 PDF 框、整页贴图冒充排版、编造图片 URL、占位、“参考原卷”式偷懒、漂亮但不准确的 SVG 重绘图。
---
...
9. 试卷图形默认不重绘。只有在后端/OCR 明确返回 `diagramSpec`、`redrawData` 或等价结构化字段，且字段包含全部几何/统计/坐标/标签/比例信息时，才允许 SVG/HTML 重画。`read` / `read_file` 读出的题干文字、OCR 普通文本、“如下图”“从图中可知”等描述不算结构化图形数据；否则必须使用真实题内图裁剪、原页局部图或后端返回的图片 URL。
10. `generate_image`（旧工具名 `picture_gen`）用于非关键阅读配图、封面氛围图或装饰插画，并必须标注为“AI配图/辅助插图”；不能把 AI 生成图计为“原卷图还原”，不能用于数学几何图、统计图、物理电路图、生物结构图、地图、坐标图等影响答题准确性的图。
11. 语言类阅读/完形/写作材料中，如果原文出现图片占位、阅读语境明显需要配图，或用户明确说“带图/好看”，且没有真实原图资源，必须先调用 `generate_image` 生成 1-4 张统一风格辅助插图，再调用 `create_file`；禁止直接生成 `img=0` 的无图版。
12. 对 PDF/截图/扫描卷/由试卷转出的 docx，若没有 OCR/文本转换出的完整题目结构，或图形依赖题没有真实题内图/裁剪 bbox/完整 `diagramSpec`，必须在生成文件前停止，回复“当前缺少 OCR 文本/题目结构化结果或题内图片裁剪结果，无法完成真正重排”；禁止用整页图片、局部识别结果或漂亮但不准确的 SVG 重绘图冒充精美排版。
...
- 高级重排：正文必须体现清晰题型层级、统一网格、克制配色、精准对齐和 A4 纸面信息密度。必须先建立纸面设计系统（颜色、字体、间距、线条、题号、图形卡片规则），再写 HTML。
- 真实图优先：图形题、统计图题、几何题准确性高于美观。优先使用后端/OCR 返回的题内图 URL 或 bbox 裁剪；没有可靠数据时不凭感觉重绘。
- 普通文本不是画图依据：从 `read_file` 看到“如下图”“看图”“根据统计图”时，只能判定“需要图”，不能据此推断图长什么样。除非上下文提供了完整 `diagramSpec/redrawData`，否则必须阻塞或等待真实图。
- AI 配图边界：英语阅读、语文素材、封面氛围可以用 `generate_image` 生成辅助插画提升观感，但它只是“配图”，不是“原图还原”。凡是影响解题数据、标签、比例关系的图，禁止用 AI 生成图替代。
...
Phase 1.5: 保真专项检查
  ...
  对每道图形依赖题建立 figure 清单：题号、页码、真实图 URL、bbox 或完整 diagramSpec/redrawData
  对语言类阅读/语文材料建立 illustration 清单：哪些位置可用 AI 辅助配图、生成提示词与统一风格
    ↓
Phase 2: 场景与模板判断
...
Phase 3: 精美排版生成
  ...
  若 illustration 清单非空，先调用 generate_image 得到真实图片 URL
  基于 Paged.js 0.4.3 输出单文件 HTML
  先落地纸面设计系统 token，再应用 A4、分页、防截断、打印按钮、高级克制视觉、正文重排、题内图卡片和原卷校验附录
    ↓
Phase 4: 自检交付
  读取 quality-gate.md
...
- 图形清单：每个图形依赖题必须标记 `questionNumber`、`page`、`figureUrl`、`bbox` 或完整 `diagramSpec/redrawData`。没有这些字段时，不得宣称已带图。
- SVG 重绘门槛：只有 `diagramSpec/redrawData` 完整时才允许 SVG 重绘；题干文字、OCR 普通文本和模型推理出来的形状一律不算。没有真实图且只有 SVG 时，必须标记为“模型重绘草图”，不能宣称“准确还原”。
- 辅助插画清单：如果使用 AI 生成阅读配图，必须单独标记为 `illustration`，不能混入 `figure` 清单，也不能用于通过原图保真验收。
- 工具名兼容：当前平台图片生成工具通常叫 `generate_image`，历史会话里可能叫 `picture_gen`。优先调用当前可用的 `generate_image`。
...
10. 带图题必须优先在题目附近显示真实题内图；附录只能用于核对，不能替代题内图
11. 禁止手动切固定高度页面，交给 Paged.js 自动分页
...
- 禁止把原图丢失后自行生成错误图。
- 禁止为了“好看”重绘不确定的几何图、统计图、电路图、地图、表格图；没有真实图 URL、bbox 或完整 `diagramSpec/redrawData` 时必须阻塞。
- 禁止把仅凭题干文字生成的 SVG 称为“精确重绘”“准确还原”“原图还原”。
- 禁止用 `generate_image` / `picture_gen` 生成的插画替代原卷图形；AI 插画只能作为阅读/封面/装饰辅助，并清楚标注。
- 禁止在英语/语文阅读材料需要辅助配图且 `generate_image` 可用时，直接交付无图版 HTML。
...
- [ ] 原卷图形、图表、几何图、电路图没有丢失。
- [ ] 图形依赖题有题内真实图片、裁剪图、明确 bbox 或完整 diagramSpec/redrawData；没有就阻塞，不凭感觉重绘。
- [ ] 没有把仅凭题干文字生成的 SVG 当作准确原图。
- [ ] AI 生成图只作为辅助插画，不被计入原卷图/题内图保真。
- [ ] 语言类阅读/语文材料若缺少非关键配图，已调用 `generate_image` 生成并插入辅助插画。
...
- [ ] 视觉设计已使用统一 token、题号锚点、题型层级、图形卡片、答题区规则，不是默认灰色模板。
- [ ] 第一页不是空白页。
- [ ] A4 打印预览正常。
- [ ] 题目、表格、图片不被分页切断。
- [ ] 填空横线为标准单横线，适合手写。
