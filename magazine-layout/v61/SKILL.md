...
**最佳输入**：用户在线下用 mineru 跑出文件、再用 `mineru_to_package.py` 打包好的 source-package。本 skill 支持 **3 种入口**（按可用性优先级）：

```text
[A] OSS URL 入口（首推）
    用户消息中贴出 https://musk-test.fbcontent.cn/.../xxx.txt 或 .json
    → 调用  read_url(url)              # 1 行/64 行皆可一次拿全
    → 拿到字符串后 JSON.parse 即可

[B] 上传文件入口（次推）
    用户在飞象网页拖入 *.txt（单行压缩 JSON）或 *.html（含 <script id="source-package"> 标签）
    → 调用  read_file(resourceId)
    → .txt：直接 JSON.parse
    → .html：用正则 /<script[^>]*id="source-package"[^>]*>([\s\S]*?)<\/script>/ 提取 JSON 后 parse

[C] 文本粘贴入口（应急）
    用户极少数情况下直接把整个 source-package JSON 粘进对话
    → 直接从最近一条用户消息里取 JSON 字符串后 parse
```

**注意**：飞象网页上传不支持 `.json/.md` 后缀，必须用 `.txt`/`.html` 两种白名单格式承载，或走 [A] OSS URL 路径。

**单文件结构**（无论哪种入口，解析后都是同一个 JSON）：
...
// 1. 三种入口取字符串
let raw;
if (用户消息含 https://...fbcontent.cn/.../*.txt 或 *.json 或 *.html) {
    raw = await read_url(那个 URL);
} else if (本轮有上传文件 resourceId) {
    raw = await read_file(resourceId);
    if (上传是 .html) {
        const m = raw.match(/<script[^>]*id="source-package"[^>]*>([\s\S]*?)<\/script>/);
        raw = m ? m[1] : raw;
    }
} else if (用户消息含 "schema":"magazine-layout/source-package@v") {
    raw = 从用户消息正则截取 JSON;
}

// 2. 解析 + 校验
const pkg = JSON.parse(raw);
assert(pkg.schema.startsWith("magazine-layout/source-package@v"));

// 3. 拿到 schema/source/document/blocks/imageLedger/routing/quality 全部字段
...
| `read_url` | **首推**：读用户在对话里贴的 OSS URL（`*.txt`/`*.json`/`*.html`） | Phase 1 主路径 |
| `read_file` | 读用户上传的 `.txt`（单行压缩 JSON）/ `.html`（含 `<script id="source-package">` 标签） | Phase 1 主路径 |
