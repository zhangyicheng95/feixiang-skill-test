...
如果已经调用 `ask_user`，后续再调用 `create_file` 视为失败；不能先问用户要补充图片，随后又生成“视觉占位符/仅文字排版”的降级 HTML。
