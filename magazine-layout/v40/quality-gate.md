...
- `--min-questions 30`
- `--min-images 1`
- 带图 DOC/DOCX/PPTX/XLSX：如果已知源文档视觉图片数，必须追加 `--expected-images <源文档视觉图片数>`；纯文字/表格截图必须追加 `--require-text-snippet` 检查 OCR 文本或表格关键内容，不能只用 `--min-images 1` 放行。
- `--require-real-images`
- `--require-compact-a4`
- `--require-mathjax`
...
17. 已知源文档含纯文字/表格截图时，截图文字或表格关键内容是否已转成可编辑正文/`<table>`，并用 `--require-text-snippet` 验证。
