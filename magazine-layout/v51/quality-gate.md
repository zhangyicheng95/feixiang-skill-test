# 生成后质量门禁（v50 Layer 3 + Layer 4 + 无 guard 手工自检）
...
- **v50 新增**：`<img src>` 在 `<figure class="question-figure">` 中被引用 ≥ 3 次（伪裁剪集中爆发，v49 真实失败模式）。
- **v50 新增**：`<section class="source-pages">` 中 `<img src>` 与 `<figure class="question-figure">` 中 `<img src>` 集合有交集（source-page 身份污染）。
- **v50 新增**：含 `\frac` / `\dfrac` / `<img>` / 长表达式的 `.options` 使用 `grid-template-columns: repeat(4, 1fr)`（4 列分数选项）。
- **v50 新增**：长宽比 0.62-0.78（A4 比例）+ 短边 ≥ 800 px 的 `<img>` 含 `style="max-height: <250px"` 或同等约束（整页扫描被缩成题图）。
- **v50 新增**：`Phase 3.fullpage` HTML（仅 source-pages）缺失 `.fullpage-tag` 或 `.fullpage-warning` 提示。
- **v50 新增**：MathJax 脚本 src 不含 `mathjax` 与 `tex-svg` 关键字（用了非标准代理脚本）。
...
node guard.js \
  ...
  --forbid-fake-cropping \
  --forbid-source-page-as-figure \
  --forbid-4col-fraction-options \
  --json
```
...
| `--require-fullpage-disclaimer` | Phase 3.fullpage 时必填 | 必须有 `fullpage-tag`/`fullpage-warning` | 未告知模式 |
| `--forbid-fake-cropping` | v50 强烈推荐 | 任一 `<img src>` 在 `.question-figure` 中引用次数 ≤ 2，且 A4 比例图未被 max-height < 250px 缩小 | 伪裁剪 |
| `--forbid-source-page-as-figure` | v50 强烈推荐 | `.source-pages` 与 `.question-figure` 的 `<img src>` 集合无交集 | 身份污染 |
| `--forbid-4col-fraction-options` | v50 强烈推荐 | 含 `\frac` / `\dfrac` / `<img>` / 长表达式的 `.options` 不使用 4 列 grid | 选项错位 |
...
## 无 guard 环境下的 Agent 手工自检 13 条（v50）

如果当前后端没有自动运行 `tools/magazine-layout-guard` 与 `tools/print-preview-guard` 的能力（典型情况是测试环境的 fxls Agent 沙盒没接入 hook），Agent 在 `create_file` 之前**必须**对照下表逐条自查并在响应里写出"已自检"的结论。任一项未通过，**停止 `create_file`**。

| # | 检查项 | 通过条件（如何在响应里证明） | 失败动作 |
|---|---|---|---|
| 1 | **MathJax 标准链路** | `<script src>` 含 `cdn.jsdelivr.net/npm/mathjax@3/es5/tex-svg.js` | 改 src 为标准 CDN |
| 2 | **@page margin ≤ 12mm** | CSS 中 `@page { ... margin: 10mm 12mm; }` 或更紧 | 改成 `10mm 12mm` |
| 3 | **普通题块没有 `break-inside: avoid`** | grep `break-inside` 仅出现在 `table/figure/img/.keep-together/.source-page/.source-figure/.question-figure` 选择器 | 删除 `.question/.question-group/...` 上的 break-inside |
| 4 | **source-page 不当 question-figure** | 列出 `.source-pages img[src]` 与 `.question-figure img[src]` 两组 src，证明集合无交集 | 重新走 figure-cropping 真实裁剪 |
| 5 | **同 src 复用率 ≤ 2** | 列出每个 `.question-figure img[src]` 出现次数，最大值 ≤ 2 | 阻塞，按题号补真实题图 |
| 6 | **A4 比例图不被 max-height 缩小** | 长宽比 0.62-0.78 的 `<img>` 不含 `style="max-height: <250px"` | 改 figure-cropping 最小宽度，或退回 source-page 附录 |
| 7 | **source-pages 不前置图片墙** | `.source-pages` 在 DOM 中位置 ≥ 文档 50%（靠后） | 移到文末附录 |
| 8 | **无占位话术** | grep 不命中："图略 / 占位 / 请参考原卷 / 预留位置 / 此处原卷为 / 对照原卷使用 / 更多题目请参考" | 替换为真实内容或阻塞 |
| 9 | **分数选项不用 4 列 grid** | 含 `\frac` / `\dfrac` / `<img>` / 长表达式的 `.options` CSS 不使用 `repeat(4, 1fr)` | 改 `repeat(2, 1fr)` 或 `1fr` |
| 10 | **无孤立方框成串** | `.option-item` 内 `.option-box` 与文字必须同 flex 父节点；不允许连续 ≥3 个 option-box 单独成行 | 改 `.option-item` 结构 |
| 11 | **text-only 必须用户明确同意** | 含 `.text-only-tag` 时，用户消息历史里有"忽略图片/只要文字版/不管图"等明确字眼 | 否则改回阻塞 |
| 12 | **答案/解析完整保留** | 若源文档存在"参考答案/答案/解析"，最终 HTML 中每题答案与解析逐条出现 | 补全答案 |
| 13 | **未用 generate_image 替学科原图** | 学科为数学/物理/化学/生物/地理时，HTML 中 `<img>` 不含 AI 生成图（即使 figcaption 标"AI 辅助插图"，学科原卷场景也禁止） | 删除 AI 图，阻塞要原图 |
...
4. source-page 不当 question-figure ✅ 两组 src 无交集（source-pages 4 张，question-figure 12 张）
5. 同 src 复用 ≤ 2 ✅ 最大复用 1 次
6. A4 比例图未被 max-height 缩小 ✅ 题图已按 figure-cropping bbox 裁剪
7. source-pages 在文末 ✅ 位于文档第 92% 处
8. 无占位话术 ✅
9. 分数选项 ≤ 2 列 ✅ 分数题用 .options-stacked
10. 无孤立方框 ✅
11. 非 text-only ✅
...
任一项未达成，**禁止 create_file**，改为阻塞或继续修复。
