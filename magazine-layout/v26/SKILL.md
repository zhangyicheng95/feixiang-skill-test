...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级讲义、练习单、试卷、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、最好看、准确还原、带图、可打印、A4、讲义、题单、模板复用、按这个格式来。本技能不搜题；内容已给定时先完整复现，再基于 Paged.js 0.4.3 做设计师级重排。生成前必须建立纸面设计系统 token（字体、颜色、间距、题号锚点、题型层级、图形卡片、答题区规则）。PDF/截图/扫描卷必须 OCR/文本转换并产出可编辑重排正文；图形题优先使用真实题内图 URL、裁剪图或 bbox，禁止凭感觉重绘；逐页图片只能作为原卷校验附录或题内原图，不能作为主体内容贴图交付。英语/语文阅读材料、封面或非关键语境图缺失时，应调用可用的 `generate_image`（旧名 `picture_gen`）生成统一风格 AI 辅助配图并插入，标注“AI配图”，不能直接交付无图版。如果无法 OCR/结构化出完整题目，或数学/理科等图形依赖题缺少题内图片裁剪/定位结果，必须停止说明缺少 OCR/题目结构化/题内图片结果。禁止同时调用 mathdesign-1-html、html-authoring、page-optimize、mathdesign-* 等其他排版/HTML Skill；禁止空白 PDF 框、整页贴图冒充排版、编造图片 URL、占位和“参考原卷”式偷懒。
---
