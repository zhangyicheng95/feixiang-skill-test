...
```bash
cd magazine-layout/tools/pdf-to-page-images
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
python pdf_to_pages.py \
  --input "../../../2026北京西城六年级（上）期末数学.pdf" \
  --out "../../../tmp/pdf-pages-test" \
  --dpi 300
```

该工具会输出 `page-001.png`、`page-002.png` 等图片和 `manifest.json`，后端上传 PNG 后把真实 URL 交给 Agent。

```bash
# 题内图裁剪
cd ../question-figure-cropper
python3 crop_figure.py \
  --page-image "../../../tmp/pdf-pages-test/page-001.png" \
  --bbox-json '{"x":520,"y":360,"width":300,"height":210,"unit":"px"}' \
  --question-id q-002 \
  --min-print-width-mm 60 \
  --out "../../../tmp/figs/fig-002.png"
```

```bash
# 聚合图片读取台账
cd ../image-ledger-builder
node build_ledger.js \
  --pages-manifest "../../../tmp/pdf-pages-test/manifest.json" \
  --figures '../../../tmp/figs/*.json' \
  --out "../../../tmp/image-ledger.json"
```

Agent 生成 HTML 时必须直接使用上传后的真实 URL，不能重新猜测或改写。逐页图片只能作为校验或裁剪来源，不能替代可编辑重排正文。
