...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级试卷、讲义、练习单、题单、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、好看、美化、可打印、A4、一比一、还原、复原、真实原图、原卷图、按这个格式来。上传试卷的目标是“真实内容一比一保真 + 设计师级美化重排”：真实原图保真是底线，美化排版是核心交付，二者都必须满足。v48 把流程分四层：上游解析、Agent 重排、HTML 静态门禁、打印产物验证（详见 engineering-pipeline.md）。v48 输入判定：只有“标准化素材包 source-package.json 齐全（题号级结构 + 可编辑正文 + 公式/表格 + 逐页图或题内视觉图 URL/bbox + 每张图片读取台账 + 图片文字/表格 OCR 可编辑还原 + 答案/解析逐条保留）+ A4 紧凑分页策略”齐全时才生成 HTML；PDF/截图/扫描卷/DOC/DOCX/PPTX 的每个图片块必须先按 screenshot-classification.md 显式分类并逐张记录：纯文字/表格/题目/答案截图先转成可编辑 HTML 文本或表格再排版，图形/照片/示意图/无法可靠 OCR 的区域才作为真实 `<img>` 保真，整页/半页截图只作为校验附录或裁剪来源、不得直接当题内图。题内图必须按 figure-cropping.md 真正用 bbox 裁剪后嵌入并满足最小可读宽度（默认 35mm，含刻度/电路/统计/装置图 ≥ 60mm 至 100mm），不得用 CSS 缩放整页图冒充。**数学/物理/化学/生物/地理等图形依赖型试卷，或源文档含“如图/下图/统计图/示意图/电路图/光路图/实验装置”等线索时，缺真实题内图默认阻塞，禁止把“仅文字版/不含图形条件”作为默认推荐降级；只有用户明确说“忽略图片，只排文字”时才允许 text-only 降级，且必须在标题与首屏明确标注“文字版，不含原图形条件，不可作为完整试卷使用”**。若图片块主要是文章段落、题目或答案，不得集中做成图片合集，必须按原阅读顺序转写进正文。若任一图片没有来自工具输出的 OCR 文本、没有保真 `<img>`、也没有在读取台账中说明去向，必须阻塞。用户或测试 prompt 提供的验收片段只能用于校验，不能当成 OCR 来源、不能据此“视觉化还原”。一旦阻塞、调用 ask_user 或发现需要补素材，本轮必须终止，禁止之后再调用 create_file 生成占位版/降级版 HTML。若存在必须保留的视觉图片对象、用户要求读图/带图，或转换结果提示 figure/image/media，但工具没有返回真实视觉图片 URL，必须禁止 create_file 并返回缺少图片提取资源的阻塞说明。纯文字截图可以转为可编辑正文，但必须逐张还原到语义位置并纳入验收。A4 分页禁止给 `.question`、`.question-group`、`.question-item`、`.answer-area`、`.point-card`、`.section`、`.card` 等普通题块/重复卡片默认 `break-inside: avoid`，否则会造成大面积空白。内容密集型 A4 必须使用 `@page { size: A4 portrait; margin: 10mm 12mm; }` 或不超过 12mm 的页边距。若源文档有“参考答案/答案/解析”，必须完整保留每条答案与解析，禁止压缩成只有选项字母。禁止 AI 生成图、SVG/Canvas/CSS 猜画、文字占位、少量 OCR 文本或大量空白分页冒充完成。**生成 HTML 后必须串联两道工程门禁：tools/magazine-layout-guard 静态校验 + tools/print-preview-guard 打印 PDF 校验，任一失败不发布**。
---
...
```plain
Layer 0: 链路自检
  读取 engineering-pipeline.md，确认四层架构
  不调用 mathdesign-1-html / html-authoring / page-optimize / mathdesign-* 等其他排版 Skill
    ↓
Layer 1: 上游解析（Phase 1 + Phase 1.2）
  读取 reproduction-guide.md
  扫描 PDF、图片型 PDF、截图类输入和带图 Office 文档读取
  ...
  按“输入判定表”分流：素材齐全才进入 Layer 2；缺图/缺结构/未分类图片必须阻塞
    ↓
Layer 1.5: 保真专项检查
  读取 math-image-fidelity.md
  校验分数公式可渲染、原图/表格已保留、题量前后一致
    ↓
Layer 2: 场景与模板判断 + 精美排版生成（Phase 2 + Phase 3）
  ...
  题内图必须使用 figure-cropping.md 的最小可读宽度
  普通题块禁止批量 break-inside: avoid
    ↓
Layer 3: HTML 静态质量门禁（Phase 4-1）
  读取 quality-gate.md
  调用 tools/magazine-layout-guard：题量、图片、MathJax、紧凑 A4、占位词、
  孤立方框、答案解析、台账验收片段、不可达 URL、break-avoid 误用
  失败 → 不发布
    ↓
Layer 4: 打印产物验证（Phase 4-2）
  读取 print-preview-guard.md
  调用 tools/print-preview-guard：Puppeteer 渲染 + 等 MathJax/Paged.js 完成 →
  page.pdf() → 解析 PDF：页数、首页非空、文本量、图片量、空白比例、孤立方框、资源加载错误
  失败 → 不发布并保留失败截图
    ↓
通过双门禁后才发布
```
