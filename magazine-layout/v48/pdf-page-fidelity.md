...
上传 PDF、扫描卷、截图卷时，必须先保证“原卷看得见”。这里的“看得见”必须是页面内真实 `<img>` 图片，不是可能显示为空白的 PDF 嵌入框。文字重排是增强层，不是替代层。
...
1. 后端 MinerU/Docling/PaddleOCR-VL 直接返回 `pageImages[]` 和 `figures[].url`。
2. 后端只有 PDF：调用 `tools/pdf-to-page-images`（PyMuPDF）将 PDF 每页转为 PNG，并产出 `manifest.json`；同时调用 `tools/question-figure-cropper` 按 bbox 裁剪题内图。HTML 用 `<img>` 嵌入每页 + 题内裁剪图。
3. 如果已有页面截图或上传图片资源，HTML 用 `<img>` 直接嵌入。
4. 如果能从 PDF 中提取题内图片，HTML 用 `<img>` 回填到对应题目。
5. 如果以上都做不到，必须告知“当前缺少 PDF 转图/图片提取能力，无法完整保留原卷图形”，并停止生成 HTML，不能生成伪保真结果。

### tools/pdf-to-page-images 输出合同

```jsonc
// manifest.json
{
  "source": { "fileName": "input.pdf", "fileType": "pdf" },
  "dpi": 300,
  "pages": [
    {
      "page": 1,
...
后端上传后填充 `url` 字段。Agent 必须使用 `url` 而非 `path`，否则 HTML 中是相对路径，打开报 404。
