...
- 表格、图片、关键题组可以加 `break-inside: avoid`；禁止给 `.section`、`.main`、`.card`、`.chapter`、`.point-card`、`.question-item` 等大容器或大量重复卡片统一加 `break-inside: avoid`，否则 Paged.js 会把整块推到下一页，造成大面积空白。
