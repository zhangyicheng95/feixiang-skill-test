...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级试卷、讲义、练习单、题单、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、好看、美化、可打印、A4、一比一、还原、复原、真实原图、原卷图、按这个格式来。上传试卷的目标是“真实内容一比一保真 + 设计师级美化重排”：真实原图保真是底线，美化排版是核心交付，二者都必须满足。v36 输入判定：只有“题号级结构 + 可编辑正文 + 公式/表格 + 真实逐页图或题内图 URL/bbox + A4 紧凑分页策略”齐全时才生成 HTML；PDF/截图/扫描卷/DOC/DOCX/PPTX 若存在图片对象、用户要求读图/带图，或转换结果提示 figure/image/media，但工具没有返回真实图片 URL，必须禁止 create_file 并返回缺少图片提取资源的阻塞说明。禁止 AI 生成图、SVG/Canvas/CSS 猜画、文字占位、少量 OCR 文本或大量空白分页冒充完成。
---
...
| DOC/DOCX/PPTX/XLSX 等 Office 文档，转换结果或用户要求显示存在图片、图示、截图、媒体、表格图片，且工具返回了真实图片 URL 或可访问裁剪图 | 可以 | 生成可编辑重排正文，把真实图片放回语义附近，并使用紧凑 A4 分页 |
...
| 已知 Office 文档含多张图片，但当前只拿到部分图片 URL | 阻塞 | 说明图片提取不完整，不能用 1 张图冒充“图都读了”；必须补齐全部图片资源或明确降级为不含图版本 |
...
- 已知源文档中存在多张图片，但最终只能放入 1 张或少于源图片数的图片。
...
- 对 Office 文档，如果已知源文件包含 N 张图片，最终 HTML 必须包含不少于 N 个真实 `<img>`，或明确说明当前图片提取只返回了部分资源并停止生成；不能只放 1 张图通过验收。
