...
- 已知源文档含多张图片时，最终 HTML 的真实 `<img>` 数量少于源文档图片数或少于工具返回的图片资源数。
...
- `--min-questions 30`
- `--min-images 1`
- 带图 DOC/DOCX/PPTX/XLSX：如果已知源文档图片数，必须追加 `--expected-images <源文档图片数>`；不能只用 `--min-images 1` 放行。
- `--require-real-images`
- `--require-compact-a4`
- `--require-mathjax`
...
16. 已知源文档图片数量时，最终 `<img>` 数量是否达到源图片数；只读到 1 张图不能算“图都读了”。
