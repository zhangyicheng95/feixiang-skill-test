...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级试卷、讲义、练习单、题单、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、好看、美化、可打印、A4、一比一、还原、复原、真实原图、原卷图、按这个格式来。上传试卷的目标是“真实内容一比一保真 + 设计师级美化重排”：真实原图保真是底线，美化排版是核心交付，二者都必须满足。v32 输入判定：只有“题号级结构 + 可编辑正文 + 公式/表格 + 真实逐页图或题内图 URL/bbox”齐全时才生成 HTML；PDF/截图/扫描卷若只有 OCR 文本、只有整页图、或预计会出现 img=0/占位/题量不足，必须禁止 create_file 并返回缺少 MinerU/结构化 OCR/题内图裁剪的阻塞说明。禁止 AI 生成图、SVG/Canvas/CSS 猜画、文字占位或少量 OCR 文本冒充完成。
---
...
| 预计最终 HTML 会出现 `<img>` 为 0、占位、图略、请参考原卷、题量不足 | 阻塞 | 不生成 HTML，返回阻塞说明 |
