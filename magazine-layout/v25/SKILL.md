...
9. 试卷图形默认不重绘。只有在结构化数据明确给出全部几何/统计/坐标/标签信息时才允许 SVG/HTML 重画；否则必须使用真实题内图裁剪、原页局部图或后端返回的图片 URL。
...
12. 对 PDF/截图/扫描卷，若没有 OCR/文本转换出的完整题目结构，或图形依赖题没有真实题内图/裁剪 bbox，必须在生成文件前停止，回复“当前缺少 OCR 文本/题目结构化结果或题内图片裁剪结果，无法完成真正重排”；禁止用整页图片、局部识别结果或漂亮但不准确的重绘图冒充精美排版。
...
Phase 1.5: 保真专项检查
  ...
  对每道图形依赖题建立 figure 清单：题号、页码、真实图 URL 或 bbox、是否可重绘
...
- 图形清单：每个图形依赖题必须标记 `questionNumber`、`page`、`figureUrl` 或 `bbox`。没有这些字段时，不得宣称已带图。
...
- 禁止把原图丢失后自行生成错误图。
- 禁止为了“好看”重绘不确定的几何图、统计图、电路图、地图、表格图；没有完整结构化数据时必须用真实图片或阻塞。
...
- [ ] 原卷图形、图表、几何图、电路图没有丢失。
- [ ] 图形依赖题有题内真实图片、裁剪图或明确 bbox；没有就阻塞，不凭感觉重绘。
