...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级试卷、讲义、练习单、题单、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、好看、美化、可打印、A4、一比一、还原、复原、真实原图、原卷图、按这个格式来。上传试卷的目标是“真实内容一比一保真 + 设计师级美化重排”：真实原图保真是底线，美化排版是核心交付，二者都必须满足。v31 发布前断路器：PDF/截图/扫描卷/试卷图片如果缺少 MinerU/结构化 OCR/题内图片 URL 或 bbox，或预计会出现 img=0、占位、图略、请参考原卷、题量不足，必须禁止 create_file 并停止说明缺少上游能力。禁止用 AI 生成图、SVG/Canvas/CSS 猜画、文字占位或少量 OCR 文本冒充完成。只有拿到真实逐页图/题内图和题号级结构后，才可生成高级 A4 重排 HTML。
---
...
在任何 `create_file` 之前，先做一次“能否发布”判断。只要命中以下任一条件，**禁止调用 `create_file`**，必须直接停止并说明缺少上游能力：

- 输入是 PDF、截图、扫描卷、试卷图片，且没有 MinerU/等效解析服务给出的题号级结构、阅读顺序、公式、表格和题内图片/bbox。
- 最终 HTML 预计会出现 `<img>` 数量为 0，或只能写出“此处/图略/占位/请参考原卷/原卷保真层/补充统计图”等文字兜底。
- 只能从 `convert_to_text` 得到部分 OCR 文本，无法确认原卷题量、题号范围、图形题和统计图位置。
- 只有整页截图 URL，但没有可编辑题目结构；这种情况下可以说明“已具备原卷校验图，但缺少 MinerU/结构化 OCR，不能真正重排”，不能用少量 OCR 题目 + 占位图形生成伪精美排版。
- 一比一原图复原模式下没有题内真实图 URL 或 bbox，且图形题需要准确还原。
...
2. PDF、截图、扫描卷、图片型教案优先使用 MinerU 或等效文档解析服务，拿到 Markdown、阅读顺序 JSON、图片、表格 HTML、公式 LaTeX、bbox 和逐页图片；如果当前链路只暴露 `convert_to_text`，也必须先调用它或等效 OCR/文本转换工具。只调用 `read_file` 不够。
...
Phase 1.2: PDF/OCR 预处理判断
  扫描 PDF、图片型 PDF、截图类输入读取 mineru-integration.md、pdf-ocr-preprocess.md 与 pdf-page-fidelity.md
  ...
  如果没有可编辑文本/结构化结果或真实图片 URL，停止，不进入 Phase 3
...
- 图片里的文字、表格、填空题应优先转成可编辑 HTML 段落或 `<table>`；图片只作为校验或识别失败兜底。
