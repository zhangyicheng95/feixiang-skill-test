...
description: 精美排版唯一主 Skill。Use when 用户要求把已给定内容或上传文档改造为 A4 打印友好的高级讲义、练习单、试卷、默写纸、知识清单、教案打印版、杂志风资料，或提到精美排版、高级排版、最好看、准确还原、带图、可打印、A4、讲义、题单、模板复用、按这个格式来。本技能不搜题；内容已给定时先完整复现，再基于 Paged.js 0.4.3 做设计师级重排。生成前必须建立纸面设计系统 token（字体、颜色、间距、题号锚点、题型层级、图形卡片、答题区规则）。PDF/截图/扫描卷必须 OCR/文本转换并产出可编辑重排正文；图形题优先使用真实题内图 URL、裁剪图或 bbox，禁止凭感觉重绘；逐页图片只能作为原卷校验附录或题内原图，不能作为主体内容贴图交付。如果无法 OCR/结构化出完整题目，或图形依赖题缺少题内图片裁剪/定位结果，必须停止说明缺少 OCR/题目结构化/题内图片结果。禁止同时调用 mathdesign-1-html、html-authoring、page-optimize、mathdesign-* 等其他排版/HTML Skill；禁止空白 PDF 框、整页贴图冒充排版、编造图片 URL、占位和“参考原卷”式偷懒。
---
...
10. `picture_gen` 只能用于非关键阅读配图、封面氛围图或装饰插画，并必须标注为“AI配图/辅助插图”；不能把 AI 生成图计为“原卷图还原”，不能用于数学几何图、统计图、物理电路图、生物结构图、地图、坐标图等影响答题准确性的图。
11. 对 PDF/截图/扫描卷，若没有 OCR/文本转换出的完整题目结构，或图形依赖题没有真实题内图/裁剪 bbox，必须在生成文件前停止，回复“当前缺少 OCR 文本/题目结构化结果或题内图片裁剪结果，无法完成真正重排”；禁止用整页图片、局部识别结果或漂亮但不准确的重绘图冒充精美排版。
...
- AI 配图边界：英语阅读、语文素材、封面氛围可以生成辅助插画提升观感，但它只是“配图”，不是“原图还原”。凡是影响解题数据、标签、比例关系的图，禁止用 AI 生成图替代。
...
- 禁止用 `picture_gen` 生成的插画替代原卷图形；AI 插画只能作为阅读/封面/装饰辅助，并清楚标注。
