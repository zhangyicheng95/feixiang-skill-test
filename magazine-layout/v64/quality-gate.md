# 质量门禁（v64·飞象 iframe 适配 + 防撒谎）

> 14 项硬基线自检 + 4 项 subtype 专项自检。
> `create_html_deliverable` 之前必须**逐条对照、列出实际计数、在响应里完整贴出**。
> 任一未过 → 停止交付、重写 HTML（**不允许**绕过、降级声明、含糊词）。
>
> v64 升级原因：v63 实测发现**飞象老师在用户全屏浏览时把模型 HTML 嵌入 `<iframe srcdoc>`**，注入 1100+ 个反爬噪声元素。用户按 Ctrl+P 打印的是外层壳页面，iframe 内 Paged.js 不生效 → PDF 几乎全空。v63 的 `.no-print` 按钮还被外层壳 `.ai-badge`（位置完全重叠）遮挡，用户找不到。v64 在 v63 防撒谎基础上加固铁律 7，要求顶部居中工具条 + 多入口降级 + 响应明确告知用户不要按 Ctrl+P。
...
| 14 | **顶部打印工具条 + 一致性**（v64 加固） | "grep `class=\"print-toolbar\"` 命中 1；grep `top:\s*8mm` + `left:\s*50%` + `translateX\\(-50%\\)` 各命中 1；grep `window.print()` 命中 ≥ 1；grep `downloadCleanHtml` 函数命中 1；grep `openInNewTab` 函数命中 1；grep `box-shadow\|linear-gradient\|filter:\|background-image:\s*url\|position:\s*fixed` 仅在 `.print-toolbar` / `.no-print` 内命中（业务区域 0 命中）" | 同上 |
...
14. 顶部打印工具条 + 一致性 ✅ .print-toolbar 含 3 按钮（top:8mm + left:50% + translateX(-50%)）；downloadCleanHtml/openInNewTab JS 完整；业务区域无 box-shadow/linear-gradient/filter
...
## 5. v62→v63→v64 失败案例（警惕实测）

| 版本 | 违规 / 撒谎 | 修复版本 | 修复点 |
|---|---|---|---|
| v62 | HTML 14 题 / 30 题，自检"27 vs 27 ✅" | v63 | 自检 10 必须列 grep 实际数 vs questionCount |
| v62 | `repeat(4, 1fr)` 装 4 张图，自检"≤ 2 列 ✅" | v63 | 自检 13 必须 grep `repeat(4` 实际命中数 |
| v62 | 38 blocks 渲染 ~5 段，自检"全文完整保留 ✅" | v63 | 自检 10 用 grep 实际数 |
| v62 | 5/5 缺 `.no-print` 按钮 | v63 | 自检 14 grep `.no-print + window.print()` |
| v62 | `max-width: 800px` 容器 → 打印被裁切 | v63 | 自检 6 必 grep 0 命中 |
| v62 | `box-shadow / linear-gradient` → 打印态丢失 | v63 | 自检 14 必 grep 0 命中 |
| v62 | `<script src="metis-online...">` | v63 | 自检 4 必 0 命中 |
| **v63** | **5/5 用户按 Ctrl+P，PDF 几乎全空（只有页眉 URL）** | **v64** | **铁律 7 加固：顶部工具条主按钮触发 iframe 内 print；响应明确告知不要按 Ctrl+P** |
| **v63** | **`.no-print` 按钮被外层 `.ai-badge` 遮挡（位置完全重叠）** | **v64** | **工具条改为 `top:8mm; left:50%; translateX(-50%)`** |
| **v63** | **iframe 高 100vh，Paged.js 多页溢出被裁切** | **v64** | **副按钮：💾 下载到本地 / 🔗 新窗口打开 让 HTML 脱离 iframe** |
...
echo "14a 顶部工具条: $(echo "$H" | grep -cE 'class=\"print-toolbar\"')"
echo "14b 工具条位置: $(echo "$H" | grep -cE 'top:\s*8mm.*left:\s*50%|translateX\(-50%\)')"
echo "14c print 主按钮: $(echo "$H" | grep -cE 'class=\"print-btn-main\".*window\.print\(\)')"
echo "14d 下载干净 HTML 函数: $(echo "$H" | grep -cE 'function downloadCleanHtml')"
echo "14e 新标签打开函数: $(echo "$H" | grep -cE 'function openInNewTab')"
echo "14f 业务区域 web 装饰: $(echo "$H" | grep -cE 'box-shadow|linear-gradient|filter:|background-image:\s*url')"
```
