...
- 表格、图片、很短且必须同页的关键题组可以加 `break-inside: avoid` 或 `.keep-together`；禁止给 `.question`、`.question-group`、`.answer-area`、`.section`、`.main`、`.card`、`.chapter`、`.point-card`、`.question-item` 等普通题块、大容器或大量重复卡片统一加 `break-inside: avoid`，否则 Paged.js 会把整块推到下一页，造成大面积空白。
