...
- 如果参考 HTML 的打印结果为 2 页左右，而删除内部自检后内容被压成 1 页，不能用“自检说明”撑页，也不能直接给 `.answer` 加 `break-before: page` 导致多余空白页。应使用合法版式调节阅读密度，例如给答案区设置 `margin-top: 8mm` 到 `12mm`、增加章节节奏或教师备注框，使 Chrome 打印保持约 2 页。
- 对英语教案、讲义、练习单这类含完整参考答案的材料，答案区可以作为清晰的教师答案区，但不得对每个答案卡片强制分页。若使用 `break-before`，必须只作用于一个外层答案页容器，并确认浏览器打印页数不会超过金样页数 1 页以上。
