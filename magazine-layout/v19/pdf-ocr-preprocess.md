...
1. 用 OCR/文本转换拿到可编辑文字和题目结构。
2. 基于可确认的文字、题型、题号、公式、答题区做高级重排正文。
3. 同时保留原图或原页面截图作为 `<img>` 校验附录或题内图回填。

OCR 是真正排版的前置条件。没有完整 OCR/结构化结果时，逐页图片只能证明“原卷可见”，不能证明“已经排版”。
...
4. Agent 必须基于 `convert_to_text` 的 OCR/文本结果生成可编辑重排正文。
5. HTML 中可以把真实逐页图片 URL 放入“原卷校验附录”；如果只有逐页图片、没有完整 OCR/题目结构，不能继续生成 HTML。
...
Agent 生成 HTML 时必须直接使用 `source-pages.html` 中的真实 URL作为校验附录，不能重新猜测或改写，也不能把该附录当作正文主体。
...
4. 把 OCR 文本、题目结构、题内图片位置、每页图片作为新资源传给 Agent。
5. Agent 用 OCR/结构化文本做高级重排正文，同时用原页图片做校验附录。
