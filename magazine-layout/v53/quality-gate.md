...
- 金样固定层：若用户要求“按金样/按已生成版本/后台测试根据这个来”，结果必须保留金样 DOM 骨架和关键 class。Lesson883 金样的最低结构信号为 `.cover=1`、`.meta-card=1`、`.meta-row=3`、`.grid-2>=2`、`.card>=5`、`.reading=1`、`.q-title>=5`、`.options>=4`、`.language-box=1`、`.term-list>=2`、`.quote>=2`、`table.compare=1`、`.word-bank=1`、`.answer=1`、`.checklist=1`、`h2>=9`、`@media print`、`print-color-adjust`。只满足颜色、CSS 字数或局部片段不算通过。
