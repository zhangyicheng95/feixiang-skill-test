...
- 如果参考 HTML 的打印结果为 2 页左右，而删除内部自检后内容被压成 1 页，不能用“自检说明”撑页。应使用合法版式保持阅读密度，例如让 `参考答案` / `.answer` 从新 A4 页开始：`.answer { break-before: page; page-break-before: always; }`。答案分页属于纸面结构，不是内容造假。
- 对英语教案、讲义、练习单这类含完整参考答案的材料，答案区通常应作为独立教师页或答案页。若正文与答案挤在同一页导致 4000+ 字一页，判定为密度过高，应将答案区分页。
