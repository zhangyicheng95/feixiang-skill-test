---
name: html-authoring
description: 生成或修改“单页交互教学 HTML / 可打印材料（A4）”时使用，覆盖视觉设计决策与技术实现规范（配色布局、动画选型、浏览器兼容、媒体资源）。不适用于“可发布的多文件 Web 应用（需要 preview/publish/持久化/增量维护）”、多页 PPT 课件、纯知识问答、不涉及 HTML 的场景。
---
...
**不适用于：**
- **可运行应用/小游戏/课堂工具**（需要多文件结构、发布链接、持久化、增量维护）：必须改用 `app-dev`（走 `create_artifact / write_artifact_file / preview / publish` 流程）
- 任何明确要求“给一个可访问链接/发布链接/预览确认再发布/刷新不丢数据/localStorage/KV/安装/多人共享”的场景：必须改用 `app-dev`
