...
- **核心功能 / 可玩类产物必做（spec 注释）**：在 `<head>` 内加一行机读 spec 注释（**不占首行**，避免与上面的首行规则冲突），承载 §1.5 的可验收契约，供 test-html 读回逐条机检：
  ...
  spec 偏大时**外置**为独立 spec 资源，注释里只写 `<!-- spec-ref: resourceId=<id> -->`；任务级约束（count、共用硬约束、核心闭环）**同时写入 `task_write` 的 constraints**（跨轮、抗压缩，每轮重注入）。
- **修改已有 HTML（必做）**：
...
- [ ] **核心机制契约（可玩/有核心功能类，§1.5）**：已显式声明最小可玩闭环且逐环节由代码实现（如 24 点的 +−×÷ 运算符可点击并参与计算，非只画数字）；已写入 `<head>` spec 注释 + `task_write`；不是只做外壳
