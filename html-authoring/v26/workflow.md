...
Phase 5 ┃ 交付硬约束（**缺一即失败，禁止 terminate**）
  ...
  ⑧ 坐标网格闸门：非坐标系/函数图像场景，Stage 默认**无背景方格线**；坐标系场景的网格/轴/刻度/曲线必须共用同一坐标变换（§2.3.1）
```
...
- **默认不铺背景方格线**：色板中的“背景网格”只提供可选视觉语言，不能自动变成 Stage 底纹；除坐标系/函数图像/数轴刻度等需要读数的场景外，Stage 背景必须使用纯色、渐变或色块，禁止默认方格纸背景
...
### 2.3.1 坐标系与方格网格硬约束（防视觉误差）

- **默认禁止** Stage 背景方格线/网格底纹；色板段落里的“背景网格”不能作为全局默认背景使用
- **仅当任务明确需要读坐标**时才允许坐标网格：如“平面直角坐标系 / 坐标轴 / x 轴 y 轴 / 函数图像 / 一次函数 / 二次函数 / 抛物线 / 图像平移 / 数轴刻度 / 坐标读取”
- 坐标网格必须服务读数，不能只是装饰；若无法保证精确对齐，**宁可只画坐标轴、刻度和必要参考线，不画背景网格**
- 允许绘制坐标网格时，必须定义唯一的 `unit`、`originX`、`originY`，并用同一变换绘制所有元素：
  ```js
  xScreen = originX + x * unit;
  yScreen = originY - y * unit;
  ```
- 网格线、坐标轴、刻度、标签、曲线、点位、辅助线必须全部来自上述同一坐标变换；x 轴和 y 轴必须落在网格线上，刻度标签必须对齐网格交点
- 优先用同一个 SVG / Canvas 绘制网格、轴、刻度、曲线；禁止用独立 CSS `repeating-linear-gradient` 当坐标背景，除非 `background-size` 与 `background-position` 严格绑定同一个 `unit/origin`
- 坐标网格颜色仍必须来自选定 palette 的浅色/边框色变量，不能引入外源灰蓝、默认蓝等非 palette 颜色
