...
> 数学场景的色板与布局见 `math-design/`。**本地附件（references/）用 `read_file` 相对路径读；math-design 子文件用 `<skill-files>` 表的 CDN `read_url`**——都按需读取，不要凭记忆默写。
...
| 路由 | 读哪些 | 配色 | 禁止 |
|---|---|---|---|
| **数学 → math-design** | **必读** `math-design/workflow.md` → 色板子文件 → `visual-impact.md`；涉及坐标系/坐标轴/函数图像再读 `manipulatives.md`（用 `<skill-files>` 表 CDN URL 原样 `read_url`，禁自拼路径） | 31 套 A/B 色板；H1=40px；按钮 80px；首行 palette 注释 | 禁"参考配色方案"；禁 math-design 外的配色决策 |
...
`workflow.md`(工作流+字号布局) · `color-palettes-a.md`(小学 A-01~11) · `color-palettes-b.md`(初高中 B-01~20) · `visual-impact.md`(演示区多色) · `manipulatives.md`(标准坐标轴/教具)

## 工作流程

1. **理解需求 + 学科路由**（第 0 步）。需求模糊时按教学经验选最合理默认继续，**禁止 `ask_user`**；在最终回复里以"默认按 X 处理，如需调整请告知"声明假设。
...
5. **生成 / 修改 HTML**：首次 `create_file`，修改先 `read_file` 再 `edit_file`（保原配色令牌·布局骨架·控件集合，只改要求处，禁借机重写）。数学场景首行写 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；非数学首行 `<!DOCTYPE html>`（若含 math-design 注释则 `edit_file` 删除）。可玩类在 `<head>` 写 spec 注释（见契约节）。
...
**spec 产出**：在 `<head>` 内写一行机读契约注释（不占首行，避开首行规则），供 test-html 读回逐条机检：
`<!-- spec: type=<品类>; core-loop=<最小可玩闭环>; forbid=<禁止项>; require=<必含项>; count=<份数> -->`
大则外置为资源、注释只写 `<!-- spec-ref: resourceId=<id> -->`；任务级契约同时写入 `task_write` 的 `spec` 字段（跨轮抗压缩）。
...
**核心机制（可玩/有核心功能类）**：□ 已声明最小可玩闭环且逐环节由代码实现（如 24 点 +−×÷ 可点击并参与计算，非只画数字）□ 已写 `<head>` spec 注释 + `task_write.spec` □ 不是只做外壳。
...
**视觉/通用**：□ 数学场景：已读 math-design 子文件 + 首行 palette 注释 + 推理声明 keyword/pool/source/palette_id + Stage flex 居中 + `#controls` 无滚动 □ 非数学：未 read math-design + 首行 `<!DOCTYPE html>` 无 math-design 注释 + ≥3 CSS 变量 □ ≥1 个过渡/`:hover` □ 有主体的图主体完整可见 □ 公式全用 `\(...\)`/`\[...\]` 无 `$` □ 现代 CSS 加 `-webkit-` □ 对比度 WCAG AA □ 兼容移动/PC 触摸与鼠标。
