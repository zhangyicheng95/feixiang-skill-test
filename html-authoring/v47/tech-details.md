...
数学 3D 几何/立体图形/体积表面积/空间关系演示：优先 Three.js，并按 `math-design/grid-templates.md` 创建 3D 场景内的标准地面网格对象；页面 `#stage` / `canvas` 容器背景保持纯色或色板背景，禁止用 CSS `linear-gradient`/`background-image` 伪装地面网格。若使用 p5 WEBGL 或 SVG 伪 3D，也必须在渲染场景内部表达地面/底面参照，不能使用页面平面底纹。
