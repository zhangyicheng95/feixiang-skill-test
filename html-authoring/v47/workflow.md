...
Phase 4 ┃ 按需读取（禁止全量加载 31 套）
  ...
  - 若页面涉及坐标网格、方格纸、点阵纸、几何画布、读点、对齐、拖拽吸附、数学 3D 地面网格 → read grid-templates.md，并按本文件 §1.1 选择 SVG/Canvas/Three.js 分支
...
Phase 5 ┃ 交付硬约束（**缺一即失败，禁止 terminate**）
  ...
  ⑨ 数学网格/点阵：只要网格/点阵用于读数、定位、对齐、吸附、坐标纸或几何画布，必须使用 grid-templates.md 的 SVG/Canvas grid-layer；CSS `background-image` / `linear-gradient` 只能作非测量装饰
