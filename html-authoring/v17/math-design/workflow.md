...
> **仅当 html-authoring 判定为数学场景时启用。** 非数学学科禁止读取本目录任何文件。
...
Phase 5 ┃ 字号与 HTML 注释自检
  - H1 必须 40px / font-weight:700 !important
  - 按钮高度 80px；按钮字号 28px
  - HTML 首行或 `<head>` 内写入：`<!-- html-authoring:math-design palette=A-07 -->`（替换为实际编号）
```
