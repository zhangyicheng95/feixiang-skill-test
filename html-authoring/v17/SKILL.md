...
- **内容组织模式**（**先于布局决定**，见"内容组织模式"章节）：
  ...
  - 禁止直接把大量内容摊平成一条滚动长页
- **配色方案**（**按学科路由二选一，禁止混用**）：
  - **数学场景**：按 [math-design/workflow.md](math-design/workflow.md) Phase 2–4 随机选 1 套 A/B 色板 → `read_file` 对应段落 + visual-impact.md → 将 hex 映射为 `--primary` 等 CSS 变量；HTML 写入 `<!-- html-authoring:math-design palette=XX -->`
...
**视觉层（必检）：**
- [ ] **数学场景**：已 read math-design 色板子文件；HTML 含 `<!-- html-authoring:math-design palette=XX -->`；H1=40px；按钮 80px；演示区 hex 来自选定色板
