...
- 显式方格线必须肉眼可见且通过 `test_html` 的样式断言：网格线 computed stroke 与 Stage/plot 背景 `contrast >= 1.5` 或 `maxDelta >= 32`，`stroke-width >= 1.25px`，`opacity >= 0.65`；禁止 `#C5BFBF` 配 `#CBCBCB` 这类近灰同色导致“有 DOM 但看不见”
