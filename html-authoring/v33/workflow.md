...
- 方格/参考线必须肉眼可见，不能只是 DOM 中存在：颜色必须明显区别于 Stage/plot 背景；禁止 `#C5BFBF` 配 `#CBCBCB` 这类近灰同色
...
显式方格线可见性测试可直接使用如下思路（按页面实际选择器调整）：

```js
const grid = document.querySelector('.grid, [data-grid-line], #grid-lines line');
const stage = document.querySelector('#stage, .stage, .plot-area, .chart-area');
const gridStyle = getComputedStyle(grid);
const stageStyle = getComputedStyle(stage);
// 将 rgb()/rgba()/hex 解析为 r,g,b,a；计算 contrast 与 maxDelta。
// 断言：gridCount > 0，horizontalCount > 0，verticalCount > 0，
// contrast >= 1.5 || maxDelta >= 32，strokeWidth >= 1.25，opacity >= 0.65。
```
