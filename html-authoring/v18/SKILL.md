...
| 路由 | 判定条件 | 视觉/配色链路 | 禁止 |
|---|---|---|---|
| **数学 → math-design** | 学科为数学 / K12 数学知识点；或含：教学动画、互动课件、算理演示、几何可视化、方程函数、统计概率、数形结合、三维展示、单位进率、乘法口诀、勾股定理、一次函数、鸡兔同笼、闯关游戏等数学触发词 | **必读** [math-design/workflow.md](math-design/workflow.md)，按 Phase 2–5 执行；按需 read 色板子文件 | 禁止使用下方「参考配色方案（5 个）」；禁止「数学-橙紫」通用配色 |
| **非数学 → 通用** | 语文、英语、物理、化学、生物、历史、地理等；或非 K12 数学的 HTML | 使用本 skill「色彩系统 → 参考配色方案」 | **禁止** read `math-design/` 下任何文件 |

**数学场景信号词（命中任一 + 产出单页 HTML → 走 math-design）：**
教学动画、互动课件、算理演示、数形结合、三维展示、互动设计、习题配置、呈现形式、单位进率、相遇问题、面积周长、平均数、勾股定理、圆面积周长、圆柱圆锥、阴影面积、鸡兔同笼、植树问题、一元一次方程、方程组消元、因式分解、一次函数、二次函数、概率转盘、等差数列等。

**歧义处理：** 用户未明示学科但知识点明显为数学（如「勾股定理演示」）→ 走 math-design；明显为非数学（如「古诗配图」「力的分解」）→ 走通用；仍不确定且含 HTML 生成意图 → 默认通用，**不要** read math-design。
...
### 1. 理解需求与视觉定位
- **第 0 步：学科路由**（见上方「学科路由」章节）—— 数学 → read `math-design/workflow.md` 并声明 `palette_id`；非数学 → 跳过 math-design
- 确认学科、学段、核心知识点
...
- **配色方案**（**按学科路由二选一，禁止混用**）：
  ...
  - **非数学场景**：从下方「参考配色方案（5 个）」中选择或自定义；输出 `primary / secondary / accent / background / foreground` 5 个 Tailwind 设计令牌
...
### 4. 生成或修改 HTML
- 首次创建 → `create_file`
- **数学场景 create_file 后必做（Phase 5 硬约束）**：检查 HTML 第一行是否为 `<!-- html-authoring:math-design palette=XX -->`（XX=Phase 3 的 palette_id）；若不是 → **`edit_file` 在文件最顶部插入该注释** → 再次确认 → 才允许进入自检与 `terminate`
- **修改已有 HTML（必做）**：
...
**视觉层（必检）：**
- [ ] **数学场景 — palette 注释（硬约束，缺则禁止 terminate）**：HTML **第一行**为 `<!-- html-authoring:math-design palette=XX -->`（XX 与 Phase 3 palette_id 一致）；若首行是 `<!DOCTYPE html>` → 立即 `edit_file` 补注释
- [ ] **数学场景**：已 read math-design 色板子文件；H1=40px；按钮 80px；演示区 hex 来自选定色板
- [ ] **非数学场景**：未 read math-design 目录；至少 3 个 CSS 变量（或 Tailwind 设计令牌）
