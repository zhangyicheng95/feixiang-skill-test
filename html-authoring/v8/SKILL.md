...
- 需求模糊但可推断时，基于教学经验合理推断
- **【硬性禁止】严禁调用 `ask_user` 向用户追问**。本 skill 运行场景包括评测 / 脚本化 / 无人值守调用，`ask_user` 会让产物流程不收敛（等不到回复 → 最终只吐文字总结 → 没有 HTML 交付）。任何信息缺失场景一律按"最贴近教学经验的合理默认"继续执行，并在最终回复里以"我默认按 X 处理，如需调整请告知"的句式把决策权延迟到下一轮
...
- **【关键】严禁硬编码任何未经 `generate_image` 真实返回的图片 URL**（如 `transparent_placeholder.png`、`placeholder.png`、`temp_*.png`、`example.jpg` 等编造的文件名）。需要图像时必须实际调用 `generate_image` 拿到真实 URL；当前页面不需要实体图像而只需占位/装饰时，用 SVG 图标、Emoji、CSS 图形或纯色色块替代，**严禁编造图片文件名**

### 4. 生成或修改 HTML
- 首次创建 → `create_file`
- **修改已有 HTML（必做）**：
  1. 用户意图是"改/调整/优化/修复/补充/重做"现有交付物时，属于修改场景
  2. 必须先用 `read_file` 读取目标 HTML 的 `resourceId`；若用户未提供 resourceId，**不要调用 `ask_user` 等待回复**——按"用户最近一次交付物"假设继续读取，并在回复里声明所作假设
  3. `read_file` 成功后用 `edit_file` 增量修改，保持原配色令牌、布局骨架、控件集合稳定，只改用户要求的部分
  4. 禁止借"改一处"之机把整页重写、简化或改换风格
  5. **必须以有效交付收尾**：所有修改必须通过 `edit_file`（或 `create_file`）真实落盘，不得只用文字总结代替实际修改。当本轮涉及多轮 read/edit 时，**最后一轮必须是一个成功的 `edit_file` 或 `create_file`**，然后再 `terminate`；禁止以纯 assistant 文字结束却无任何文件操作
- 严格遵守下方"技术约束规则"
...
- ❌ **编造的占位图文件名**（如 `transparent_placeholder.png`、`placeholder_*.png`、`temp_*.png`、`example.jpg` 等），哪怕拼成飞象域路径也不行——必须是 `generate_image` 真实返回的 URL
...
- [ ] **所有 `<img src>` / `background-image: url(...)` 的图片 URL 都来自 `generate_image` 真实返回，未出现 `*_placeholder.png` / `temp_*.png` / `example.jpg` 等编造文件名**
- [ ] 所有引用的图片/音频 URL 都能在 `musk-test.fbcontent.cn` 环境访问

**流程自检（必检）：**
- [ ] **本轮对话未调用 `ask_user`**（信息缺失一律按合理默认继续 + 回复声明）
- [ ] **修改场景以成功的 `edit_file` / `create_file` 收尾**，不是以纯文字总结结束
...
### 案例 F：评测/无人场景调用 ask_user 导致产物不闭环
- **现象**：用户让在上传的 HTML 基础上新增 N 个功能，agent 读完原文件后调用 `ask_user` 确认交互细节；评测场景无人回应，会话长时间等待后只吐一段文字总结就 terminate，没有任何 HTML 交付物被系统识别
- **根因**：把"需求模糊"当"应该问用户"处理，但本 skill 运行场景包含评测/脚本化调用，`ask_user` 会让流程不收敛
- **修复**：遇到信息缺失直接选"最贴近教学经验的合理默认"继续 `edit_file` / `create_file`；在最终回复的使用指南里声明"我默认按 X 处理，若需调整请继续告知"，把决策权延迟到下一轮。收尾必须是成功的文件操作 + terminate，禁止纯文字结束

### 案例 G：图片 URL 被编造为假占位文件名
- **现象**：生成的 HTML 里出现 `<img src=".../transparent_placeholder.png">`，浏览器打开 404，用户看到空白图位
- **根因**：agent 没有实际调用 `generate_image`，凭借"看起来像飞象域路径"编造了一个并不存在的占位文件名
- **修复**：需要图片时必须先 `generate_image` 拿到真实 URL；若当前场景并不需要真实图像（如纯文字交互卡片、图标装饰），用 SVG、Emoji、CSS 图形或纯色色块替代——**绝不允许写任何未经真实生成的图片文件名**
