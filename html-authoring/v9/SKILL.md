...
- **【关键】图片 URL 必须来自 `generate_image` 真实返回，严禁编造文件名**（`*_placeholder.png`、`temp_*.png`、`example.jpg` 等一律禁止）；不需要真实图像时用 SVG / Emoji / CSS 图形替代
...
- ❌ 编造的图片文件名（`*_placeholder.png`、`temp_*.png`、`example.jpg` 等），即使拼成飞象域路径也不行——必须是 `generate_image` 真实返回的 URL
...
- [ ] 所有图片 URL 来自 `generate_image` 真实返回，无编造文件名（`*_placeholder.png` / `temp_*.png` / `example.jpg` 等）
