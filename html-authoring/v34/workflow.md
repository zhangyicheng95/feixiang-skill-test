...
- `test_html`/自检必须验证：方格/参考线数量 > 0；横竖方向都存在；坐标轴/图表轴存在；方格/参考线与轴线不应同色同粗；并计算上述 contrast/maxDelta/stroke-width/opacity，未达阈值即视为方格线缺失
...
- 未显式要求方格/参考线时，坐标轴场景代码中禁止出现：用于背景方格的 CSS `background-image` 网格、成对 `linear-gradient`、`repeating-linear-gradient`、SVG `<pattern>` 方格底纹、`gridLayer`、`initGrid()`、`drawGrid()`、`createGrid*()`、Canvas/SVG 中覆盖全宽全高的全量横竖线循环；此处不禁止 CSS 布局 `display:grid`
...
- `test_html`/自检必须按意图分支：坐标系/坐标轴场景且未要求方格/参考线时，断言 Stage/plot 容器 computed `backgroundImage` 不含 `linear-gradient`/`repeating-linear-gradient`，无贯穿绘图区的全幅网格/参考线，同时坐标轴/图表轴和轴标签存在；明确要求方格/参考线时，断言方格/参考线存在、横竖方向都有、坐标轴存在且轴线比方格线更醒目，并断言网格线 computed stroke 相对 Stage/plot 背景 `contrast >= 1.5` 或 `maxDelta >= 32`、`stroke-width >= 1.25px`、`opacity >= 0.65`；非坐标轴场景不执行本分支
...
grid_check = page.evaluate("""() => {
  ...
  const gStyle = getComputedStyle(grids[0]);
  const stroke = parseColor(gStyle.stroke || grids[0].getAttribute('stroke'));
  ...
  const strokeWidth = parseFloat(gStyle.strokeWidth || grids[0].getAttribute('stroke-width') || '0');
  const opacity = Math.min(parseFloat(gStyle.opacity || '1'), parseFloat(gStyle.strokeOpacity || grids[0].getAttribute('stroke-opacity') || '1'));
  const horizontal = grids.filter(l => l.tagName.toLowerCase() === 'line' && l.getAttribute('y1') === l.getAttribute('y2')).length;
  const vertical = grids.filter(l => l.tagName.toLowerCase() === 'line' && l.getAttribute('x1') === l.getAttribute('x2')).length;
  return {count: grids.length, horizontal, vertical, contrast, maxDelta, strokeWidth, opacity};
}""")
