...
- 显式要求坐标方格/参考线时，方格/参考线必须可见并与坐标轴、刻度、标签、曲线/柱形共用同一 `unit/origin` 或图表比例尺；未要求时不得在 Stage/plot/SVG/Canvas 使用 `background-image`、成对 `linear-gradient`、`repeating-linear-gradient`、SVG `<pattern>`、`initGrid()`/`drawGrid()` 或全宽全高横竖线循环生成背景网格
