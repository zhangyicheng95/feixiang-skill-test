...
- **默认不铺整页/整图方格纸背景**：色板中的“背景网格”只提供可选视觉语言，不能自动变成 Stage 底纹或函数坐标图底纹；统计直方图、柱状图、条形图、表格等可以使用图表/模块内部参考线，但这些线必须属于具体图表或模块，不能覆盖整个 Stage 形成独立背景方格纸
...
- 未显式要求坐标方格时，坐标图代码中禁止出现：用于坐标背景的 CSS `repeating-linear-gradient`、SVG `<pattern>` 方格底纹、Canvas/SVG 中覆盖全宽全高的 `drawGrid()` / 全量横竖线循环；此处不禁止 CSS 布局 `display:grid`
