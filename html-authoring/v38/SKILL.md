...
每次启用 html-authoring 后，**必须先判定学科**，再进入后续设计决策。禁止跳过。

**判定顺序（按优先级，命中即停止）：**

1. **非数学学科前缀（最高优先级）** — prompt 含「语文/物理/化学/生物/地理/政治/道法/历史/英语/…」+ 非数学知识点 → **通用链路**，即使同时出现「教学动画」「互动课件」也**不走** math-design
2. **数学知识点** — 勾股定理、一次函数、鸡兔同笼、方程、概率、等差数列、面积周长、单位进率等 K12 数学内容 → **math-design 子链路**
3. **歧义** — 仅「教学动画/单页 HTML」但无学科且无数学知识点 → **默认通用**，**不要** read math-design

| 路由 | 判定条件 | 读哪些文件 | 视觉/配色 | 禁止 |
|---|---|---|---|---|
| **数学 → math-design** | 命中数学知识点（见下）；且无步骤 1 的非数学学科前缀 | **必读** `math-design/workflow.md` → 色板子文件 → `visual-impact.md`（须 `read_file`/`read_url`，见下） | 31 套 A/B 色板；H1=40px；按钮 80px；首行 palette 注释 | 禁止「参考配色方案」；禁止 math-design 以外的配色决策 |
| **非数学 → 通用** | 步骤 1 学科前缀；或步骤 3 默认 | **仅读本 skill 的 SKILL.md 主体**（含参考配色、布局、技术约束） | 「参考配色方案（5 个）」+ CSS 变量 | **禁止** read `math-design/` 任何文件；**禁止** HTML 写入 `html-authoring:math-design` 注释；**禁止** H1=40px/按钮 80px 数学规格 |

**数学知识点触发词（命中 + 产出单页 HTML + 无步骤 1 非数学前缀 → math-design）：**
勾股定理、一次函数、二次函数、鸡兔同笼、植树问题、相遇问题、单位进率、面积周长、平均数、圆面积周长、圆柱圆锥、阴影面积、方程组消元、因式分解、概率转盘、等差数列、表内乘法、平行四边形、数轴、绝对值、质数合数、全等三角形、勾股、分数加减、面动成体等。

**⚠️ 以下词单独出现不构成数学路由**（须结合学科或数学知识点）：教学动画、互动课件、互动设计、习题配置、呈现形式、闯关游戏。

**⚠️ `call_skill` 注入的 `<skill-files>` 表格 ≠ 已读取** — 数学场景仍须主动 `read_url`/`read_file` 打开 workflow 与色板；非数学场景**不得**因表格中存在 math-design 路径而去读取。

**⚠️ math-design 附件 CDN URL** — 数学场景读取子文件时，**必须使用 `<skill-files>` 表中 CDN URL 原样 `read_url`**，禁止自行拼接 `math-design/` 路径。平台可能将附件扁平化为 `.../vN/workflow.md`、`color-palettes-a.md` 等（以 skill-files 表实际 URL 为准）。

## math-design 子文件（仅数学场景）

| 文件 | 路径 |
|---|---|
| 工作流 + 布局/字号 | [math-design/workflow.md](math-design/workflow.md) |
| 小学色板 A-01~11 | [math-design/color-palettes-a.md](math-design/color-palettes-a.md) |
| 初高中色板 B-01~20 | [math-design/color-palettes-b.md](math-design/color-palettes-b.md) |
| 演示区多色强化 | [math-design/visual-impact.md](math-design/visual-impact.md) |
...
### 1. 理解需求与视觉定位
- **第 0 步：学科路由**（见上方「学科路由」章节）
  - **数学** → 声明 `palette_id` → 用 `<skill-files>` 表 CDN URL 原样 `read_url`/`read_file` 打开 workflow 及色板子文件（禁止自行拼路径）
  - **非数学** → **仅使用 SKILL.md 主体**；**跳过** math-design 目录；在推理中声明「走通用配色」
- 确认学科、学段、核心知识点
...
- **内容组织模式**（**先于布局决定**，见"内容组织模式"章节）：
  ...
  - 禁止直接把大量内容摊平成一条滚动长页
- **配色方案**（**按学科路由二选一，禁止混用**）：
  - **数学场景**：按 [math-design/workflow.md](math-design/workflow.md) Phase 2–4 **机械抽选** 1 套 A/B 色板（Phase 3 优先查表，未命中走 codepoint hash，A-01/B-12 与其它编号同权）→ `read_file` 对应段落 + visual-impact.md → 将 hex 映射为 `--primary` 等 CSS 变量；**Phase 5 硬约束**：HTML **第一行**（`<!DOCTYPE html>` 之前）必须写入 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`，缺则 `edit_file` 补上后才可 `terminate`
  - **非数学场景**：从下方「参考配色方案（5 个）」中选择或自定义；输出 `primary / secondary / accent / background / foreground` 5 个 CSS 变量；**HTML 首行必须是 `<!DOCTYPE html>`，禁止任何 `html-authoring:math-design` 注释**
- **布局结构**：左右结构（控制器 + 展示区）或上下结构（说明 + 演示）；**数学场景**优先 math-design 一屏公式（Header + Stage + Controls）
- **排版**：
  - **数学场景**：H1=40px、H2=30px、Body=28px、按钮 80px（见 math-design/workflow.md 第三节，禁止 42px）
  - **非数学场景**：最多 2 个字体家族；标题为正文 1.5–2 倍；行高 1.4–1.6
- **动画类型**：按"动画效果选择指南"选库
...
### 4. 生成或修改 HTML
- 首次创建 → `create_file`
- **数学场景 create_file 后必做（Phase 5 硬约束）**：HTML **第一行**必须是 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`（缺则 `edit_file` 补首行）；自检 `#controls` 无 `overflow-y:auto|scroll`；自检 Stage 居中 CSS 存在；满足后才可 `terminate`
- **非数学场景 create_file 后必做（硬约束）**：HTML **第一行**必须是 `<!DOCTYPE html>`；若含 `html-authoring:math-design` → **`edit_file` 删除该注释** → 再自检 → 才可 `terminate`
- **修改已有 HTML（必做）**：
...
> **学科分流：** 数学场景 → 仅遵循 math-design 子链路（31 套 A/B 色板），**禁止**使用本节「参考配色方案」。非数学场景 → 使用本节通用规则，**禁止** read math-design 目录。

**非数学 — 通用规则：**
- 根据学科选：科学-蓝绿，语文-暖色，物理/化学-沉稳色系（**数学已走 math-design，不在此列**）
...
**数学 — math-design 规则（摘要，完整见 workflow.md）：**
- 小学 1–6 → pool A（**A-01 ~ A-11**，11 套）；初中高中 → pool B（**B-01 ~ B-20**，20 套）
- Phase 3 **机械抽选**（所有编号同权；A-01 / B-12 与其它编号一样可被选中也可不选）：
  1. **优先查表**（workflow.md §Phase 3 Step 2）：30 个常见知识点→预算定 palette，命中即采用
  2. **算式兜底**：`hash = ord(keyword[0])×7 + ord(keyword[-1])×5 + len(prompt)`，`palette_id = X-{(hash mod N)+1:02d}`
  3. 推理中显式输出 `keyword / pool / source / palette_id` 四项，**禁止跳过**
- 布局变体（**L1底栏 / L2左栏 / L3右栏**）按 `(ord(keyword[0]) + len(prompt)) mod 3` 选定，详见 workflow.md §2.2；**严禁未声明算式直接落 L1**
- Stage 必须 flex 居中（`align-items:center; justify-content:center`），演示主体居中渲染，详见 workflow.md §2.3
- 坐标系/坐标轴场景默认**无背景方格线**：平面坐标系、函数图、数轴、直方图、柱状图、折线统计图等凡是绘制 x/y 轴或图表轴线的页面，只画轴线、短刻度、标签、曲线/柱形/数据图形和必要语义辅助线；只有用户原文明确要求“方格线/网格线/坐标网格/坐标纸/方格纸/参考线/辅助线/方便学生读点”时才可画完整方格或图表参考线
- **意图判定优先**：用户只说“平面直角坐标系/坐标系/坐标轴/坐标平面/点坐标/读坐标/在坐标系中演示点或函数”不等于要求方格，必须判为 `grid_intent=default_no_grid`；测试和修复也必须按默认无方格分支执行，禁止把 must-cover 写成“坐标方格可见性”
- 未显式要求方格/网格时，不允许把“切换网格/显示网格”作为附加功能：不得默认生成隐藏或可切换的 `gridLayer`、`.grid-line`、`toggleGrid()`、`gridVisible` 或全宽全高横竖线循环；需要帮助读点时用轴线短刻度、刻度标签、交点/点位标签解决
- 显式要求**坐标方格**（如“带方格线的平面直角坐标系/坐标网格/坐标纸/方格纸/方便读点”）时，必须生成横线和竖线都真实渲染的二维方格，不能只画水平参考线；统计图仅显式要求“参考线/辅助线/方便读数”时可按读数需要绘制单向参考线，但不能冒充坐标方格
- 显式要求坐标方格/参考线时，方格/参考线必须可见并与坐标轴、刻度、标签、曲线/柱形共用同一 `unit/origin` 或图表比例尺；平面直角坐标系/坐标纸必须让 `originX/originY` 落在网格交点上，即 `(originX-gridStartX) % unit == 0` 且 `(originY-gridStartY) % unit == 0`（容差 ≤0.5px），禁止 `GRID_SIZE=600, UNIT=40, ORIGIN=300` 这类轴线夹在两条网格线中间的组合；坐标轴本身也必须位于 SVG `viewBox`/Canvas/plot 可视范围内，不得把 x 轴或 y 轴裁掉
- 未要求时不得在 Stage/plot/SVG/Canvas 使用 `background-image`、成对 `linear-gradient`、`repeating-linear-gradient`、SVG `<pattern>`、`initGrid()`/`drawGrid()` 或全宽全高横竖线循环生成背景网格；显式要求时必须在 `test_html` 里同时验证方格可见性和轴线-网格对齐
- 显式方格线必须肉眼可见且通过 `test_html` 的样式断言：网格线 computed stroke 与 Stage/plot 背景 `contrast >= 1.5` 或 `maxDelta >= 32`，`stroke-width >= 1.25px`，`opacity >= 0.65`；禁止 `#C5BFBF` 配 `#CBCBCB`、`#DDE3CA` 配 `#EBEDD4` 这类近灰/近浅色导致“有 DOM 但看不见”；测试脚本只数 `grid_count > 0` 不合格，必须计算 `contrast` 或 `maxDelta`，并断言网格线是有效 SVG 元素、源码/DOM 不含 `[unsafe-url-removed]`
- 默认无方格场景若测试发现 `.grid-line`、`#grid-layer`、`initGrid()` 或全幅网格线，修复方向是删除方格和相关测试断言，只保留坐标轴/短刻度/标签；禁止通过加深 `--grid-color`、提高 opacity 等方式让默认方格“通过测试”
- 统计图不要删轴：直方图、柱状图、折线统计图必须保留清晰的图表坐标轴、轴标签、短刻度、刻度/分箱标签和数据标签；`grid-lines` 命名可用于短刻度，但未显式要求时不得铺 Stage 方格背景或贯穿绘图区的全幅参考线
- **Controls 禁滚动**：`#controls` 不得设 `overflow-y:auto|scroll`；超量控件用 Tabs/Accordion 折叠（详见 workflow.md §2.2）
- HTML 首行：`<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`
- Phase 4 必须 `read_file` 对应色板段落；演示区遵循 visual-impact.md（多色利用率 ≥90%）
- 所有背景/边框/按钮/演示区颜色必须来自选定色板 hex，禁止 Tailwind 默认蓝 `#3B82F6` 等外源色
...
4. **外部库引用**：常用库（Tailwind / Anime.js / GSAP / p5.js / Three.js + OrbitControls / MathJax / Core.js）的自定义 CDN URL 由 System Prompt "HTML 产物硬约束 → 外部库 CDN 白名单"统一规定，必须直接复用其中给出的 URL，禁止改写成 cdnjs / jsdelivr 等公共源；未列出的库才允许使用公共 CDN。**严禁引入白名单与公共 CDN 都不存在的、自己想象出来的库或路径**
5. **CSS 兼容性**：所有现代 CSS 特性必须添加 `-webkit-` 前缀以支持 Chrome >= 63
6. **响应式**：< 768px 使用 rem 缩放，>= 768px 撑满视口
7. **数学表达式**：使用 LaTeX 标准语法（MathJax 3 渲染），行内公式定界符**必须**用 `\(...\)`、块级公式用 `\[...\]`，**严禁** `$...$` 或 `$$...$$`（详见下方"数学公式渲染规范（MathJax 3）"；CDN URL 见 System Prompt 白名单）
8. **JS 中文引号**：含中文字符串用模板字符串或转义，禁止裸引号嵌套

### CDN 资源使用（注入位置与加载规则）

**CDN URL 不在本 skill 重复定义** —— 常用库（Tailwind / Anime.js / GSAP / p5.js / Three.js + OrbitControls / MathJax / Core.js）的自定义 CDN URL 统一由 System Prompt "HTML 产物硬约束 → 外部库 CDN 白名单"声明，请直接从那里复用 URL，本 skill 只负责说明**单页 HTML 场景的注入位置与加载规则**：

**注入位置：**
- 所有 `<script>` 直接放在 `<head>` 中（多页 PPT 课件的注入方式不同，由 `courseware-generator` 管辖）
- Tailwind 优先加载，其余库按需引入

**未列出的库（如 D3.js、ECharts、HanziWriter 等）**可使用公共 CDN（cdnjs / jsdelivr），示例：
...
定界符与基本规范见 System Prompt "数学公式" 与 "HTML 产物硬约束 → 三条通用底线"，本节补充 html-authoring 场景下的具体落点：

- **CDN**：复用 System Prompt 白名单中 MathJax 3 的 URL，放入 `<head>` 中
- **适用范围**：HTML 中**所有**数学表达式、变量、符号都必须用 `\(...\)` / `\[...\]` 包裹（包括标题、说明文本、按钮文案、tooltip 等任何用户可见位置）
- 正确示例：`若 \(a=b\)`、`\(x+1=6\)`、`\(\pm\sqrt{10}\)`、`\[\int_0^1 x^2 \, dx\]`
- 错误示例：`若 $a=b$`、`$x+1=6$`、`$\pm\sqrt{10}$`、`$$\int_0^1 x^2 \, dx$$`
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；Phase 3 推理已声明 `keyword / pool / source(table|hash) / palette_id`；布局推理已声明 `layout_hash` 数字且与算式一致（禁止默认 L1）；Stage `display:flex; align-items:center; justify-content:center`；H1=40px；按钮 80px；若页面实际绘制坐标系/坐标轴/图表轴线（函数图、直方图、柱状图、折线统计图等）：先从用户原文声明 `grid_intent`，且“平面直角坐标系/坐标系/坐标轴/点坐标/在坐标系中演示”本身必须判为 `default_no_grid`；用户未明确要求“方格线/网格线/坐标网格/坐标纸/方格纸/参考线/辅助线/方便学生读点”时，Stage/plot/SVG/Canvas 不得出现 CSS `background-image`/`linear-gradient`/`repeating-linear-gradient`/SVG `pattern`、`gridLayer`/`#grid-layer`、`.grid-line`、`initGrid()`/`drawGrid()`/`createGrid*()`、`toggleGrid()`/`gridVisible` 或贯穿绘图区的全幅网格/参考线，测试 must-cover 不得写“方格可见性/网格对比度”；必须保留且可视化完整呈现坐标轴/图表轴、短刻度、刻度/轴标签、曲线/柱形/数据标签，坐标轴不得被 `viewBox`/Canvas 边界裁掉；用户明确要求坐标方格时必须生成可见且与轴/刻度/比例尺对齐的二维方格（横竖方向都真实渲染，DOM/source 不含 `[unsafe-url-removed]`），用户仅要求统计参考线时按图表读数需要生成参考线；`test_html` 必须按分支断言：默认无方格且轴存在并位于可视范围，显式有方格/参考线且轴存在，且显式方格线 `contrast >= 1.5` 或 `maxDelta >= 32`、`stroke-width >= 1.25px`、`opacity >= 0.65`，平面坐标纸还必须断言 x/y 轴落在网格线上（轴到最近同向网格线距离 ≤0.5px）；默认无方格场景若出现方格，修复是删除方格而不是加深方格颜色；非坐标轴场景不执行本检查；**`#controls` 未设 `overflow-y:auto|scroll`**；`<body>` 仅允许 `overflow:hidden`
- [ ] **非数学场景**：**未** read math-design 目录；HTML **第一行**为 `<!DOCTYPE html>` 且**不含** `html-authoring:math-design`；至少 3 个 CSS 变量；未使用 H1=40px/按钮 80px 数学规格
...
- [ ] 白名单内库（Tailwind / Anime.js / p5.js / GSAP / Three.js + OrbitControls / MathJax / Core.js）全部使用 System Prompt "HTML 产物硬约束 → 外部库 CDN 白名单"中列出的自定义 CDN 地址
...
### 案例 F：非数学误写 math-design 注释（phy-02 类）
- **现象**：初中物理 HTML 首行写了 `<!-- html-authoring:math-design palette=B-05 -->`，但未 read math-design
- **根因**：Agent 将数学交付格式套用到非数学场景
- **修复**：按非数学硬约束删除注释；改用「参考配色方案」；禁止 read math-design
...
- 学段：初中 → pool B
- Phase 3 Step 2 查表：`keyword=勾股定理` → 命中 → **palette_id=B-04**
- 抽选 ┃ keyword=勾股定理  pool=B  source=table  palette_id=B-04
- 布局：`ord("勾")=21246`，`len(prompt)=25` → `layout_hash = (21246+25) mod 3 = 21271 mod 3 = 1` → **L2 左栏**
- Phase 4：`read_file` color-palettes-b.md 中 B-04 段落 + visual-impact.md

**Agent 执行：** `create_file`，首行 `<!-- html-authoring:math-design palette=B-04 layout=L2 -->`；按 §2.4 L2 骨架渲染。
...
**Agent 推理：**
- pool=A（小学）；Phase 3 查表：`鸡兔同笼` → **palette_id=A-06**
- 抽选 ┃ keyword=鸡兔同笼  pool=A  source=table  palette_id=A-06
- 布局：`ord("鸡")=40481`，`len(prompt)=13` → `(40481+13) mod 3 = 40494 mod 3 = 0` → **L1 底栏**

**Agent 执行：** 首行 `<!-- html-authoring:math-design palette=A-06 layout=L1 -->`；按 §2.4 L1 骨架。

---

**示例 M-3：知识点不在表中 → 算式兜底**

**用户：** "做一个余弦定理交互动画"（余弦定理 不在 30 条表内）

**Agent 推理：**
- pool=B；Phase 3 Step 2 未命中 → Step 3 算式：
  - `c1 = ord("余") = 20313`，`c2 = ord("理") = 29702`，`len(prompt) = 11`
  - `hash = 20313×7 + 29702×5 + 11 = 142191 + 148510 + 11 = 290712`
  - `palette_id = B-{(290712 mod 20)+1:02d} = B-{12+1:02d} = B-13`
- 抽选 ┃ keyword=余弦定理  pool=B  source=hash  palette_id=B-13
- 布局：`(20313+11) mod 3 = 20324 mod 3 = 2` → **L3 右栏**

**Agent 执行：** 首行 `<!-- html-authoring:math-design palette=B-13 layout=L3 -->`；按 §2.4 L3 骨架。
...
**Agent 第 1 轮（推理）：**
- **学科路由：物理（步骤 1）→ 通用**；**禁止** read `math-design/`；**禁止** palette 注释
- 设计决策：沉稳橙（`primary=#FF5600`…）；左右结构；Anime.js

（工具执行完成）

**Agent 第 2 轮：** `create_file`，首行 `<!DOCTYPE html>`（无 math-design 注释），按参考配色生成页面。
