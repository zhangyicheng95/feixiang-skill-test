...
- `grid.type === "GridHelper"` 或地面线段对象存在；若使用 `GridHelper`，`grid.material.opacity >= 0.55`，`grid.position.y > ground.position.y`，地面材质 `transparent=true` 且 `opacity <= 0.25`。
...
- 禁止相机/控制器允许转到地面以下导致地面遮挡模型；设置 `controls.maxPolarAngle < Math.PI / 2`，并保持 `controls.enablePan=false`。
- HTML 必须暴露 `window.__math3d`，供 `test_html` 断言 `hasGroundGrid`、`grid.type`、`grid.material.opacity`、`ground.material.opacity` 和 CSS 背景。
