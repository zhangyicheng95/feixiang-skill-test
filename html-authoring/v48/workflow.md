...
- `test_html` 必须按 `grid_intent` 分支写断言：默认无方格时断言没有 `grid-layer/.grid-line/renderGrid/drawGrid` 且轴线/数据存在；显式坐标方格时断言横竖网格均存在、可见、轴线落在网格线上；数学 3D 时断言存在可测试的 3D 地面网格对象，而不是只检查 canvas。
