...
- **第 0 步：学科路由**（见上方「学科路由」章节）
  - **数学** → 声明 `palette_id` → `read_url`/`read_file` 打开 `math-design/workflow.md` 及色板子文件
...
- **内容组织模式**（**先于布局决定**，见"内容组织模式"章节）：
  ...
  - 禁止直接把大量内容摊平成一条滚动长页
- **配色方案**（**按学科路由二选一，禁止混用**）：
  - **数学场景**：按 [math-design/workflow.md](math-design/workflow.md) Phase 2–4 随机选 1 套 A/B 色板 → `read_file` 对应段落 + visual-impact.md → 将 hex 映射为 `--primary` 等 CSS 变量；**Phase 5 硬约束**：HTML **第一行**（`<!DOCTYPE html>` 之前）必须写入 `<!-- html-authoring:math-design palette=XX -->`，缺则 `edit_file` 补上后才可 `terminate`
...
- 小学 1–6 → pool A（11 套高饱和）；初中高中 → pool B（20 套低饱和）
- Phase 3 均匀随机 1 套，禁止固定 A-01
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=XX -->`；H1=40px；按钮 80px
...
- 学段：初中 → pool B；Phase 3 随机选定 **B-12**
- Phase 4：`read_file` color-palettes-b.md 中 B-12 段落 + visual-impact.md
- 配色：将 B-12 hex 映射为 CSS 变量（禁止参考配色「沉稳橙」）
- 排版：H1=40px，按钮 80px；布局 Header+Stage+Controls

**Agent 执行：** `create_file` 生成 HTML，首行注释 `<!-- html-authoring:math-design palette=B-12 -->`
