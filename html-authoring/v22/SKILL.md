...
- **内容组织模式**（**先于布局决定**，见"内容组织模式"章节）：
  ...
  - 禁止直接把大量内容摊平成一条滚动长页
- **配色方案**（**按学科路由二选一，禁止混用**）：
  - **数学场景**：按 [math-design/workflow.md](math-design/workflow.md) Phase 2–4 **机械抽选** 1 套 A/B 色板（Phase 3 算式，禁 A-01/B-12）→ `read_file` 对应段落 + visual-impact.md → 将 hex 映射为 `--primary` 等 CSS 变量；**Phase 5 硬约束**：HTML **第一行**（`<!DOCTYPE html>` 之前）必须写入 `<!-- html-authoring:math-design palette=XX -->`，缺则 `edit_file` 补上后才可 `terminate`
...
- 小学 1–6 → pool A（**抽选 A-02~A-11**，共 10 套）；初中高中 → pool B（**抽选 B-01~B-20 除 B-12**，共 19 套）
- **A-01 与 B-12 永久禁选**（同一灰黄视觉 #F3F3F3+#FFDF5E）
- Phase 3 **机械抽选**：`seed = (len(prompt)×17 + len(keyword)) mod len(eligible)`，推理中须写出算式，禁止主观改选
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=XX -->`；**palette 不是 A-01 / B-12**；Phase 3 算式已在推理中写出；H1=40px；按钮 80px
...
- 学段：初中 → pool B
- Phase 3 机械抽选（**禁止照搬本示例编号**）：
  - keyword=`勾股定理`(len=4)，prompt len=25（「做一个勾股定理证明教学动画，输出单页互动 HTML」）
  - eligible = B-01~B-11, B-13~B-20（19 套，已排除 B-12）
  - seed = (25×17 + 4) mod 19 = 429 mod 19 = **11** → eligible[11] = **B-13**
- Phase 4：`read_file` color-palettes-b.md 中 **B-13** 段落 + visual-impact.md
- 配色：将 B-13 hex 映射为 CSS 变量（禁止参考配色「沉稳橙」）
- 排版：H1=40px，按钮 80px；布局 Header+Stage+Controls

**Agent 执行：** `create_file` 生成 HTML，首行注释 `<!-- html-authoring:math-design palette=B-13 -->`（须与 Phase 3 算式结果一致，**不得写 B-12 / A-01**）
