...
要求：
- **Phase 3 机械抽选池**：仅 **A-02 ~ A-11**（共 10 套）；**A-01 永久禁选**（与 B-12 灰黄视觉重复，见 workflow.md Phase 3）
- 禁止主观挑选、禁止照搬 skill 示例中的 palette 编号

###A【活力高饱和】色彩体系，共11种适配于小学阶段知识点（1-6年级）。**抽选时跳过 A-01**，从 A-02~A-11 中按 workflow Phase 3 算式选取。

> ⛔ **A-01 禁选**：与 B-12 视觉完全重复（#F3F3F3 + #FFDF5E），不得选用。下列定义仅作归档，Phase 3 不可抽中。
