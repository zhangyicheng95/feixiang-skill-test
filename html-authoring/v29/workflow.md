...
Phase 5 ┃ 交付硬约束（**缺一即失败，禁止 terminate**）
  ...
  ⑨ **无整图方格背景**：数学场景默认禁止在 `#stage`、`.stage`、`.stage-area`、`.plot-area`、`.chart-area`、`.svg-container`、Canvas/SVG 容器上用 CSS 背景生成方格纸；出现 §1.1 任一命中项 → 必须 edit_file 删除后再 terminate
```
...
- 在 `#stage`、`.stage`、`.stage-area`、`.plot-area`、`.chart-area`、`.svg-container`、Canvas/SVG 外层容器上写 `background-image` 生成网格
- 成对 CSS 背景网格：`linear-gradient(... 1px, transparent 1px)` + `linear-gradient(90deg, ... 1px, transparent 1px)` + `background-size: <n>px <n>px`
- `repeating-linear-gradient`、SVG `<pattern>`、名为 `grid`/`grid-line` 的全屏双向背景层、覆盖全宽全高的 Canvas/SVG 横竖线循环
...
- 未显式要求坐标方格时，坐标图代码中禁止出现：用于坐标背景的 CSS `background-image` 网格、成对 `linear-gradient`、`repeating-linear-gradient`、SVG `<pattern>` 方格底纹、Canvas/SVG 中覆盖全宽全高的 `drawGrid()` / 全量横竖线循环；此处不禁止 CSS 布局 `display:grid`
