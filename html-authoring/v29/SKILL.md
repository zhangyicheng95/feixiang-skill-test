...
- 所有数学场景默认禁止整页/整图方格背景：不得在 `#stage/.stage/.plot-area/.chart-area/.svg-container` 上用 `background-image`、成对 `linear-gradient` + `background-size`、`repeating-linear-gradient`、SVG `<pattern>` 或全屏横竖线循环生成方格纸；统计图只允许绘图区内部少量水平参考线/分箱线
