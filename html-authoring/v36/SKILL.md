...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；Phase 3 推理已声明 `keyword / pool / source(table|hash) / palette_id`；布局推理已声明 `layout_hash` 数字且与算式一致（禁止默认 L1）；Stage `display:flex; align-items:center; justify-content:center`；H1=40px；按钮 80px；若页面实际绘制坐标系/坐标轴/图表轴线（函数图、直方图、柱状图、折线统计图等）：用户未明确要求“方格线/网格线/坐标网格/坐标纸/方格纸/参考线/辅助线/方便学生读点”时，Stage/plot/SVG/Canvas 不得出现 CSS `background-image`/`linear-gradient`/`repeating-linear-gradient`/SVG `pattern`、`toggleGrid()`/`gridVisible` 或贯穿绘图区的全幅网格/参考线；必须保留且可视化完整呈现坐标轴/图表轴、短刻度、刻度/轴标签、曲线/柱形/数据标签，坐标轴不得被 `viewBox`/Canvas 边界裁掉；用户明确要求坐标方格时必须生成可见且与轴/刻度/比例尺对齐的二维方格（横竖方向都真实渲染，DOM/source 不含 `[unsafe-url-removed]`），用户仅要求统计参考线时按图表读数需要生成参考线；`test_html` 必须按分支断言：默认无方格且轴存在并位于可视范围，显式有方格/参考线且轴存在，且显式方格线 `contrast >= 1.5` 或 `maxDelta >= 32`、`stroke-width >= 1.25px`、`opacity >= 0.65`，平面坐标纸还必须断言 x/y 轴落在网格线上（轴到最近同向网格线距离 ≤0.5px）；显式方格测试若只检查 `grid_count > 0` 而未计算 `contrast/maxDelta`、未检查有效 SVG namespace 或未检查轴网格对齐，不得通过自检；非坐标轴场景不执行本检查；**`#controls` 未设 `overflow-y:auto|scroll`**；`<body>` 仅允许 `overflow:hidden`
