---
name: html-authoring
description: 生成或修改单页教学交互 HTML（动画、单页课件、教学海报、教学网页、可打印练习册/字帖）时使用，覆盖视觉设计决策与技术实现规范（配色布局、动画选型、浏览器兼容、媒体资源）。K12 数学单页 HTML 内置 math-design 子链路（31 套 A/B 色板、三向布局、Stage 自适配、侧栏滚动与多视口溢出检测）；数学场景走 math-design，非数学场景禁止读 math-design 子文件。不适用于多页 PPT 课件、纯知识问答、不涉及 HTML 的场景。
---
...
|---|---|---|---|---|
| **数学 → math-design** | 命中数学知识点（见下）；且无步骤 1 的非数学学科前缀 | **必读** `math-design/workflow.md` → 色板子文件 → `visual-impact.md`（须 `read_file`/`read_url`，见下） | 31 套 A/B 色板；三向布局；Stage 自适配；首行 palette 注释 | 禁止「参考配色方案」；禁止用裁切掩盖溢出 |
...
- **排版**：
  - **数学场景**：H1 基准 40px；L1 主按钮目标 80px，空间不足时按 workflow 使用 `clamp()`；L2/L3 使用紧凑控件规格
...
### 4. 生成或修改 HTML
- 首次创建 → `create_file`
- **数学场景 create_file 后必做（硬约束）**：HTML **第一行**必须是 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`（缺则 `edit_file` 补首行）；L1 Controls 不滚动，L2/L3 内容超高时使用内部可滚区并固定关键操作；自检 Stage 居中；在 1600×700、1366×768、1280×720 下检查初始、最大内容、MathJax 和动画状态均无裁切后才可 `terminate`
...
- **Controls 分级滚动**：L1 底栏禁止滚动；L2/L3 优先折叠，仍超高时必须使用 `.controls-scroll { overflow-y:auto }`，并把提交、重置等按钮放入固定的 `.controls-actions`（详见 workflow.md）
...
**视觉层（必检）：**
- [ ] **数学场景**：已读取 workflow、对应色板和 visual-impact；HTML 第一行为 math-design 注释；已声明 `keyword / pool / source / palette_id / layout_hash / layout`；Stage 已居中且主演示根节点完整位于 Stage 内；SVG 选择器命中真实节点；L1 Controls 不滚动；L2/L3 超高时内部可纵向滚动、无横向滚动且关键操作固定可见；1600×700、1366×768、1280×720 的初始、最大内容、MathJax 和动画状态均通过溢出检测
