...
- 优先使用 Tabs、Accordion、Popover 降低内容高度。
- 当侧栏在真实可视高度下仍放不完时，必须启用侧栏内部纵向滚动；禁止裁切。

### 4.3 L2/L3 侧栏滚动与触摸滑动

侧栏必须拆成“可滚内容区 + 固定操作区”。不要让整个 `#controls` 无差别滚动，否则重置、提交等收尾操作会被滚出视野。
...
.controls-scroll::-webkit-scrollbar-thumb {
  background: color-mix(in srgb, var(--primary) 70%, var(--foreground));
  border-radius: 999px;
}
```
...
- 仅 L2/L3 侧栏允许纵向滚动；L1 底栏禁止滚动。
- 滚动容器只能是 `.controls-scroll` 或等价内部节点，不能是 `body`、`main` 或 `#stage`。
- `.controls-scroll.scrollHeight > .controls-scroll.clientHeight` 时，滚轮、触控板和触摸滑动必须有效。
- 关键操作按钮必须位于 `.controls-actions`，始终可见。
- 侧栏顶部和底部应有渐隐或滚动条提示，让用户知道还有内容。
- 侧栏不得产生横向滚动条。
- 允许使用 `scroll-snap-type: y proximity`，但禁止强制吸附导致控件无法停留。
- 若滚动后仍有单个控件被截断，继续降低间距或折叠内容，不得依赖半截显示。
...
8. L1 Controls 没有滚动条；L2/L3 若内容超高，侧栏可滚且关键操作始终可见。
9. 标签、公式、输入框和按钮没有重叠。
10. 所有按钮均可用，并形成教学闭环。
...
- 禁止使用绝对定位堆叠多个大型内容块。
- 禁止 L1 Controls 滚动；L2/L3 仅允许内部 `.controls-scroll` 纵向滚动。
- 禁止在 L2/L3 使用 `overflow-y:hidden` 裁掉侧栏内容。
- 禁止空按钮、无反馈操作和无法重置的动画。
- 禁止未通过三种视口检查就交付。
