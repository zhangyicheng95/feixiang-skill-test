...
- 显式要求坐标方格/参考线时，方格/参考线必须可见并与坐标轴、刻度、标签、曲线/柱形共用同一 `unit/origin` 或图表比例尺；平面直角坐标系/坐标纸必须让 `originX/originY` 落在网格交点上，即 `(originX-gridStartX) % unit == 0` 且 `(originY-gridStartY) % unit == 0`（容差 ≤0.5px），禁止 `GRID_SIZE=600, UNIT=40, ORIGIN=300` 这类轴线夹在两条网格线中间的组合
...
- 显式方格线必须肉眼可见且通过 `test_html` 的样式断言：网格线 computed stroke 与 Stage/plot 背景 `contrast >= 1.5` 或 `maxDelta >= 32`，`stroke-width >= 1.25px`，`opacity >= 0.65`；禁止 `#C5BFBF` 配 `#CBCBCB`、`#DDE3CA` 配 `#EBEDD4` 这类近灰/近浅色导致“有 DOM 但看不见”；测试脚本只数 `grid_count > 0` 不合格，必须计算 `contrast` 或 `maxDelta`
