...
- `test_html`/自检必须按意图分支：坐标系/坐标轴场景且未要求方格/参考线时，断言 Stage/plot 容器 computed `backgroundImage` 不含 `linear-gradient`/`repeating-linear-gradient`，无贯穿绘图区的全幅网格/参考线，无 `toggleGrid()`/`gridVisible`/“切换网格”这类可切换网格入口，同时坐标轴/图表轴和轴标签存在；明确要求方格/参考线时，断言方格/参考线存在、横竖方向都有、坐标轴存在且轴线比方格线更醒目，并断言网格线 computed stroke 相对 Stage/plot 背景 `contrast >= 1.5` 或 `maxDelta >= 32`、`stroke-width >= 1.25px`、`opacity >= 0.65`，平面坐标纸还必须断言轴线落在网格线上；非坐标轴场景不执行本分支
