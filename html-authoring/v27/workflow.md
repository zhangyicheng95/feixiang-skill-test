...
Phase 5 ┃ 交付硬约束（**缺一即失败，禁止 terminate**）
  ...
  ⑧ 平面直角坐标系闸门：仅当实际绘制平面直角坐标系/函数坐标图时执行 §2.3.1；统计直方图、柱状图、条形图等图表内参考线不按“坐标系背景方格”处理
```
...
- **默认不铺整页方格纸背景**：色板中的“背景网格”只提供可选视觉语言，不能自动变成 Stage 底纹；统计直方图、柱状图、条形图、表格等可以使用图表/模块内部参考线，但这些线必须属于具体图表或模块，不能覆盖整个 Stage 形成独立背景方格纸
...
- 坐标系场景默认不铺独立背景方格纸；色板段落里的“背景网格”不能作为全局默认背景使用
- 坐标系方格必须服务坐标读数，不能只是装饰；若无法保证与坐标轴精确对齐，**宁可只画坐标轴、刻度和必要参考线，不画方格背景**
...
- 优先用同一个 SVG / Canvas 绘制方格、轴、刻度、曲线；禁止用独立 CSS `repeating-linear-gradient` 当坐标系背景，除非 `background-size` 与 `background-position` 严格绑定同一个 `unit/origin`
- 坐标系方格颜色仍必须来自选定 palette 的浅色/边框色变量，不能引入外源灰蓝、默认蓝等非 palette 颜色
