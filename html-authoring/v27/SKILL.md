...
- 平面直角坐标系/函数坐标图才执行坐标方格约束：默认不铺独立方格纸背景；若出现坐标方格，方格/轴/刻度/标签/曲线必须共用同一 `unit/origin` 坐标变换，不能保证则不画方格。统计直方图、柱状图、条形图等图表内部参考线不受此条禁止
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；Phase 3 推理已声明 `keyword / pool / source(table|hash) / palette_id`；布局推理已声明 `layout_hash` 数字且与算式一致（禁止默认 L1）；Stage `display:flex; align-items:center; justify-content:center`；H1=40px；按钮 80px；若绘制平面直角坐标系/函数坐标图，默认不铺独立方格纸背景，需方格时方格/轴/刻度/标签/曲线共用同一 `unit/origin`；统计直方图、柱状图、条形图等图表内部参考线不按坐标系背景方格处理；**`#controls` 未设 `overflow-y:auto|scroll`**；`<body>` 仅允许 `overflow:hidden`
