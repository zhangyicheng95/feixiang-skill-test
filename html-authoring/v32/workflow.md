...
- 方格/参考线必须肉眼可见：颜色应明显区别于 Stage 背景，不要用与背景过近的浅色；必要时提高 `opacity` 到 `0.45` 以上或使用更深的同色系浅线
- `test_html`/自检必须验证：方格/参考线数量 > 0；坐标轴/图表轴存在；方格/参考线与轴线不应同色同粗
...
- **显式要求才画坐标方格/参考线**：用户原文出现“方格线 / 网格线 / 坐标网格 / 坐标纸 / 方格纸 / 网格背景 / 带网格坐标系 / 参考线 / 辅助线 / 方便学生读点”等要求时，才绘制完整坐标方格或图表参考线，且必须可见、与坐标轴/比例尺对齐
...
- `test_html`/自检必须按意图分支：坐标系/坐标轴场景且未要求方格/参考线时，断言 Stage/plot 容器 computed `backgroundImage` 不含 `linear-gradient`/`repeating-linear-gradient`，无贯穿绘图区的全幅网格/参考线，同时坐标轴/图表轴和轴标签存在；明确要求方格/参考线时，断言方格/参考线存在、坐标轴存在且轴线比方格线更醒目；非坐标轴场景不执行本分支
