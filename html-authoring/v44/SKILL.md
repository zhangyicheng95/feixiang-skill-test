...
|---|---|---|---|---|
| **数学 → math-design** | 命中数学知识点（见下）；且无步骤 1 的非数学学科前缀 | **必读** `math-design/workflow.md` → 色板子文件 → `visual-impact.md`；涉及平面直角坐标系/x-y 坐标轴/函数图像时加读 `manipulatives.md`（须 `read_file`/`read_url`，见下） | 31 套 A/B 色板；H1=40px；按钮 80px；首行 palette 注释；标准坐标轴组件 | 禁止「参考配色方案」；禁止 math-design 以外的配色决策 |
...
| 标准数学教具样式 | [math-design/manipulatives.md](math-design/manipulatives.md) |
...
- **第 0 步：学科路由**（见上方「学科路由」章节）
  - **数学** → 声明 `palette_id` → 用 `<skill-files>` 表 CDN URL 原样 `read_url`/`read_file` 打开 workflow 及色板子文件；涉及平面直角坐标系/x-y 坐标轴/函数图像时加读 `manipulatives.md`（禁止自行拼路径）
  - **非数学** → **仅使用 SKILL.md 主体**；**跳过** math-design 目录；在推理中声明「走通用配色」
- 确认学科、学段、核心知识点
- 需求模糊时按教学经验选最合理默认继续，**禁止 `ask_user`**；在最终回复里以"默认按 X 处理，如需调整请告知"声明假设

### 1.5 核心机制契约（可玩 / 有核心功能类产物，**create_file 前必做，先于设计决策**）

产物若是**游戏 / 答题 / 模拟器 / 互动演示 / 计算器**等"有核心可玩或可用闭环"的类型，
**动手写代码前先声明该产物的「核心机制契约」——即最小可玩闭环**，并把它当作"做完"的硬标准（Definition-of-Done），
**而不是留给测试阶段去发现**。只做外壳（有 UI、无核心闭环）= 失败交付。
...
| 拖拽配对 | 可拖 → 命中判定 → 提交/检查 → 对错反馈 → 重置 |
| 答题闯关 | 出题 → 作答 → 判分 → 反馈 → 下一题 |
| 模拟器 / 实时动画 | 开始/暂停/重置 + 实时状态显示（与下方"模拟器控制 UI"规则一致） |

**硬约束：**
- 上述类型，**未在推理中显式声明核心闭环契约 → 不得 `create_file`**；
- 契约里每个环节**都要由代码真实实现**（如 24 点必须有可点击的 +−×÷ 运算符并参与计算，不能只画数字）；
- **契约项未全部落地 → 不得 `terminate`**；
- 把契约写入产物 spec（见第 4 步 spec 注释）与 `task_write` 的 `constraints`，供测试阶段按**同一契约**验收（对应 test-html 模板 6.3）。

> 治的是真实失败 #14134：交付了"连运算符都没有"的 24 点游戏外壳。核心玩法是**设计契约（前置）**，
> 测试只负责验收它有没有被良好实现——不要反过来靠测试去发现核心玩法缺失。
...
- **核心功能 / 可玩类产物必做（spec 注释）**：在 `<head>` 内加一行机读 spec 注释（**不占首行**，避免与上面的首行规则冲突），承载 §1.5 的可验收契约，供 test-html 读回逐条机检：
  `<!-- spec: type=<品类>; core-loop=<最小可玩闭环>; forbid=<禁止项,如 box-shadow,blue,green>; require=<必含项,如 watermark:飞象样例v3>; count=<份数> -->`
  spec 偏大时**外置**为独立 spec 资源，注释里只写 `<!-- spec-ref: resourceId=<id> -->`；任务级契约（count、共用硬约束、核心闭环）**同时写入 `task_write` 的 `spec` 字段**（专门承载验收契约、独立 cap、不与 constraints 抢容量、每轮必注入；契约大则填 `specRef=resourceId`，跨轮抗压缩）。
- **修改已有 HTML（必做）**：
...
- **标准坐标轴**：平面直角坐标系、x/y 坐标轴、函数图像坐标轴必须读取 `math-design/manipulatives.md`，使用同一 SVG 内的标准轴线、箭头与 x/y 字母；放大/缩小时只缩放整个根 SVG，不得单独移动 x/y 字母；标准轴色 `#251F20` 是 palette 色彩规则的特例
- HTML 首行：`<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`
- Phase 4 必须 `read_file` 对应色板段落；演示区遵循 visual-impact.md（多色利用率 ≥90%）
- 除 `manipulatives.md` 标准坐标轴固定色 `#251F20` 外，所有背景/边框/按钮/演示区颜色必须来自选定色板 hex，禁止 Tailwind 默认蓝 `#3B82F6` 等外源色
...
- [ ] **核心机制契约（可玩/有核心功能类，§1.5）**：已显式声明最小可玩闭环且逐环节由代码实现（如 24 点的 +−×÷ 运算符可点击并参与计算，非只画数字）；已写入 `<head>` spec 注释 + `task_write` 的 `spec` 字段；不是只做外壳

**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact；涉及平面直角坐标系/x-y 坐标轴/函数图像时还要读取 manipulatives）；HTML **第一行**为 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；Phase 3 推理已声明 `keyword / pool / source(table|hash) / palette_id`；布局推理已声明 `layout_hash` 数字且与算式一致（禁止默认 L1）；Stage `display:flex; align-items:center; justify-content:center`；H1=40px；按钮 80px；若绘制二维坐标轴，x/y 字母必须和轴线位于同一 `.standard-axis` SVG 且随根 SVG 整体缩放；**`#controls` 未设 `overflow-y:auto|scroll`**；`<body>` 仅允许 `overflow:hidden`
