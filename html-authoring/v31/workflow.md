...
Phase 5 ┃ 交付硬约束（**缺一即失败，禁止 terminate**）
  ...
  ⑧ 平面直角坐标系闸门：仅当实际绘制平面直角坐标系/函数坐标图时执行 §2.3.1；未显式要求“方格线/坐标网格/方格纸/方便读点”时不画完整方格背景；显式要求时必须画可见、对齐的坐标方格
  ⑨ 统计图保护：统计直方图、柱状图、条形图、折线统计图等不套用坐标系方格禁令；不得为了避开“坐标系方格”而删除图表坐标轴、刻度、轴标签或必要参考线
```
...
- **坐标系方格只由用户意图触发**：色板中的“背景网格”只提供可选视觉语言，不能自动变成函数坐标图底纹；统计直方图、柱状图、条形图、表格等可以使用图表内部参考线和图表轴线
- 禁止标题前 Emoji；具体元素用 SVG 绘制，配色符合选定色板

### 1.1 坐标系方格开关（只适用于平面直角坐标系）

**只在实际绘制平面直角坐标系/函数图像时使用本节。** 统计直方图、柱状图、条形图、折线统计图不是本节限制对象。

当用户原文明确出现“方格线 / 网格线 / 坐标网格 / 坐标纸 / 方格纸 / 网格背景 / 带网格坐标系 / 方便学生读点”等表达时：

- 必须生成坐标方格，不能因为“默认无方格”规则删除方格
- 方格必须与坐标轴、刻度、标签、曲线共用同一 `unit/origin` 变换；x 轴、y 轴必须落在方格线上
- 方格线放在曲线/点/坐标轴下层，坐标轴在上层且更粗更深；建议 `axis stroke-width >= 2`，`grid stroke-width >= 1`
- 方格线必须肉眼可见：颜色应明显区别于 Stage 背景，不要用与背景过近的浅色；必要时提高 `opacity` 到 `0.45` 以上或使用更深的同色系浅线
- `test_html`/自检必须验证：方格线数量 > 0，横竖两方向都存在；坐标轴数量 >= 2；方格线与轴线不应同色同粗

当用户没有明确要求上述方格/网格表达时：

- 坐标图默认只画坐标轴、短刻度、刻度标签、函数曲线/点位和必要语义辅助线
- 不画覆盖整个 SVG/Canvas/Stage 的完整方格背景；不要用 CSS `background-image` / 成对 `linear-gradient` / `repeating-linear-gradient` / SVG `<pattern>` 伪装坐标方格
- 不要写 `grid-layer/initGrid/drawGrid` 等完整坐标网格生成器；如需刻度，只画轴附近短刻度线

**统计图保护（避免过度限制）：**
- 统计直方图、柱状图、条形图可以保留图表内部参考线、分箱线、x/y 轴、轴标签、刻度标签
- 坐标系方格限制不得用于删除统计图的 x/y 轴；统计图至少应有清晰的底轴/纵轴或等价的坐标标尺，轴线颜色要与背景有明显对比
- 统计图的参考线可以命名为 `grid-layer` 或 `ref-line`；不要因为出现 `grid` 字样就删除图表结构
...
- **不适用范围**：统计直方图、柱状图、条形图、折线统计图、扇形图、表格、分块卡片等图表/模块；这些场景允许图表内部参考线、分隔线、分箱线、x/y 轴和轴标签
- **坐标系默认形态**：只画坐标轴、箭头、短刻度、刻度标签、函数曲线/点位，以及必要的语义辅助线（如对称轴、区间边界、极值垂线）；默认**不画覆盖整个 SVG/Canvas/Stage 的方格纸背景**
- **显式要求必须画坐标方格**：用户原文出现“方格线 / 网格线 / 坐标网格 / 坐标纸 / 方格纸 / 网格背景 / 带网格坐标系 / 方便学生读点”等要求时，必须绘制完整坐标方格，且方格可见、与坐标轴对齐
- 未显式要求坐标方格时，坐标图代码中禁止出现：用于坐标背景的 CSS `background-image` 网格、成对 `linear-gradient`、`repeating-linear-gradient`、SVG `<pattern>` 方格底纹、`gridLayer`、`initGrid()`、`drawGrid()`、`createGrid*()`、Canvas/SVG 中覆盖全宽全高的全量横竖线循环；此处不禁止 CSS 布局 `display:grid`，也不限制统计图内部的 `grid-layer`
- 若需要刻度，只画短刻度线：x 轴刻度为 y=-4 到 y=4 的短线，y 轴刻度为 x=-4 到 x=4 的短线；禁止把默认刻度线延伸成贯穿整个绘图区的横线/竖线
...
- 坐标系方格颜色仍必须来自选定 palette 的浅色/边框色变量，不能引入外源灰蓝、默认蓝等非 palette 颜色；若用户没有要求方格且无法保证与坐标轴精确对齐，**必须删除方格，只保留坐标轴/刻度/标签**
- `test_html`/自检必须按意图分支：未要求方格时，断言坐标图无完整网格生成器；明确要求方格时，断言方格线存在、横竖方向都有、坐标轴存在且轴线比方格线更醒目；统计图则断言轴线/轴标签存在，不做“无 grid-layer”断言
