...
- 平面直角坐标系/函数坐标图默认**无完整方格纸背景**：只画坐标轴、短刻度、标签、曲线/点位和必要语义辅助线；只有用户原文明确要求“方格线/网格线/坐标网格/坐标纸/方格纸/方便学生读点”时才可画完整坐标方格
- 显式要求坐标方格时，方格必须可见并与坐标轴、刻度、标签、曲线共用同一 `unit/origin`；未要求时不得用 `background-image`、成对 `linear-gradient`、`repeating-linear-gradient`、SVG `<pattern>`、`initGrid()`/`drawGrid()` 等生成完整坐标网格
- 统计直方图、柱状图、条形图、折线统计图**不套用坐标系方格禁令**：必须保留清晰的图表坐标轴、轴标签、刻度/分箱标签；允许图表内部参考线，不因 `grid-layer`/`ref-line` 命名被删除
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=<id> layout=<L1|L2|L3> -->`；Phase 3 推理已声明 `keyword / pool / source(table|hash) / palette_id`；布局推理已声明 `layout_hash` 数字且与算式一致（禁止默认 L1）；Stage `display:flex; align-items:center; justify-content:center`；H1=40px；按钮 80px；若是平面直角坐标系/函数图像：用户未明确要求“方格线/网格线/坐标网格/坐标纸/方格纸/方便读点”时只保留轴/短刻度/标签/曲线/必要语义辅助线，不生成完整坐标方格；用户明确要求时必须生成可见且与轴/刻度对齐的坐标方格，并在 `test_html` 中断言方格线和坐标轴都存在；若是统计直方图/柱状图/条形图：不套用坐标系方格禁令，必须保留清晰的图表轴线、轴标签、刻度/分箱标签，允许图表内部参考线；**`#controls` 未设 `overflow-y:auto|scroll`**；`<body>` 仅允许 `overflow:hidden`
