...
- **内容组织模式**（**先于布局决定**，见"内容组织模式"章节）：
  ...
  - 禁止直接把大量内容摊平成一条滚动长页
- **配色方案**（**按学科路由二选一，禁止混用**）：
  - **数学场景**：按 [math-design/workflow.md](math-design/workflow.md) Phase 2–4 **机械抽选** 1 套 A/B 色板（Phase 3 算式覆盖完整 pool，A-01/B-12 与其它编号同权参与抽选）→ `read_file` 对应段落 + visual-impact.md → 将 hex 映射为 `--primary` 等 CSS 变量；**Phase 5 硬约束**：HTML **第一行**（`<!DOCTYPE html>` 之前）必须写入 `<!-- html-authoring:math-design palette=XX -->`，缺则 `edit_file` 补上后才可 `terminate`
...
### 4. 生成或修改 HTML
- 首次创建 → `create_file`
- **数学场景 create_file 后必做（Phase 5 硬约束）**：HTML **第一行**必须是 `<!-- html-authoring:math-design palette=XX -->`；若不是 → `edit_file` 补首行 → 再自检 → 才可 `terminate`
...
- Phase 3 **机械抽选**（A-01/B-12 与其它编号同权）：
  ```
  hash = len(prompt) × 31 + len(keyword) × 7
  pool=A → palette_id = A-{(hash mod 11)+1:02d}
  pool=B → palette_id = B-{(hash mod 20)+1:02d}
  ```
  推理中须写出完整算式；列表顺序仅作索引，禁止主观偏好/跳号
- 布局变体（**L1底栏 / L2左栏 / L3右栏**）按 `(len(prompt)×3 + len(keyword)×5) mod 3` 选定，详见 workflow.md §2.2
...
**视觉层（必检）：**
- [ ] **数学场景**：已 `read_url`/`read_file` math-design 子文件（workflow + 色板 + visual-impact）；HTML **第一行**为 `<!-- html-authoring:math-design palette=XX -->`；Phase 3 算式（含 hash/seed）已在推理中写出，palette_id 与算式结果一致（含 A-01/B-12 命中场景）；**布局变体**（L1/L2/L3）已声明且与 `(len(prompt)×3+len(keyword)×5) mod 3` 一致或符合例外覆盖规则；Stage `display:flex; align-items:center; justify-content:center`，演示主体居中；H1=40px；按钮 80px
...
### 示例 M：数学场景（走 math-design 子链路）

**用户：** "做一个勾股定理证明教学动画，输出单页互动 HTML"

**Agent 推理（每次必须按算式重算，禁止照搬本示例编号）：**
- 学科路由：数学 → read `math-design/workflow.md`
- 学段：初中 → pool B（20 套，含 B-12）
- **Phase 3 机械抽选**：
  - keyword=`勾股定理`(len=4)；prompt 长度按实际用户句计，本例 len(prompt)=25
  - `hash = 25 × 31 + 4 × 7 = 775 + 28 = 803`
  - `seed = 803 mod 20 = 3` → `palette_id = B-04`（不同 prompt 会得到不同编号；若命中 B-12 必须采用，不得跳号）
- **布局变体**：
  - `layout_hash = (25 × 3 + 4 × 5) mod 3 = (75 + 20) mod 3 = 95 mod 3 = 2` → **L3 右栏**
  - 控件以「上一步/下一步/重置」为主，无强例外 → 维持 L3
- Phase 4：`read_file` color-palettes-b.md 中 **B-04** 段落 + visual-impact.md
- 配色：将 B-04 hex 映射为 CSS 变量（禁止参考配色「沉稳橙」）
- 排版：H1=40px，按钮 80px；Stage `display:flex; align-items:center; justify-content:center`；Controls 在右栏（宽 ≤ 28%）

**Agent 执行：** `create_file` 生成 HTML，首行注释 `<!-- html-authoring:math-design palette=B-04 -->`（与算式一致；下一条 prompt 几乎必然得到不同编号与不同变体）
